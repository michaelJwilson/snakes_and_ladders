...
| [`python/snakes_and_ladders/sandbox/CLAUDE.md`](python/snakes_and_ladders/sandbox/CLAUDE.md) | The home of the second implementation: what a framework replaced, or the framework front that lost, kept either way to referee the other |
...
| `CHECKS.md`, `SEAMS.md` | The checks the claims rest on, read from the tests; the protocols and shared contracts with their implementers and consumers, read from the package. Both are generated and not committed (issue #425): write them with `infra/ledgers.sh`, or read them from a CI run's summary |
...
| [`docs/paper.pdf`](docs/paper.pdf), [`docs/textbook.pdf`](docs/textbook.pdf) | The rendered documents, committed and regenerated by `infra/build_documents.sh` |
