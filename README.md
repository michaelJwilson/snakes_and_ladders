...
Ten required checks run on every pull request: a title naming the base
...
| [`docs/tex/api_map.tex`](docs/tex/api_map.tex) | The API map: the package's Python surface, typeset from its own docstrings and edited by no one (issue #576) |
| [`docs/tex/mind_map.tex`](docs/tex/mind_map.tex) | The mind map: the two concerns, the packages, and the milestone each module is claimed against (issue #664) |
| [`docs/paper.pdf`](docs/paper.pdf), [`docs/textbook.pdf`](docs/textbook.pdf), `docs/api_map.pdf`, `docs/mind_map.pdf` | The rendered documents, committed and regenerated by `infra/build_documents.sh` |
...
| [`docs/blue_sky.md`](docs/blue_sky.md) | Stage 5 candidates: every earlier proposal with its state, and each new one with the oracle that referees it |
...
| [`docs/reviews/`](docs/reviews) | Dated reviews of the whole between releases: what is established, what disagrees, what is missing, what is next |
...
informs one concern, and `REFERENCES.md` holds the full list with the rule that a
