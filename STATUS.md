...
over the five problems, 76 pinned by a named `oracle` test and 13 carrying the
ticket instead of one --- the Potts ladder pinned to the foot, each of its five
open rungs against the rung below it (issue #734, step 2), and the tree ladder
with it, its four against the closed-form mixture, enumeration, pruning and the
analytic surrogate (step 3).
