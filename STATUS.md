...
**Modules.** The generators and the registry this milestone's ground truth comes from: `sim.jc`, `sim.gtr`, `sim.simulate`, `sim.simulator` (the one way to simulate, #829), `sim.tree`, `sim.newick`, `sim.params`, `sim.css`, `sim.emission_mixture`, `sim.rust.count_pairs` and `sim.fixtures`, which declares every instance the suite is checked on. `sim.galois`, `sim.reed_solomon`, `sim.elementary_codes` and `sim.capacity` are the algebraic codes and the capacity they are read against (#700); `sim.polar` is the polar construction, promoted from `sandbox` with its row (#826).
...
**Modules.** The evaluators and the oracle they are pinned to: `likelihood.brute_force`, `likelihood.parsimony`, `likelihood.algebraic`, `likelihood.polar` (successive cancellation and the list, #826), `likelihood.css`, `likelihood.mixture_assignments`, `likelihood.schedule`, `likelihood.rust.spatio_sequential`, and `likelihood.device`, which owns the cross-device tolerance this milestone's claims are stated against. `likelihood.rust.ragged` is the compiled kernel behind the ragged path, conserved beside `sandbox.rectangular_hmm` (#666).
...
1. **`inference_mode` on evaluation-only paths.** Every such path already runs
   ...
   `likelihood.torch.pruning`, `learn.policy`, `planning`, `critic`, `ppo`,
...
`likelihood.rust.message_passing` --- the tree schedule's two passes in Rust,
the default since
...
`likelihood.rust.convolutional`, is 26.2x the decode at the declared
...
`likelihood.jax.pruning` traces `pruning_torch`'s post-order once per
...
`likelihood.rust.pruning`, `pruning_torch`, `pruning_analytic`,
...
recursions and `likelihood.rust.convolutional` marshals the two trellis
...
constraint on it --- and `likelihood.rust.message_passing` marshals the
