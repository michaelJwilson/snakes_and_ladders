...
component --- and the Gumbel-softmax relaxation of :mod:`snakes_and_ladders.learn.relaxed`
...
:mod:`snakes_and_ladders.learn.relaxed` makes for the simplex holds for the box,
term by term). The dynamics ascend ``S`` while the ramp pulls ``x`` to
``{-1, +1}``; the readout is the label with the largest ``x`` at each site,
and its energy is :func:`snakes_and_ladders.sim.potts.energy` on that labelling,
...
class BifurcationResult:
    ...
    labelling : np.ndarray
        ...
        of the best replica.
    energy : float
        :func:`snakes_and_ladders.sim.potts.energy` of that labelling --- the
...
def simulated_bifurcation(
    ...
    *,
    ...
    field_values : np.ndarray
        ...
        :func:`snakes_and_ladders.sim.potts.site_field`.
    n_states : int
        ``q >= 2``.
    rng : np.random.Generator
