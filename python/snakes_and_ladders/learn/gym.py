...
It lives beside the interface it adapts (issue #779) and imports no
application module. ``gymnasium`` is the ``frameworks`` extra rather than a
core dependency, and the package's lazy submodule ``__getattr__`` is what keeps
that optional: importing ``snakes_and_ladders.learn`` does not import this
module.
"""
...
class GymnasiumEnvironment[S, A](gymnasium.Env[Observation, int]):
    ...
    environment : Environment[S, A]
        ...
        :class:`~snakes_and_ladders.learn.tree.TreeEnvironment` keeps its cache
