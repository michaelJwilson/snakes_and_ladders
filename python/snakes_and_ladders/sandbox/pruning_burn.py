...
Route A of issue #449, declined and conserved. The recursion is rebuilt in
Rust out of ``burn_tensor`` operations and differentiated by ``burn-autodiff``
...
33.06, with the kernel alone at 48.01 ms by Criterion, so the boundary is
not the reason; ``docs/experiments/007-pruning-gradient-routes.md`` carries
the comparison. The route is kept so that comparison stays re-runnable, per
this package's ``CLAUDE.md``: only ``tests/`` and ``snakes_and_ladders.qa``
import from here, and no hot path does.
