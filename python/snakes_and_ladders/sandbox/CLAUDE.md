...
Five declined routes, each answering a question its ticket posed and losing on
...
And three framework fronts, each measured against the implementation it would
...
they are checked against — and those stay where they are used. The candidate
the ticket named that is still open is `learn.ppo`'s advantage estimate and
clipped loss behind TorchRL; it moves in with the measurement that settles it,
whichever way it settles, and `TICKETS.md` carries it as a follow-up.
