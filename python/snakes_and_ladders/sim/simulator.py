...
:data:`snakes_and_ladders.sim.fixtures.LOADERS` is, so the loader and the
...
from snakes_and_ladders.sim.potts import SimulatedPottsDataset, simulate_potts
...
def simulate_potts_lattice(
    ...
    """:func:`~snakes_and_ladders.sim.potts.simulate_potts` on the lattice the fixture declares."""
...
def simulate_spatio_only(
    ...
    """:func:`~snakes_and_ladders.sim.potts.simulate_potts` on the fixture's own graph and field."""
...
def simulate_potts_chain(params: PottsParams, rng: np.random.Generator) -> np.ndarray:
    """The exact open-chain draw, as :func:`snakes_and_ladders.opt.potts.simulate_chains` makes it.
