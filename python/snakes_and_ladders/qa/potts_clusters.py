...
:class:`~snakes_and_ladders.opt.starts.StartsBenchmark` with the notebook's
...
Run as ``uv run --no-sync python -m snakes_and_ladders.qa.potts_clusters
...
class DiagnosedClusters(ClusterCounter):
    """:class:`~snakes_and_ladders.sample.potts_mcmc.ClusterCounter` that also reads each cluster against the field.
