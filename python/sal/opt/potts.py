...
:func:`sal.opt.constrain.log_simplex`. Without that the model is
unidentifiable and a fitted field has no value to compare against truth.

**Relationship to** :mod:`sal.sim.potts`. That module
...
``sal.sim``. Issue #186 tracks the duplication, by moving
``PottsParams``/``PottsParams`` out of ``sal.opt`` as
issue #171 moved ``sal.opt.hmm``'s truth type into
``sal.sim.hmm``.
"""
...
class PottsLatticeObjective(_PottsObjectiveBase):
    ...
    parameters. Nothing in ``sal.opt.fit`` changes to carry it,
...
class PottsObjective(_PottsObjectiveBase):
    """Negative log-likelihood of Potts chains, as an :class:`~sal.opt.objective.Objective`.
