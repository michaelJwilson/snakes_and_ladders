...
ran every start over its seeds through :func:`~sal.opt.budget.compare`,
...
**A start is an** :class:`~sal.opt.initialize.Initializer`, **or
...
:func:`~sal.opt.budget.compare` gives the cell, so each
...
:func:`~sal.opt.budget.restarts`, so a restart baseline is a
start like any other rather than a second loop. The seeding budget, in
:attr:`~sal.cost.Cost.EVALUATIONS`, bounds the starts the seam
scores, one objective evaluation each; what an initializer spends inside
:meth:`~sal.opt.initialize.Initializer.starts` is read in
seconds, since a chain and a quantile share no other unit.

**Every cell runs under** :func:`sal.track.track`. The seam
...
(:meth:`sal.track.MemoryRun.stamps`) its seconds, and the entry
...
host (:attr:`~sal.cost.Cost.SECONDS`), so
:meth:`SolverComparison.table` holds none and :meth:`SolverComparison.readings`
holds them.

The renderers are in `sal.qa.starts`: `qa/` imports `opt/`, and
...
class Polished:
    ...
    :class:`~sal.opt.objective.Objective`, and
    :class:`~sal.search.mixture_starts.MixturePolished`, a
...
class Trial:
    ...
    subclass's: :class:`~sal.search.projection.ProjectedTrial`
    for the projected fit, and
    :class:`~sal.search.mixture_starts.MixtureTrial` for a
...
def polish_by_fit(
    ...
    """:func:`sal.opt.fit.fit` from ``theta``, one L-BFGS iteration a unit.
...
def _mixture_at(objective: Objective, theta: torch.Tensor) -> tuple[Any, Any, Any]:
    ...
    ``log_weight`` key of :meth:`~sal.opt.objective.Objective.constrain`
    --- which :class:`~sal.opt.mixture.GaussianMixtureObjective`
...
def polish_by_emission_em(
    ...
    """:func:`sal.opt.emission_mixture.expectation_maximization` from ``theta``.
...
def polish_by_baum_welch(
    ...
    Through :func:`~sal.opt.hmm.baum_welch_family`, which
...
@dataclass(frozen=True)
class _Cell:
    """One start as an :mod:`sal.opt.budget` method; a dataclass so it pickles."""
...
class StartsRow:
    ...
    gap : float
        ...
        :meth:`~sal.opt.budget.Comparison.mean_gap`.
    spent : int
...
class SolverComparison:
    ...
    comparison : Comparison
        The underlying :class:`~sal.opt.budget.Comparison`,
...
class StartsBenchmark:
    ...
    `sal.qa.mixture_seeding` and the starts notebook
