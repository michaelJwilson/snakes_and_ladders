...
:mod:`sal.learn.exact`'s enumeration depends on it: the
...
* the same episode replays identically on any thread, which
  ``sal.parallel`` needs and a shared generator would deny.
...
def state_key(key: int, state: np.ndarray, action_code: int) -> int:
    ...
    action_code : int
        ...
        :meth:`~sal.learn.potts_nd.PottsNDEnvironment.action_code`
...
class KeyedMove(Protocol):
    ...
    The implementations live in ``sal.search``, which may import
    ...
    :class:`~sal.learn.potts_nd.PottsNDEnvironment`.
