...
:class:`~sal.learn.potts.PottsEnvironment`, and both are the
point:
...
Randomness is keyed, not streamed (:mod:`sal.learn.keyed`), so
...
from sal.sample.accept import accept_at_temperature
...
class FeatureColumn(StrEnum):
    ...
    :class:`~sal.learn.environment.Environment`'s gauge rule
...
class PottsNDEnvironment(Environment[Configuration, PottsAction]):
    ...
    edges : Sequence[tuple[int, int]]
        ...
        :meth:`~sal.learn.potts.PottsEnvironment.on_graph`
    ...
    moves : Mapping[MoveKind, KeyedMove] | None
        ...
        admits, from :func:`sal.sample.potts_keyed.cluster_moves`
    ...
    generator : np.random.Generator
        ...
        (:mod:`sal.learn.keyed`). A generator rather than a
    ...
    def _set_neighbours(self, edges: Sequence[tuple[int, int]]) -> None:
        ...
        :class:`~sal.learn.potts.PottsEnvironment`'s, for the
    ...
    def action_code(self, action: PottsAction) -> int:
        """A distinct integer per action, for :func:`~sal.learn.keyed.state_key`.
    ...
    def step(
        ...
        :class:`~sal.learn.environment.Episode` states. It is
    ...
    def is_terminal(self, state: Configuration) -> bool:
        ...
        :class:`~sal.learn.potts.PottsEnvironment` ends an
        ...
        :class:`~sal.learn.environment.Episode` reports the
        truncation.
        """
        del state
        return False
