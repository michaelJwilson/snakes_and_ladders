...
:class:`sal.learn.environment.Environment`, beside
:mod:`sal.learn.potts`, :mod:`sal.learn.potts_nd`
and :mod:`sal.learn.hmm`, which are the same object for the
other problems (issue #779). With :mod:`sal.learn.ranking` it is
one of the two modules here that import an application package --- the topology
moves and the likelihood they are scored by. Everything else in ``learn/``
imports none, which is what keeps an agent from being shaped by trees;
``tests/regression/learn/test_learn_environment.py`` names the two exceptions
and refuses a third.
...
:mod:`sal.qa.rl_reward_surface` measures how far apart the two
are.
...
from sal.search.support import split_pattern_support
...
def exchanged_subtrees(
    ...
    :func:`sal.sim.topology.leaf_bipartitions`: the anchor
...
class TreeEnvironment(Environment[Topology, Topology]):
    ...
    branch_length : float
        ...
        :mod:`sal.qa.rl_reward_surface` measures how much the resulting
    ...
    def split_support(self, split: frozenset[str]) -> float:
        ...
        :func:`sal.search.support.split_pattern_support`, which
