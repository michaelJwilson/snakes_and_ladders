...
:class:`sal.sim.graph.PottsGraph` permits a doubled bond, a
...
class RelaxationMode(StrEnum):
    ...
    :class:`sal.sim.graph.BoundaryCondition` is one: an
...
class RelaxedHmmPath(RelaxedObjective):
    ...
    :func:`sal.likelihood.hmm_paths.path_log_probability` exactly, which a
...
def enumerate_optimum(objective: RelaxedObjective) -> tuple[Configuration, float]:
    ...
    :func:`sal.learn.potts.optimum` plays for the chain and exhaustive
    topology enumeration plays for tree search. The three share one body,
    :func:`sal.enumeration.enumerated_optimum` (issue #755).
    """
...
def optimize(
    ...
    *,
    ...
    final_temperature : float | None
        ...
        it over ``steps`` on
        :class:`sal.sample.schedule.ExponentialTempSchedule`:
