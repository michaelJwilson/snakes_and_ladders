...
does for :class:`~sal.opt.objective.Objective`. An interface justified by
...
from sal.enumeration import configurations, enumerated_optimum
...
class PottsEnvironment(Environment[Configuration, Flip]):
    ...
    def on_graph(
        ...
        differs. `sal.sim.graph.lattice_graph` builds the N-D lattices this
    ...
    def coupling(self) -> float:
        ...
        Exposed because :mod:`sal.learn.relaxed` evaluates the same score on
...
def enumerate_configurations(
    ...
    plays for tree search in ``sal.sim.topology``. The
    space is :func:`sal.enumeration.configurations`, so the
...
def optimum(environment: PottsEnvironment) -> tuple[Configuration, float]:
    ...
    :func:`sal.enumeration.enumerated_optimum`; what is local
