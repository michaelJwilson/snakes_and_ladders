...
:mod:`sal.sample.hmc`, and run on JAX, a second autodiff stack
the package does not take on: autodiff here is PyTorch by decision, so JAX
runs only in ``scripts/blackjax.py``, in a subprocess.

:func:`integrate` runs the package's :class:`~sal.sample.hmc.Integrator`
...
def coefficients(weights: tuple[float, ...]) -> np.ndarray:
    ...
    :class:`~sal.sample.hmc.Integrator`, each a kick-drift-kick
...
def mala(
    ...
    *,
    ...
    :func:`sal.sample.langevin.mala` makes at ``h``.
    """
    result = run(
        SCRIPT,
        {
...
def replay(
    ...
    *,
    ...
    so :func:`sal.sample.metropolis.replay` runs the same chain.
    """
    result = run(
        SCRIPT,
        {
