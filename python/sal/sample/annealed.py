...
:func:`~sal.sample.potts_mcmc.sweep_for` once per rung, so
every move set :class:`~sal.sample.potts_mcmc.PottsMove`
...
are judged against :func:`~sal.likelihood.potts.strip_log_partition`
...
:attr:`~sal.sample.tempered.TemperedEnsemble.walkers`, so
:func:`~sal.sample.tempered.round_trips` reads it unchanged.
"""
...
def geometric_betas(beta: float, n_rungs: int, *, beta_min: float) -> tuple[float, ...]:
    ...
    exact. The rest is
    :class:`~sal.sample.schedule.ExponentialTempSchedule`'s own
...
def annealed_importance_sampling(
    ...
    *,
    ...
    rung_log_z = [log_zero]
    # One lookup, one record per rung (`sal.track`, issue
...
class SimulatedTempered:
    ...
    def walkers(self) -> np.ndarray:
        ...
        The shape
        :func:`~sal.sample.tempered.round_trips` reads, so a
...
def simulated_tempering(
    ...
    *,
    ...
    n_sweeps, burn_in, thin : int
        As :func:`~sal.sample.potts_mcmc.sample_potts`. A
