...
Three of :mod:`sal.opt.initialize`'s starting points are a
...
:class:`~sal.opt.initialize.Initializer` seam and are reached
...
spend. They live here rather than beside :class:`~sal.sample.hmc.Adaptation`
because :func:`~sal.sample.hmc.parallel_tempering` takes a
...
#: These are the values :data:`~sal.sample.hmc.DUAL_AVERAGING_GAMMA`
...
#: Gelman's target for HMC. :mod:`~sal.sample.hmc` states no
...
class FromChain(Initializer):
    ...
    **The chain is warmed up by default.** Unless ``adaptation`` says
    ...
    :class:`~sal.sample.hmc.Adaptation`, ``step_size`` is the
...
class FromTempering(Initializer):
    ...
    **The ladder may be calibrated first, and the calibration is a warm-up**
    ...
    :func:`~sal.sample.hmc.parallel_tempering` and revises
    them by :func:`~sal.sample.schedule.adapt_ladder` or
    :func:`~sal.sample.schedule.adapt_ladder_by_round_trips`,
    ...
    **A budget in seconds is a ceiling the rounds are predicted inside.** With
    ``n_rounds`` a :class:`~sal.opt.budget.Budget` in
    :attr:`~sal.cost.Cost.SECONDS`, the clock starts at the
    ...
    temperatures : tuple[float, ...]
        ...
        :func:`sal.sample.hmc.parallel_tempering` requires;
...
class LadderRule(StrEnum):
    ...
    :func:`~sal.sample.schedule.adapt_ladder`."""
...
class LadderCalibration:
    ...
    :func:`~sal.sample.hmc.parallel_tempering` run of
    ...
    * :attr:`LadderRule.ACCEPTANCE` reads the per-pair exchange acceptance
      and hands it to :func:`~sal.sample.schedule.adapt_ladder`
    ...
    * :attr:`LadderRule.ROUND_TRIPS` reads
      :func:`~sal.sample.tempered.up_fraction` on the walker
