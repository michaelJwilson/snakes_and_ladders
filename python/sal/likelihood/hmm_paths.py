...
as :mod:`sal.likelihood.brute_force`: it shares no recursion with the
forward algorithm in :mod:`sal.opt.hmm`, so agreement between them is
...
the reason :func:`sal.sim.canonical.ambiguous_hmm` exists and the reason
...
#: :data:`sal.enumeration.MAX_ENUMERABLE_CONFIGURATIONS` under the name the
