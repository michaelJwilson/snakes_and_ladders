...
Belief propagation (:mod:`sal.likelihood.belief_propagation`) is checked
against the first where it is exact and *measured* against the second where it
is not.

The model is the one :func:`sal.sim.potts.simulate_potts` samples from, and
...
widened once by :func:`sal.sim.potts.site_field` as it is
...
#: :func:`sal.sample.potts_mcmc.anneal_potts` scores a single
...
def enumerate_potts(
    ...
    :mod:`sal.likelihood.belief_propagation` -- only the model convention,
...
def strip_log_partition(
    ...
    shape : tuple[int, int]
        ...
        node indexing :func:`sal.sim.graph.lattice_graph` uses.
    boundary : BoundaryCondition
        ``OPEN`` only.
    coupling : float
        Uniform ``J``.
