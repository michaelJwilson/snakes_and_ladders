...
It lives here rather than in ``sal.opt`` because
...
``sal.opt.constrain`` and is imported by nothing in
``sal.opt``.
...
class SubstitutionModelObjective(Objective):
    ...
    * ``pi`` goes through :func:`sal.opt.constrain.log_simplex`, which pins
