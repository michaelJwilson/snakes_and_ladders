...
The general :func:`sal.likelihood.message_passing.sum_product`
already decodes a code through
:func:`sal.sim.factor_graph.from_parity_check`, one table per
...
from sal.sim.ldpc import LLR_CAP, ParityCheck, generator_matrix
...
class CodeMetrics:
    ...
    * ``syndrome_weight`` is the number of unsatisfied checks,
      :meth:`sal.sim.ldpc.ParityCheck.syndrome` summed. Zero
    ...
    sent : np.ndarray
        ...
        :func:`sal.sim.ldpc.all_zero_transmission` sends the
        zero word.
    """
...
def enumerate_codewords(code: ParityCheck) -> np.ndarray:
    ...
    :func:`sal.sim.ldpc.generator_matrix`, each checked against
    ...
    ------
    ValueError
        Past :data:`sal.enumeration.MAX_ENUMERABLE_CONFIGURATIONS`
