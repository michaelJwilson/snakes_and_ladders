"""Rust CPU Felsenstein pruning (`sal.oxisal.pruning_log_likelihood`) --
...
given, so site-pattern weights (``sal.likelihood.patterns``)
...
last), by ``sal.likelihood.pruning_common.postorder``, which
...
def log_likelihood(
    ...
    *,
    ...
    ``sal.oxisal.pruning_log_likelihood`` expects and calls the compiled
    Rust kernel.
    ...
    weights : np.ndarray | None
        ...
        ``sal.likelihood.pruning``. The kernel takes no
