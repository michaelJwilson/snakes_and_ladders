...
:func:`sal.likelihood.message_passing.sum_product` choosing
...
:class:`~sal.likelihood.schedule.Guarantee` rather than a boolean, and a caller that wants a cheaper
or a partial answer asks for it instead of reading a field that quietly means
something narrower than it says.

**One interface reaches every graph.** :mod:`sal.sim.factor_graph`
...
schedule was the only part that did not generalise; over :class:`~sal.likelihood.schedule.Layout` it
...
:mod:`sal.sample.schedule`. They were both called ``Schedule``,
...
class Layout:
    ...
    def tree_passes(self) -> tuple[list[Step], list[Step]]:
        ...
        :mod:`sal.likelihood.message_passing_reference` is. A
    ...
    def sequential_steps(self) -> list[Step]:
        ...
        :mod:`sal.likelihood.ldpc` call through this module.
        """
        return [
            (
...
class MessageSchedule(ABC):
    ...
    def compiled(self) -> bool:
        """Whether :mod:`sal.likelihood.message_passing_rust` runs this order.
        ...
        bitwise against :func:`sal.likelihood.pruning.log_likelihood`,
...
class UpwardMessageSchedule(MessageSchedule):
    ...
    def log_partition(
        ...
        agrees with :func:`sal.likelihood.pruning.log_likelihood`
...
def resolve(
    ...
    """A :class:`~sal.likelihood.schedule.Schedule` from itself, its enum member, or its name.
