...
against `sal.likelihood.ldpc.exact_decoding` rather than cited
...
from sal.sim.elementary_codes import crc_remainder
from sal.sim.ldpc import LLR_CAP
from sal.sim.polar import PolarCode, polar_transform
...
def decode_scl(
    ...
    *,
    ...
    llr : np.ndarray
        ...
        ``sal.sim.ldpc`` states: positive favours zero.
    list_size : int
...
def crc_encode(payload: np.ndarray, crc: np.ndarray) -> np.ndarray:
    ...
    ``crc`` is the generator :func:`sal.sim.elementary_codes.crc_remainder`
