...
approximation :mod:`sal.likelihood.belief_propagation` computes
...
:mod:`sal.likelihood.message_passing_reference` is the
...
:class:`~sal.incidence.SparseIncidence` would pay nothing
...
:data:`~sal.backend.Backend.PYTHON`. Which schedules it runs
is the schedule's own answer ---
:attr:`~sal.likelihood.schedule.MessageSchedule.compiled` ---
...
def _logsumexp_last(values: np.ndarray) -> np.ndarray:
    """:func:`sal.numerics.logsumexp` over the last axis, keeping it.
...
class _Run:
    ...
    scale : float | None
        ...
        :meth:`~sal.likelihood.schedule.MessageSchedule.log_partition`
...
def max_product(
    ...
    *,
    ...
    :func:`sal.likelihood.hmm_paths.enumerate_hidden_paths` uses.
