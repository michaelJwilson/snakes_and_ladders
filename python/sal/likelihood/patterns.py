...
``pruning``, ``likelihood.pruning.torch`` and ``likelihood.pruning.rust`` all take a ``weights``
...
cannot take them (``likelihood.pruning.rust`` says how it works around that), which is
...
def compress(alignment: Mapping[str, np.ndarray]) -> SitePatterns:
    ...
    alignment : Mapping[str, np.ndarray]
        ...
        what ``sal.sim.simulate.SimulatedDataset.alignment``
        produces.
