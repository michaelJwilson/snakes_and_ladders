...
:mod:`sal.learn.relaxed` took the Gumbel-softmax one.
...
``tests/`` and :mod:`sal.qa` import it, and
:mod:`sal.qa.tropical_relaxation` keeps reporting the figure the
paper cites.
...
condition of ``sec:spectral-start`` (:func:`sal.search.neighbor_joining.four_point_violation`
...
fit :func:`sal.search.infer.score_topology` performs
...
tropical analogue of :mod:`sal.learn.relaxed`'s
...
#: :func:`sal.search.neighbor_joining.four_point_violation`
...
#: :data:`sal.learn.relaxed.MINIMUM_TEMPERATURE`.
...
def corner(topology: Topology, branch_length: float = 1.0) -> np.ndarray:
    ...
    The analogue of :func:`sal.learn.relaxed.one_hot`. Every
    ...
    :func:`sal.likelihood.distance.tree_distances` instead,
...
def anneal(start: float, end: float, steps: int, step: int) -> float:
    ...
    Geometric for the reason :func:`sal.learn.relaxed.anneal`
...
def metric_from_split_weights(
    ...
    weights : Mapping[frozenset[str], float]
        Split to weight, as
        :func:`sal.likelihood.hadamard.split_weights` returns.
