...
:mod:`sal.likelihood.message_passing`'s Bethe value and the
updates find that module's fixed point. Only ``tests/`` and
:mod:`sal.qa` import it.
...
:mod:`sal.likelihood.message_passing` already computes --- so
...
def lattice_plaquettes(shape: tuple[int, int]) -> list[frozenset[str]]:
    ...
    **This function knows one naming convention**, the row-major ``s{index}``
    that :func:`sal.sim.factor_graph.from_potts` writes, and
