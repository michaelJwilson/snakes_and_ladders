...
Each satisfies :class:`sal.opt.initialize.Initializer` on the
...
(:mod:`sal.likelihood.distance`) and neighbor joining
(:mod:`sal.search.neighbor_joining`), the Jukes--Cantor
...
conjugation (:mod:`sal.likelihood.hadamard`) on the
...
from sal.search.neighbor_joining import neighbor_joining, split_lengths
from sal.sim.gtr import n_exchangeabilities
...
def _on_topology(
    ...
    :mod:`sal.likelihood.objective` reports them by.
    """
...
class FromHadamard(Initializer):
    ...
    (:func:`~sal.likelihood.hadamard.binary_recoding`), its
