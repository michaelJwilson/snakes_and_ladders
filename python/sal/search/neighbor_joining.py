...
:mod:`sal.likelihood.distance`). Saitou and Nei's algorithm
...
trifurcating-root convention :mod:`sal.sim.topology` walks
...
from sal.sim.topology import NodeId, from_adjacency, leaf_names_of
from sal.sim.tree import Node, edges
...
def split_lengths(tau: Node) -> dict[frozenset[str], float]:
    """Branch length per split, keyed as :func:`~sal.sim.topology.branch_splits` keys them.
