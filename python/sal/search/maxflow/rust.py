"""Rust Boykov--Kolmogorov max flow (`sal.oxisal`), pinned against `sal.search.maxflow`.
...
conserved in :mod:`sal.sandbox.maxflow_declined` behind the
...
:func:`sal.sim.potts.energy`, which issue #341 vectorized
...
from sal.search.maxflow import FlowNetwork, GroundState, MinCut
...
def min_cut(network: FlowNetwork, source: int, sink: int) -> MinCut:
    ...
    :func:`sal.search.maxflow.max_flow`, so a caller chooses
    the solver and nothing else. The network crosses once, as the three
    contiguous arrays :meth:`~sal.search.maxflow.FlowNetwork.as_arrays`
    ...
    **The network is not mutated.** The pure implementation turns its
    ...
    side, so :mod:`sal.search.alpha_expansion`, which needs
