...
Renders what `sal.sim.mixture` and
`sal.opt.mixture` computed; it reimplements no density, no
seeding and no M step (`qa/CLAUDE.md`). Panel (b)'s seedings are the seeding
phase of one :class:`sal.opt.starts.StartsBenchmark` on the
...
from sal.opt.starts import StartsBenchmark, polish_by_fit
...
def seeding_ratios(
    ...
    :class:`~sal.opt.starts.StartsBenchmark`: each start offers
