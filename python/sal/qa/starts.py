...
Every consumer of :class:`sal.opt.starts.SolverComparison`
...
:func:`~sal.search.mixture_starts.curve_band` reads the trials
of an :class:`~sal.opt.starts.SolverComparison` onto the
:class:`~sal.search.mixture_starts.GapBand` the figure takes.
"""
...
from sal.opt.starts import Curve, SolverComparison
...
from sal.search.mixture_starts import GapBand
...
def _encoding(n_starts: int) -> list[tuple[str, str]]:
    ...
    The colours are :func:`~sal.qa.style.discrete_palette`'s;
...
def starts_figure(
    ...
    result : SolverComparison
        What :meth:`sal.opt.starts.StartsBenchmark.run` returned.
    reference_label : str
    ...
    -------
    tuple[Figure, str]
        The figure and a caption :func:`~sal.qa.figure.check_latex_safe` passes.
    """
    curves = {
...
def starts_latex(result: SolverComparison) -> tuple[str, str]:
    ...
    The body and caption :func:`~sal.qa.figure.write_qa_table`
    ...
    tuple[str, str]
        ...
        :func:`~sal.qa.figure.check_latex_safe` passes.
    """
...
def gap_table(result: SolverComparison, *, unit: str) -> tuple[str, str, str]:
    ...
    tuple[str, str, str]
        ...
        :func:`~sal.qa.figure.check_latex_safe` passes, and
        the ``array``.
    """
    header = [
