...
entries `sal.qa.manifest` states, summed from their declared
...
def selected(
    ...
    if only:
        ...
        if missing:
            msg = f"no sal.qa.manifest entry renders {sorted(missing)}"
    ...
    if missing:
        ...
        msg = (
            ...
            f"sal.qa.manifest entry renders; add an entry or "
