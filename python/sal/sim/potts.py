...
An N-D lattice (:func:`sal.sim.graph.lattice_graph`) is a constructed case
of :class:`~sal.sim.graph.PottsGraph`, not a separate code path, per
...
same backward-message recursion :func:`sal.sim.potts_chain.simulate_chains`
...
def energies(graph: PottsGraph, field: np.ndarray, states: np.ndarray) -> np.ndarray:
    ...
    The negated log weight :func:`sal.likelihood.potts.log_weights`
...
class PottsMetrics:
    ...
    A :class:`sal.track.Metrics` over a labelling, satisfied
    ...
    **One metric; the second the ticket listed is left out.** Neither a
    ...
    :func:`sal.sample.potts_mcmc.anneal_potts` records the
...
def local_fields(
    ...
    step (:mod:`sal.sample.balanced`), and a Python loop over
...
class PottsLatticeParams:
    ...
    field : np.ndarray
        ...
        :func:`sal.search.ground_state.lattice_rung` builds it.
    seed : int
...
def _simulate_open_chain_exact(
    ...
    ``sal.sim.potts_chain.simulate_chains`` used before this module existed: the
    ...
    # `sal.opt.potts.log_partition` sums via transfer matrix, one
...
class SpatioOnlyParams:
    ...
    :mod:`sal.sim.spatio_sequential` with the chains and the
...
def _spatio_only_sizes(path: Path, declared: Any, n_nodes: int) -> np.ndarray:
    ...
    :mod:`sal.sim.count_pairs` states its counts.
