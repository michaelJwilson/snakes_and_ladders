...
:mod:`sal.sim.spatio_sequential` at the size the roadmap asks
...
:class:`~sal.emissions.EmissionFamily`, so the swap is the
...
class IndependentCountPair(EmissionFamily):
    ...
    :func:`sal.sim.spatio_sequential.gated_log_density` reads the
...
class IndependentCountPairSeeding:
    ...
    :class:`~sal.opt.emission_mixture.CountPairSeeding`, read
...
def chain_states(
    ...
    draw of
    :func:`~sal.sim.spatio_sequential.simulate_spatio_sequential`
...
def simulate_count_pairs(
    ...
    **A stream per vertex.** Vertex ``v``'s counts come from
    ...
    (:mod:`sal.sim.count_pairs_rust`) be checked against this
    ...
    backend : Backend
        ...
        :mod:`sal.sim.count_pairs_rust`, pinned to it draw for
