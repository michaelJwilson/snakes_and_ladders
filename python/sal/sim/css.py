...
class CssCode:
    ...
    def logical_label(self, vectors: np.ndarray) -> np.ndarray:
        ...
        :func:`sal.likelihood.css.error_cosets` uses it for:
...
def sample_x_error(
    ...
    is :class:`~sal.sim.ldpc.BinarySymmetricChannel` and no
...
class CssBicycleParams:
    ...
    def code(self) -> CssCode:
        ...
        CssCode
            ...
            :func:`~sal.sim.ldpc.bicycle_code` under this
    ...
    def from_declared(cls, declared: Mapping[str, Any], _path: Path, /) -> Self:
        ...
        :func:`~sal.sim.ldpc.bicycle_code`'s and the ``k >
