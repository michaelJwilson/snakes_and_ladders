...
class FactorGraph:
    ...
    def log_density(self, assignment: Mapping[str, int]) -> float:
        ...
        (:meth:`sal.sample.gibbs.Indexed.log_density`, issue
...
def from_hmm(
    ...
    log_density : np.ndarray
        ...
        position under each state, as :func:`sal.opt.hmm.forward_log_likelihood_from_density`
...
def from_tree(
    ...
    :func:`sal.likelihood.pruning.log_likelihood` at that site (``eq:pruning``,
...
def from_parity_check(code: ParityCheck, llr: np.ndarray) -> FactorGraph:
    ...
    :func:`sal.likelihood.message_passing.sum_product` on it is
...
def from_trellis(
    ...
    *,
    ...
    :func:`sal.likelihood.convolutional.bcjr`, and the graph
    is a chain, so :func:`sal.likelihood.message_passing.sum_product`
