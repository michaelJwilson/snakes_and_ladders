...
``sal.opt.hmm`` holds the recursions that consume it.
...
:mod:`sal.numerics` on the same terms: importable from
...
through :func:`~sal.emissions.base.as_tensor`, reproducing the
...
:mod:`~sal.emissions.counts` calls are public in
:mod:`~sal.emissions.mstep` --- and one that belongs to neither
...
#: :data:`sal.emissions.mstep.M_STEP_BACKEND`. Declared, not
