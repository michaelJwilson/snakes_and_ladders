...
:mod:`sal.log` takes, for the same reason. A loop calls
...
:mod:`sal.parallel`'s thread or process backend starts from the
...
def as_aim(fig: Figure) -> Any:
    ...
    accepts a matplotlib ``Figure``, which is what :mod:`sal.qa`
