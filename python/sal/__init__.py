...
``import sal`` (issue #301) handed a user 150 modules and
...
_SURFACE: dict[str, tuple[str, str | None]] = {
    "Backend": ("sal.backend", "Backend"),
    "Environment": ("sal.learn.environment", "Environment"),
    "FactorGraph": ("sal.sim.factor_graph", "FactorGraph"),
    "Objective": ("sal.opt.objective", "Objective"),
    "Params": ("sal.fixtures", "Params"),
    "PottsGraph": ("sal.sim.graph", "PottsGraph"),
    "Simulator": ("sal.sim.simulator", "Simulator"),
    "fixture": ("sal.sim.fixtures", "fixture"),
    "load_params": ("sal.fixtures", "load_params"),
    "parallel": ("sal.parallel", None),
}
...
def _submodules(package: str) -> Callable[[str], ModuleType]:
    ...
    ``import sal``, ``sal.sim`` is an ``AttributeError``
    until ``sal.sim`` has been imported somewhere. Every
