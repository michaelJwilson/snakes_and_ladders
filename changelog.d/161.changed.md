...
`opt/` to 163.7 s. The saving is uneven because the import graph is: `sal.qa`
imports four of the other five modules and is 33.6% of the suite's time, so
most changes reach it, while `sal.learn` is imported by nothing.

Three rules keep it honest. Dependents come from the import graph, so a change
to `sal.likelihood` runs `sal.search`, which imports it. A change that
