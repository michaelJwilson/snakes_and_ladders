...
`sal.likelihood.distance` estimates pairwise distances from an
alignment — the Jukes–Cantor closed form and the log-det distance — each with
its delta-method variance; `sal.search.neighbor_joining` builds
the tree from them, exact on additive distances, with the four-point condition
and Atteson's radius beside it; `sal.likelihood.hadamard` is the
...
and `FromHadamard` in `sal.search.initialize` are the fourth and
