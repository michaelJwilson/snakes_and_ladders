...
Branch protection names each required check by the job's `name:`, not by its id, so renaming one is two changes. Issue #377 renamed the LaTeX job to **Documents (paper and textbook)**, and merged as #379 on 2026-09-08; until the maintainer replaced its former name in the branch protection, the renamed job reported and the retired entry sat pending, blocking every merge. Confirming that replacement was a precondition of the 0.5.0 release, and the 0.5.0 audit confirms it **met**: `main` took three merges on 2026-09-09 after the rename, and a pull request opened that day reports ten checks including `Documents (paper and textbook)` with a `clean` mergeable state rather than a blocked one.
