...
| `infra/build_documents.sh` | Regenerates QA figures, checks citation integrity (`infra/check_citations.py`), then builds `docs/paper.pdf`, `docs/textbook.pdf` and `docs/mind_map.pdf` (all committed, rebuilt only by a "Rebuild the documents" pull request; the `.aux`, `.bbl`, `.log` and other files `latexmk` leaves beside them are ignored). `--no-figures` drops the regeneration, for the one caller that has already rendered and compared every figure (`infra/release.sh`, issue #530). |
...
Pull requests change `docs/tex/` and never `docs/paper.pdf`, `docs/textbook.pdf` or `docs/mind_map.pdf`. The three PDFs are committed for readers and are rebuilt, from `main`, by the pull request a **Rebuild the documents** ticket asks for (`.github/ISSUE_TEMPLATE/documents.yml`), whose title carries that phrase; CI refuses a PDF change in any other pull request --- a *change*, not an addition: the rule is about rebuild churn, so the pull request that introduces a document adds its PDF and is reviewed as the new file it is (issue #664). Between rebuilds the PDFs lag the sources by design (issue #369): comparing the committed PDFs on every pull request made every document pull request conflict with every merge and cost one rebuild per merge per open pull request, eleven builds in one afternoon. `infra/build_documents.sh` runs against the synced environment (`uv run --no-sync`) and no longer rebuilds the Rust extension per build.
...
| `docs/paper.pdf`, `docs/textbook.pdf`, `docs/mind_map.pdf` | rewrites all three on every run, whether or not a figure changed | yes, and never by an ordinary pull request |
...
One row of that table needs a rule, the row both rewritten every run and tracked: **revert `docs/paper.pdf`, `docs/textbook.pdf` and `docs/mind_map.pdf` after any local build**. A byproduct cannot be committed by accident and a figure should be; a PDF is the only output where the right action is neither. `infra/review_gates.sh` and the `lint` job both refuse the change, so the cost of forgetting is a failed gate rather than a wrong `main`. `tests/regression/docs/test_document_build_outputs.py` holds the table to the manifest, the script and `git`.
