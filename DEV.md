...
Building on merge rather than per pull request is issue #488: at 15.8 s of LaTeX plus 305.3 s of renders (issue #433, and every cited figure since #490) the build is the wrong cost on the path a merge waits for, and a document that only built at a release would accumulate a release's worth of breakage with the author of each long gone. On merge the blame window is one commit wide, so a red `documents` on `main` is a defect to fix and never a known-bad background state.
...
4. **The render is committed** --- `<stem>.pdf` or `<stem>.tex`, `<stem>_caption.txt` and `<stem>.inputs` --- **and the document cites it**, from the section the request named. The caption names the seed, the sizes and the model, and the body says why the figure is there without restating what the caption says (`docs/CLAUDE.md`).
...
* **Four tiers, three budgets.** A test's tier is decided by *what its size is for*, never by how slow it happens to be: a size chosen so an exact oracle stays available is a CI size even when it is slow, and a size chosen to show behaviour at scale is a stress size even when it is fast.
  ...
  * **The local validation is one command and 300 s.** `infra/validate.sh` runs, against the diff from `origin/main` (or `--base <ref>`), in the order that fails fastest and with each step's wall clock printed: `ruff` and `mypy --strict` over the changed Python files (CI keeps the whole `files` set); `pytest -m critical`; the tests `infra/select_tests.py` names for the diff, capped at 180 s, with `--durations=10`; `sphinx-build -W` only when `docs/source/` or a docstring changed, keeping its doctrees under the scratch directory so a second run is seconds; the notebook checker, which executes all six notebooks under `docs/nb/` in 137 s and never a subset a digest chose (issue #480); and the document build when `docs/tex/` changed, which renders only the cited figures whose inputs changed (issue #372). Three classes of change against the budget, measured on the reference host (4 cores, load under 1, one BLAS thread per process), stated in the pull request that measured them and reproduced by `infra/measure_test_budget.sh`:
  ...
  * **The document build: every cited figure rendered, 305.3 s of renders plus 15.8 s of LaTeX.** Measured 2026-09-09 on the 4-core reference host, by hashing the tree before and after each run (issue #429). The renders were 0 s or 305.8 s before, depending on a stamp; issue #490 deleted the stamps, so the figure pass is now the manifest's declared sum over the 21 cited entries and does not vary with the diff. The job runs on the push to `main`, not on the pull request (issue #488).

    | build | what it does | wall |
    | --- | --- | --- |
    | every cited figure rendered | 21 renders, then the applicability tables and `latexmk` over both documents | **305.3 s + 15.8 s** |
    ...
    The LaTeX readings were measured 2026-09-09 on the 4-core reference host under the `measure` lock, by hashing the tree before and after each run (issue #429). The figure pass is arithmetic over `snakes_and_ladders.qa.manifest`'s declared `seconds`, not a timed pass; `tests/regression/test_release_gate.py` fails if this file and the manifest disagree. The stamped build's two clean readings measured the same tree three commits apart: at `1da89a3` nine stamps were stale and cost 216 s of a 230.7 s build, and every one of those nine renders produced a **byte-identical** figure and caption.
...
* **`critical` is the early gate, and it is a second axis rather than a kind.** It marks the tests whose failure invalidates everything after them — the import graph, the documentation index, the `CLAUDE.md` pointers, `select_tests` itself, the compiled extension, and the categorical sampler. **177 tests in 16.6 s** (pytest's clock; 19 to 20 s wall), two consecutive runs on 2026-09-09 on the 4-core host at a 1-minute load of 0.86 and 0.98 with `OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1`, against a `-m "not release"` suite of several minutes. The host carried no other job, which is now the ordinary case (issue #521); the load it reports is this run's own. At the 0.5.0 audit it was 149 tests in 13.0 s at a load of 0.45, at the 0.4.0 audit 88 tests in 12.6 s under a load of 2.27, and the earliest uncontended figure 76 tests in 3.0 s. The count rose with #372's stamp and duration guards and #376's problem-statement guard, and the clock did not, which is what a guard tier is for. CI runs `pytest -m critical` unconditionally before selection, so a broken invariant reports in seconds; run it locally the same way. A test is critical *and* a kind, never instead of one.
...
* **One agent at a time, and it has the whole host.** An agent works in its own worktree from `main` (`infra/new_worktree.sh`) with `OMP_NUM_THREADS=1`, and nothing else runs beside it, so the machine is quiet by construction and no job waits on another. The readers-writer lock that arbitrated a shared host is gone (issue #521). It was built to make measurements comparable under parallel agents and, measured, did not exclude: during an exclusive run 2.68 cores were busy and 1.75 of them foreign; `with_lock measure` gave up after its full 1,800 s wait while shared slots ran ahead of it, issue #399's Rust agent measured 40 minutes of starvation inside a 104-minute run at five agents, and three validation loops starved a measurement for an entire 18 s stream where the pre-#427 lock admitted it in 0.06 s ([#431](https://github.com/michaelJwilson/snakes_and_ladders/issues/431), closed as moot); and it taxed jobs that never needed it, `infra/baselines.py --write` costing 30 minutes of queueing for a job that runs in 28 s (#427). With one agent there is nothing left to arbitrate and only those failure modes remained, so a job runs when it is started and a measurement states the load average it was taken at.
  ...
  **A quiet host is not a wide one, and this change does not widen it.** `tests/conftest.py` still pins one BLAS thread per process, and that stays: every baseline in `STATUS.md` and in this file was taken at one thread, so a four-thread number is comparable with none of them. Widening the thread count would stale every recorded number and is a separate decision, not a consequence of removing the queue. A measurement of something that is itself parallel therefore states its thread count wherever it is quoted, and sets the thread variables itself; the figure renderers are exempt by a different route, `qa.build` stripping the variables so a render matches its manifest.
...
1. **Run the gate.** `infra/release.sh` runs every per-PR CI check
   ...
   per PR: `cargo clippy --all-targets --features sandbox` and `cargo test
   --locked --features sandbox` (see "Build System" above — together the only
   thing that compiles the `sandbox` feature), the full `pytest` suite
   including `@pytest.mark.release` tests (see "Release-Gated" above), the
   full `sphinx-build -W`, every figure, and `infra/build_documents.sh`. It
   runs every check regardless of earlier
   ...
   | --- | --- | --- |
   | `QA figures (every figure)` | every manifest figure, cited and uncited, compared against the committed bytes | **431.8 s**, the declared `seconds` in `snakes_and_ladders.qa.manifest` summed over its 23 entries; **not yet measured as a pass on this host** |
   ...
   **Neither step predicts.** The figure step passes `--all`, which selects
   the whole manifest rather than what a document cites; no digest narrows it
   further, the stamps that once did having been deleted on a 100%
   false-positive rate over 476 decisions (issues #476, #490). The guarantee
   that a figure whose rendered bytes would change cannot reach a release
   claiming to be current is enforced by rendering every figure, not by a
   digest. `--check` compares a rebuild
