...
* **Agents run in parallel; the host runs one heavy step at a time.** Up to five implementation agents work at once, each in its own worktree from `main` with `OMP_NUM_THREADS=1`. The cap is set by review bandwidth and by contention on the host lock, not by the reference host's 4 cores: an agent's wall clock is mostly model latency, so the host is idle while it thinks, and every CPU-bound step is serialized on the lock however many agents exist — a sixth agent lengthens the review queue and the lock queue rather than the wall clock of any one ticket. What is CPU-bound — a test tier, a document build, a measurement, a full `infra/validate.sh` — takes the shared lock (`flock` on `build.lock` under the scratch directory) and waits for it in the foreground, so at most one such step runs and the timings stay comparable to the budgets above. Measured queue on 2026-09-08, before the cap rose: eleven tickets estimated at 19 hours serial and about 7 at three agents, with the lock the only contention (CLAUDE.md, Throughput).
