...
* **The thread rule.** `torch` and BLAS multithread inside a kernel, so a pool
  ...
  kernel with the GIL released, is the case for `backend="threads"`; every
  kernel of `oxi_snakes_and_ladders` releases it (`Python::detach`, PyO3
  0.29's name for `allow_threads`) since issue #604, so that case has
