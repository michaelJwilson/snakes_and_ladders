...
| `infra/build_documents.sh` | Regenerates QA figures, checks citation integrity (`infra/check_citations.py`), then builds `docs/paper.pdf`, `docs/textbook.pdf` and `docs/api_map.pdf` (all committed, rebuilt only by a "Rebuild the documents" pull request; the `.aux`, `.bbl`, `.log` and other files `latexmk` leaves beside them are ignored). `--no-figures` drops the regeneration, for the one caller that has already rendered and compared every figure (`infra/release.sh`, issue #530). |
...
| `docs/paper.pdf`, `docs/textbook.pdf`, `docs/api_map.pdf` | rewrites all three on every run, whether or not a figure changed | yes, and never by an ordinary pull request |
...
One row of that table needs a rule, the row both rewritten every run and tracked: **revert `docs/paper.pdf`, `docs/textbook.pdf` and `docs/api_map.pdf` after any local build**. A byproduct cannot be committed by accident and a figure should be; a PDF is the only output where the right action is neither. `infra/review_gates.sh` and the `lint` job both refuse the change, so the cost of forgetting is a failed gate rather than a wrong `main`. `tests/regression/docs/test_document_build_outputs.py` holds the table to the manifest, the script and `git`.
