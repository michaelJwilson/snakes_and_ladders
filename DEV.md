...
**Moving the job is two changes, as a rename is.** `documents` no longer reports on a pull request, so **Documents (paper and textbook)** must leave the required checks in branch protection; until it does, every pull request has one required check that no job will report and no merge is possible. This is the trap issue #377's rename left, in the other direction.
