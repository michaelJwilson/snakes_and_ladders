...
New issues are filed through `.github/ISSUE_TEMPLATE/task.yml`, or through the template for a shape that recurs — `mis_merge.yml`, `optimization.yml`, `seam.yml`, `documents.yml`, `coverage.yml`, `markers.yml`, `release.yml` — each of which asks for what its tickets had been restating (issue #381); blank issues are disabled via `.github/ISSUE_TEMPLATE/config.yml`.
