...
* **Review starts from `infra/review_gates.sh`.** It runs the Definition-of-Done gates CI does not assert or asserts late — the head carries the base, the two PDFs are the base's unless the pull request is a rebuild, a changelog fragment exists, every cited figure's stamp matches the tree, the critical tier passes — and prints a pass/fail table (about 20 s, dominated by the critical tier). Reading then covers what a script cannot: the diff against the plan on the ticket, and whether each test pins what it claims to.
