...
* **Four tiers, three budgets.** A test's tier is decided by *what its size is for*, never by how slow it happens to be: a size chosen so an exact oracle stays available is a CI size even when it is slow, and a size chosen to show behaviour at scale is a stress size even when it is fast.
  ...
  * **The document build: every cited figure rendered, 305.3 s of renders plus 15.8 s of LaTeX.** The renders were 0 s or 305.8 s before, depending on a stamp; issue #490 deleted the stamps, so the figure pass is now the manifest's declared sum over the 21 cited entries and does not vary with the diff. The job runs on the push to `main`, not on the pull request (issue #488).
