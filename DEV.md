...
| `python/snakes_and_ladders/sandbox/` | The home of the second implementation, whichever side won: the hand-rolled code a framework replaced on a hot path, or the framework front that lost, kept either way to referee the other. Imported by `tests/` and `qa/` only, asserted by test. Carries the three fronts declined by issues #391 and #392 (issue #322). |
