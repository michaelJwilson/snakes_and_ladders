...
* **Agents run in parallel; the host runs one heavy step at a time.** On the 4-core reference host up to three implementation agents work at once, each in its own worktree from `main` with `OMP_NUM_THREADS=1`, because an agent's wall clock is mostly model latency and the host is idle while it thinks. What is CPU-bound — a test tier, a document build, a measurement, a full `infra/validate.sh` — takes the shared lock (`flock` on `build.lock` under the scratch directory) and waits for it in the foreground, so at most one such step runs and the timings stay comparable to the budgets above. Measured queue on 2026-09-08: eleven tickets estimated at 19 hours serial and about 7 at three agents, with the lock the only contention (CLAUDE.md, Throughput).
