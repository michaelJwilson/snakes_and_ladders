...
* **The run store is optional, and `aim` is declared nowhere.** `snakes_and_ladders.track` is the seam (issue #778): a sampler, annealer or optimizer calls `current()` once at the top of its loop and records, per sweep, numbers its result dataclass already returns. Outside a `track(...)` block the tracker is `NullTracker` and every method is a no-op, so an untracked run is the run before the seam existed, bitwise --- which `tests/regression/test_track.py` pins on `hmc.sample`, `anneal_potts` and `potts_mcmc.parallel_tempering`, and which the walls in `STATUS.md` price. `AimTracker` imports `aim` lazily, inside its own `__init__`, and **the dependency is in no extra**: `pip-audit` reports PYSEC-2026-1087 (XSS in the report endpoint) and PYSEC-2026-1088 (a sandbox escape in the query handler) against 3.29.1, the current release, with no fixed version, so declaring it would make the local audit red for every worktree sharing this host's environment. Install it yourself to use it --- `uv pip install aim`, then `aim up --repo <path>` serves that repository at `http://127.0.0.1:43800`, on a loopback address and not beyond it while those two stand. A run written by a script is indexed by `aim storage --repo <path> reindex` and is not readable back until it is. The test that writes a real Aim run carries `release` and `importorskip`s the package, so the suite needs none of this.
