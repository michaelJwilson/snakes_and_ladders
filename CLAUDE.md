...
*   **The FFI boundary.** Cross it once per call with contiguous arrays; time a kernel alone *and* through its binding. The marshalling was assumed to be the dominant term here and measured not to be, on every path measured so far, so the boundary is timed rather than presumed; `DEV.md` carries the mitigations and the profile each one needs before it lands (Gorelick & Ozsvald ch. 7; Antão).
