...
while read -r stem; do
  time_it "$stem" uv run python -m sal.qa.build \
    --output-dir "$scratch" --only "$stem"
done < <(uv run python -m sal.qa.build --all --list)
...
uv run python -m sal.qa.build --list | tr '\n' ' '
echo
time_it "cited figures" uv run python -m sal.qa.build \
  --output-dir "$scratch"
