...
ticket that drives it. `CLAUDE.md` remains authoritative: where it and this file
disagree, it wins.

**Every number here is a rough upper bound, rounded up.** They are here so a
reader can judge the work and the hours a release commits them to before
...
and its notes then describe a tree nobody has (issue #468).
...
conflict on generated files, and the resolution was to regenerate rather than
re-render — at the 0.5.0 cut, 30 figure-stamp and baseline conflicts across four
merges resolved with no figure's bytes moving and no baseline's value changing.
Both of those file classes have since gone: issue #490 deleted every figure
stamp and issue #460 the baseline digest. What is left is text. Where a file is
a list rather than a generated artefact — a package `__init__`, a module
`CLAUDE.md`, `STATUS.md` — both sides are kept in the file's existing order, and
where both sides *deleted* a line, as `TICKETS.md`'s branches do, the union is
of the deletions. Two hazards a merge carries that the tree does not show: a
textual merge of two branches' bibliography entries can leave an entry's closing
brace missing, which `infra/check_citations.py` walks for (issue #503) and a
reader caught by hand before it did; and concurrent branches write colliding
experiment numbers that `git` cannot see, three branches having written
`docs/experiments/007` (issue #491).
...
answers (issue #468), not a blank template.
...
template's boxes, since the checklist has been the shorter of the two; and the
...
rule. Ten frameworks were measured between 0.3.0 and the 0.5.0 audit and ten
...
to be reserved before starting it — the host lock is gone (issue #521) — but the
gate holds the machine for as long as it runs.

**Budget hours, not minutes, and expect two sinks to be most of it.**
...
attempt on 2026-09-09 ran for over an hour, reached 46% with nothing failing,
and was stopped when `main` moved beneath it. Report that run as pass/fail: on a
host this size a timing states the host rather than the tree.
...
decisions (issues #476, #490). It also passes `--check`, which makes a mismatch
a failure naming the figure rather than a silent refresh. It runs **before**
`infra/build_documents.sh`, because that script renders a stale cited figure
into `docs/tex/figures/` and would otherwise supply the very bytes the
comparison is against. The Sphinx step is `-E -a` for the same reason in
miniature: an incremental build's verdict depends on what `docs/_build/` holds
from an earlier branch, while a release claims all 134 modules are clean. That
autodoc records each module as a dependency, so an incremental build does
...
multi-threaded reduction is the one that degrades superlinearly when the host is
oversubscribed.
...
built. A reader who finds `CHANGELOG.md` stepping from `[0.3.0]` to a later
version is looking at that held cut and not at a missing release. Which version
the next cut carries is issue #522's.
...
`## [<version>]` section into `CHANGELOG.md`; see `changelog.d/README.md` for a
fragment's form, and note that CI's `towncrier check` is what makes one exist
per user-visible change. Commit the result. The consumed set belongs to the tree
the cut is taken from: a section built on one tree and landed on another
describes neither.
...
documentation corrections and the template fixes — as its own small pull
request, and re-cut later. Each correction is re-checked against current `main`
rather than carried over: of seven corrections salvaged from the 0.5.0 cut, one
had already landed elsewhere and re-applying it would have fought a restructure,
and one was damage the union merge itself had created and no defect on `main` at
all (issue #468).
...
* **It does not rebuild the committed PDFs.** A pull request changes `docs/tex/`
  ...
  its base carried, and they lag the sources by design.
...
* **"The review-gate table is measured at 30 s."** 30 s is the *budget*. The
  ...
  `DEV.md` carries that figure; a measurement that coincides with a budget is
  still not that budget.
