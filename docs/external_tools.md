...
with known truth, and adopts no external solver today. Nothing here is
installed or declared in `pyproject.toml`, `uv.lock` or `Cargo.toml`. Adopting
one is a separate decision, taken on its own evidence, under `DEV.md`'s
...
Every licence and star count below was read on 2026-09-08 from the project's
own repository. `CLAUDE.md` flags any candidate under 1,000 stars for explicit
review.
...
the [OSI list](https://opensource.org/licenses). What matters on adoption is
reach rather than approval: a copyleft library imported into this package
carries its terms into this package, while a copyleft binary invoked as a
subprocess does not. The two inference tools are binaries; the five Python
...
derivatives. Benchmarking is the role our own referees cannot fill —
enumeration stops at eight taxa, and simulated truth gives a Robinson–Foulds
distance but no competitive likelihood to beat at 50 to 200 taxa, which
`ROADMAP.md` §2.3 and `docs/blue_sky.md`'s external-baseline candidate ask for.

The first adoption, if there is one, is **IQ-TREE 2, for benchmarking** and for
the fixed-tree likelihood that would cross-check the NumPy oracle from outside.
It is GPL-2.0 and invoked as a subprocess, so its terms do not reach this
package; RAxML-NG is the same shape under AGPL-3.0 and is the second opinion.
Its 336 stars are below the 1,000 the rule sets and would need explicit review:
for a compiled research tool the star count understates use, and the metric to
argue from is the citation record of Minh et al. 2020.
...
comparison at 50 to 200 taxa is the standing example.
