...
| [008](008-ffi-profile-and-alpha-expansion.md) | potts-lattice | ci | python, rust | 0 | confirmed | #447 held: the ranking puts the Python Dinic solver at 49.0% of `alpha_expansion`, and the reason was never the boundary --- `oxisal.max_flow` computed the s... |
...
| [020](020-the-stress-tier-ranking.md) | tree | stress | profile, hoisted-post-order | 20 | confirmed | No family's top term is Rust's yet: two are Torch autograd, one is a default already measured, and the one loop at 95% of its run is the BCJR trellis. `pytho... |
...
| [024](024-the-bcjr-trellis-in-rust.md) | turbo | stress | numpy, rust | 0 | confirmed | Yes, by 13x the bar: 26.2x on the enclosing eight-iteration decode at the declared `K = 256`, saving **35.6 ms of 37.0**, so the default flips and the releas... |
| [025](025-the-swendsen-wang-pass-in-rust.md) | potts-lattice | stress | numpy, rust, hoisted-multiply, sorted-grouping | 0 | confirmed | Yes, by 32x the bar: **63.4x** on the enclosing `propose`, saving **19.9 ms of 20.2**, and the ranking's 42.2 ms row ends at 0.32 ms --- of which **2.04x is... |
| [026](026-what-a-draw-costs-in-gradients.md) | mixture | stress | hmc, mala, slice | 0 | confirmed | **No: the trajectory is not paid for at these dimensions, and neither unit decides it alone.** MALA takes 1.7x to 2.6x HMC's effective samples per gradient o... |
| [027](027-the-tree-schedule-in-rust.md) | hmm-path | stress | numpy, stored-plan, rust | 0 | confirmed | A pass to port, by 4x the bar: **9.05x** and **21.6 ms of 24.3** at the chain of 200, where the stored plan alone is 1.23x and is subsumed --- the kernel wal... |
| [028](028-the-chain-transfer-product-by-squaring.md) | potts-lattice | stress | recursion, squaring | 0 | confirmed | Yes, **3.57x**, saving **17.0 s of 23.5** over the draws and 17.0 ms of 23.6 per draw, against the ticket's predicted 1.5-2x --- and the backward row was **o... |
