...
| [7](007-per-fit-cost-of-the-tree-likelihood.md) | tree | release | eager, torch-compile, rust-binding | 0 | confirmed | Only level-synchronous batching clears the bar --- **#443 PR 2**, 10--18% of a search's self time as a floor and 30--53% as an upper bound; **#443 PR 3** re-... |
...
| [009](009-batched-rollout-and-torchrl-ppo.md) | potts-lattice | ci | sequential, gymnasium-vector, torchrl | 1920 | confirmed | Both declined: `SyncVectorEnv` is a serial loop with no parallelism to amortize its bookkeeping against and costs 1.18x-1.75x at every batch size, `GAE` is 2... |
| [010](010-pruning-gradient-routes.md) | tree | ci | taped, burn, analytic | 60 | confirmed | A is declined at 1.42x per gradient and 1.70x per fit, and the FFI boundary is not the reason --- its kernel alone is already 1.45x PyTorch's whole evaluatio... |
