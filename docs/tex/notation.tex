...
\newcommand{\transfer}{\mathsf{T}}       % a transfer matrix over columns
...
\newcommand{\energy}{\mathcal{H}}        % a Hamiltonian, script H: the
                                         % parity-check matrix is bold H
                                         % and the Hadamard matrix italic
\newcommand{\hidden}{\mathbf{Z}}         % a hidden state path
\newcommand{\Prob}{\mathbb{P}}           % a probability
\newcommand{\indicator}{\mathbf{1}}      % one where its condition holds
\newcommand{\emission}{\mathcal{E}}      % an emission family
\newcommand{\parity}{\mathbf{H}}        % a parity-check matrix, bold
\newcommand{\hadamard}{H}               % the Sylvester-Hadamard matrix, italic
