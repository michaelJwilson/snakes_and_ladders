...
output: empirical substitution
frequencies along a branch are compared against the closed form, within a
Monte Carlo tolerance set by the site count. Table~\ref{tab:sim-problem-sizes}
is the three instances that comparison runs on, each with the seed and the
tolerance it is validated within; the site count is what buys the tolerance,
and the eight-taxon instance is the one that exercises a tree deep enough for
the frequencies along an interior branch to be worth measuring.
...
different question, and the referee for it is simulated truth rather than
enumeration: it is decided by how much signal the alignment carries, so it is
asked against the site count. Figure~\ref{fig:topology-accuracy} sweeps that,
at a low end deliberately below what the fixtures use, since a sweep confined
to sizes already known to work locates no margin. The requirement it is read
against is the roadmap's, a normalized Robinson--Foulds distance at or below
$0.05$ from the generating topology.
