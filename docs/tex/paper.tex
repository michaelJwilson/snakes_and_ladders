...
class is under review and its results are not in this version. No comparison
against IQ-TREE~2 or RAxML-NG has been run: the tools are not in the
environment, and installing them is a ticket of its own (\#126). And no
accelerator exists to measure on, so hardware scaling is reported for the CPU
alone.
...
separates anything from restarts is what it needs. A richer feature set is
under review (\#328).
...
the global minimum from any of 10 starts. Restarts are therefore not beaten
...
hidden Markov models, the coupled model and the mixture without encoding any
of them, with the parity-check class following. Every evaluator is pinned to
...
result, the parity-check class, the external tools the competitiveness claim
needs, and the accelerator the scaling claim needs.
