...
20{,}000-site one is---and the set is tabulated rather than described because
a seed and a tolerance are what make the comparison repeatable at all.
...
data}. Whether the best tree for the data is the tree that generated it is a
different question, refereed by simulated truth rather than by enumeration and
decided by how much signal the alignment carries---so it is asked against the
site count. Figure~\ref{fig:topology-accuracy} sweeps it, at a low end
deliberately below what the fixtures use, because a sweep confined to sizes
already known to work locates no margin, and reads the result against the
roadmap's accuracy requirement. Where the sweep first clears that requirement
is not quoted: it is a threshold crossing over an optimizer's output, and one
that moves in its last digits on another machine.
