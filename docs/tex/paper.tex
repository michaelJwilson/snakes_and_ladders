...
draw that reads none of the data reaches the best projected value on every one,
while the seedings that read it, or that sample a Gaussian surrogate for
twenty-four times the fit's own budget, recover more of the generating
component on every one---both directions at $p = 0.031$ under McNemar. At forty
observations a component the projected likelihood is not a proxy for recovery,
so the referee and not the candidate is what the comparison exposed, and no
default moved.
