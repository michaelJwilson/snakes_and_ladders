...
Figure \ref{fig:rl-convergence} poses Milestone 2.1's phylogenetic comparison on a topology landscape where hill climbing demonstrably fails, which is the condition that makes the comparison askable at all: on a landscape the baseline already solves from every start, a policy can be neither better nor worse than it. Its outcome bears on two items the roadmap schedules --- the richer feature set of Milestone 2.1, and the accepted-worsening steps of Stage 3's stochastic escape mechanisms --- because the limits it runs into are the ones those items remove. The remaining metrics below will be populated upon completion of Milestone 2.3.
