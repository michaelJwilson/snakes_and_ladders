...
\begin{tikzpicture}[
  ...
  \foreach \i in {1,...,4} {
    \node[label] (z\i) at (2.1*\i, 2.2) {$z_{\i}$};
    \node[factor] (e\i) at (2.1*\i, 1.4) {};
    \node[observed] (n\i) at (2.1*\i - 0.45, 0.6) {$n_{\i}$};
    \node[observed] (y\i) at (2.1*\i + 0.45, 0.6) {$y_{\i}$};
    \draw[link] (z\i) -- (e\i);
    \draw[link] (e\i) -- (n\i);
    \draw[link] (e\i) -- (y\i);
    \draw[within] (n\i) -- (y\i);
  }
  \node[anchor=west, font=\footnotesize] at (9.4, 2.2)
    {no pairwise factor};
  \node[anchor=west, font=\footnotesize] at (9.4, 1.4)
    {$\psi_i = w_{z_i}\, p(n_i, y_i \mid z_i)$};
  \node[anchor=west, font=\footnotesize] at (9.4, 0.6)
