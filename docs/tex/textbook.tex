...
under the Sylvester--Hadamard matrix $H$ of that order,
\begin{equation}
  s = H^{-1} \exp(H q), \qquad q = H^{-1} \log(H s),
  \label{eq:hadamard}
\end{equation}
so the exact spectrum returns the tree's own lengths on its splits and zero
elsewhere. The \emph{closest tree} takes the $n-3$ largest mutually
compatible weights of an estimated $q$. A four-state Jukes--Cantor alignment
...
matrix $\mathbf{H} \in \{0,1\}^{m \times n}$ over $\mathrm{GF}(2)$: the
codewords are the $c$ with $\mathbf{H} c = 0$, there are $2^k$ of them with
$k = n - \operatorname{rank} \mathbf{H}$, and the rate is $k/n$. A
low-density code makes $\mathbf{H}$ sparse. Gallager's regular
$(d_v, d_c)$ ensemble \citep{gallager1962} puts $d_v$ ones in every column
and $d_c$ in every row: $\mathbf{H}$ is $d_v$ bands of $n / d_c$ rows, the
...
is then
\begin{equation}
  p(c \mid y) \;\propto\; \mathbb{1}[\mathbf{H} c = 0]\, \exp(-c \cdot L),
  \label{eq:ldpc-code}
\end{equation}
...
per row of $\mathbf{H}$, a hard constraint of degree $d_c$ whose log table
...
$d_v + 1$. The graph has cycles---a random draw of the ensemble does not
...
\citep{gallager1962, mackay2003, richardson2008}. From bit $i$ to check $a$,
...
$\mathbf{H} \hat c = 0$, the syndrome check, and otherwise at an iteration
...
and not truth. The encoder is pinned by $\mathbf{H} c = 0$ and the offsets
