...
parameterizations serve. A \emph{linear} scorer, $f = w^{\top} x(s, a)$, is
the reference, because its weights are what a recovery test can state a truth
...
and sits inside the policy class rather than beside it, and ``did the agent
learn the landscape'' becomes a statement about $w$ rather than about a loss
curve. A \emph{multilayer-perceptron} scorer --- two hidden layers over the
same features, one output per action --- is the parameterization for a reward
that is not linear in them. It is measured against the linear one at a matched
budget and never assumed better: a wider class that is not needed costs sample
efficiency and gives up the recovery statement, so the burden is on it.
Both share the softmax's gauge. Adding a constant to every score in a state
changes nothing, so a feature constant over the neighbourhood carries no
identifiable weight and is excluded rather than fitted, and neither scorer has
a bias term. The critic of Section~\ref{sec:actor-critic} is the case that
shows the rule is about the softmax and not about features: a value is not a
softmax, a constant there \emph{is} identifiable, and the critic carries one.
...
$\epsilon$, where it draws uniformly from the neighbourhood --- which is the
only way a \emph{worsening} move is ever taken.
It is deliberately not trainable and exposes no weights, so a measured escape
rate is attributable to $\epsilon$ and not to a training run that happened
alongside it; $\epsilon$ follows a declared schedule over the iterations,
exactly as a temperature does in Section~\ref{sec:tempering}. At $\epsilon =
0$ the agent still walks, and deterministically: past a local optimum the best
available action is the least-bad move, which is not the same object as a
climb that would have stopped.

Two consequences for the accounting, and both are easy to get wrong in a
direction that flatters the agent. An episode allowed to continue past a local
optimum is scored on the \emph{best} state it saw and not on its last, or a
better searcher looks worse the longer it runs. And the matched baseline is
then greedy \emph{with restarts}: a single greedy run stops after a few
decisions and leaves the rest of the budget unspent, so a result stated
against it overstates itself. One failure is worth naming because it produces
plausible numbers rather than an error --- wrapping an \emph{untrained} policy
is not hill climbing, since with equal scores the best action is whichever the
neighbourhood happens to list first, and what is then measured is exploration
around a fixed arbitrary move.
...
logarithm of the neighbourhood's size --- which covers every instance without
widening the interface, a terminal state having no actions and so the zero
vector. The rule that makes the baseline legitimate is the one in
Appendix~\ref{app:rl}: the critic may read the state and never the action
sampled at that step. Where the return is exact by enumeration $V^\pi$ is
exact too, and the critic is scored against \emph{that}; a critic scored
against its own training loss can fit its targets, miss $V^\pi$, and leave an
estimator that is unbiased and useless.
...
s_t)$ makes the estimator of Equation~\ref{eq:reinforce} unbiased for $\pi$
on episodes that $\beta$ collected \citep{suttonbarto2018}. The support condition
is not a technicality: an action $\beta$ never samples contributes nothing and
its weight under $\pi$ is never corrected, so a \emph{deterministic} behaviour
policy corrects nothing at all. That is the second reason the exploring
wrapper's uniform draw is there, and the reason its mixture must be available
as a distribution and not only as a sampler --- $(1 - \epsilon)$ on the best
action and $\epsilon / |\neighbourhood(s)|$ everywhere is what the ratio
divides by. That $\epsilon$ is the exploration rate; the one in
Equation~\ref{eq:ppo-clip} is a clip width, and the two are unrelated.

What the correction costs is variance, and the cost has a shape. A ratio is
unbounded above, so one decision the behaviour policy made rarely can dominate
a batch; and a weight formed as the \emph{product} of ratios along a
trajectory is worse, its variance growing with the episode length. Two answers
are carried, and they are not the same answer. The per-decision form of
Equation~\ref{eq:ppo-clip} weights each step by its own ratio rather than by
the trajectory's product, which removes the growth with episode length. The clip
bounds the ratio's effect on the objective, and therefore the size of an
update, while leaving the estimator's variance exactly where it was: it is a
trust region and not a variance reduction, and reading it as the second is the
...
neighbourhood holds one interchange must find a sacrifice and its payoff as
two separate decisions --- which is exactly the credit assignment a truncated
episode destroys. An \emph{option} is a policy over primitive moves with an
initiation set and a termination rule, so a sequence that pays off only as a
unit becomes one decision and is credited as one \citep{sutton1999}; the
termination rule can be learned beside the policy that calls it
\citep{bacon2017}. Nothing here builds one, and the budget rule says what it
would have to survive: a compound move evaluates every primitive it composes,
so an option reaching a better optimum by evaluating more candidates than
hill climbing has not won.

\paragraph{Planning with a learned model.}
The planner above expands the \emph{true} successor and reads the true reward,
which is affordable here only because a move's reward is a closed form at
known parameters. It stops being affordable the moment the reward is a fit per
candidate, which is the regime every problem in this document is actually in.
The answer in that regime is to plan on a model that is learned rather than
queried: a transition and a reward in a latent space, trained so that the
values, policies and rewards the plan reads agree with the observed ones, and
not so that the latent state reconstructs anything about the real one
\citep{schrittwieser2020}. Requiring only the quantities the search reads to
be right is what makes it a method rather than a reconstruction, and it is
also what makes it checkable here: on an instance small enough to enumerate, a
planner on a learned model must still return an optimal action wherever the
planner on the true one does. It is not built.
...
merely close at the corners is optimizing a different problem and nothing
measured on it transfers, which is the same demand Section~\ref{sec:tropical}
makes of the tropical relaxation and the reason
Equation~\ref{eq:tropical-corner} is a derived tolerance rather than a
constant.
...
\end{equation}
exactly, so the form evaluated at the marginals \emph{is} the expected
discrete score and not an approximation of it. The condition is easy to state
wrongly. It is not that the model must be a chain --- a lattice is as
multilinear --- and it is not that the terms must be pairwise, a term coupling
three \emph{distinct} sites being still one factor per site. What breaks it is
a term reading one site twice, since for an indicator $\mathbb{E}[X^2] =
\mathbb{E}[X] \neq \mathbb{E}[X]^2$. Two things follow. A multilinear function
on a product of simplices attains its maximum at a vertex, so the relaxation
introduces no optimum the discrete problem lacks and everything a relaxed
search loses is lost to the ascent rather than to the relaxation. And the
deterministic relaxation needs no sampling at all, which makes the question
about the stochastic one sharp: since Equation~\ref{eq:multilinear} is exact,
whatever bias an estimator shows belongs to the estimator.
...
reparameterized --- differentiable in the probabilities --- and biased at
every $\tau > 0$, exact only in a limit where the gradient vanishes. The
bias is measured against the exact gradient enumeration supplies and never
assumed small, and the score-function estimator of
Equation~\ref{eq:reinforce} is the unbiased alternative at higher variance,
which is Section~\ref{sec:actor-critic}'s trade in a second place. Carrying
the construction from one categorical variable to a \emph{structured} object,
where the $\arg\max$ is a combinatorial solver rather than a coordinate, is
the stochastic-softmax-trick family \citep{paulus2020} --- what a
differentiable topology would need, and the direction
...
A third trades exactness for cost by the site count rather than by the tree.
Site-pattern compression, Equation~\ref{eq:site-patterns}, is exact and
saturates; blocks of $N > 1$ consecutive sites do not repeat often enough for
the same identity to save anything, and what a block partition buys instead is
a bracket. Partition the $L$ columns into $\lceil L/N \rceil$ blocks of $N$
...
(Appendix~\ref{app:bounds}); relaxing them to those one-pass values is what
makes the bracket cheap and what makes it loose. The width is exactly
$B\,(\overline\lambda - \underline\lambda)$, so the cutoff $m$ sets the trade
directly: $m = 1$ leaves $B = 0$ and returns the exact value, and raising $m$
moves blocks into the tail and never out of it. Only the lower end bounds
$\ell^\ast(\tree)$, by the same argument that
Equation~\ref{eq:plugin-bound} rests on. Measured at five taxa and 2,000
...
Every problem statement below ends with the same question: given more than
one method that applies, which one is worth a budget at the sizes that
section supports. Three axes answer it, and each is asked in the same order.

\emph{The precondition} is a structural property of the instance that decides
...
than run badly, which is what separates the comparisons here from a ranking
--- a cell of Section~\ref{sec:applicability} is empty where a precondition
failed as often as where nobody wrote the code.
...
counted in seconds. A cost in operations and in the live set a recursion
holds is a property of the algorithm and transfers between machines; a
wall clock is a property of neither, and the language a kernel is expressed
in --- and the boundary between it and the language that calls it --- has
been measured a small fraction of a call rather than the term that decides a
method. No choice in this document turns on it, and the measurements
themselves belong to the companion paper.

\emph{The reach of the referee} is the size past which nothing exact scores
the answer. It is the axis that changes what may be \emph{claimed} rather
than what may be run: inside it a method is scored against a truth, and
outside it a method is scored against a recovered structure, a bound, or
another method --- which is agreement and not truth
(Section~\ref{sec:properties}). Two methods that agree inside the reach and
diverge outside it have not been separated, and a comparison that crosses
the reach without saying so has changed its subject mid-sentence.

Where the three leave a genuine choice, the answer is a measurement at an
equal budget in the problem's own unit over shared seeds, and it is a
statement about that instance. Where they do not, the choice is already
made and the section says which of the three made it.

\paragraph{What a referee assumes about the application.}
A referee is only as good as the property it rests on, and those properties
are not shared: they are facts about the model each section states, and each
would be false of a neighbouring one.
Table~\ref{tab:referee-assumptions} collects them, one per problem, so that a
result read out of one section carries the assumption it was refereed under.
Each problem's own discussion says where the assumption enters.
...
summation order distinguishes the two sides. It is the standard first step of
a maximum-likelihood implementation \citep{felsenstein2004}, and it saturates,
since $P \le k^{\,n}$ however long the alignment---at $k = 4$ and four taxa
$P \le 256$ whatever $L$ is, so the recursion's cost stops growing once every
column has appeared. Compression is a property of the alignment rather than of
an evaluator, so every backend consumes patterns and weights.
...
each; a single root-to-tip pass returns all of them together, and it is the
transpose of the post-order pass rather than a second algorithm
\citep{felsenstein2004}. Write $A^s_u(i) = \partial \ell / \partial L^s_u(i)$
...
Two properties of that pass are worth stating, because both are places an
implementation goes wrong silently. First, the rescaling contributes nothing:
$\ell$ is algebraically independent of the per-node scales, the divided
partial and the accumulated logarithm moving against each other, so the
derivative may be taken with the scales frozen at the values the forward pass
produced, and each adjoint is simply divided by its own node's scale. Second,
the sibling product $\prod_{w \neq v} m^s_w$ must be formed by a scan over the
siblings and not as $L^s_u / m^s_v$: a message vanishes at any site whose
observation forbids that state, where the quotient is $0/0$ and the derivative
is finite.
...
against central differences relative to its own norm---which is what refereed
Algorithm~\ref{alg:pruning-backward} against the same likelihood
differentiated by a tape, the two agreeing to a tighter tolerance than either
agrees with the difference quotient. And the recovery test of
...
Three further economies apply to the same climb, and each is separable from
the others. The climb may begin at a most-parsimonious topology rather than an
arbitrary one, since a parsimony pass costs no fit. The regraft may be bounded
...
That last is an optimization over a subspace, so its value is a lower bound on
the candidate's maximum rather than equal to it: it decides which candidate to
accept and the accepted one is refitted over every branch, exactly as a ranked
neighbourhood is.
...
above are asserted exactly over every topology. Each economy above is held to
that same enumeration alone and in combination, or the rate at which it loses
the optimum is measured and stated. Two of them also have an exact reduction
to something already refereed, which a count alone would not catch: the
bounded regraft at $r=1$ must equal the NNI neighbourhood, and at $r$ past the
diameter must equal the unbounded one candidate for candidate. A warm start is
pinned by reaching the cold fit's optimum; a cached partial likelihood by
equalling the recomputed one bitwise, since it is the same arithmetic in the
same order. Large parsimony is held to the same enumeration from every
...
tropicalization of the Pl\"ucker relations for two-planes in $n$-space, is
exactly the space of trees on $n$ leaves --- its points the vectors
$d \in \mathbb{R}^{\binom{n}{2}}$ for which every quartet's largest pairing
sum is attained twice. Two consequences make it a relaxation of topology
search. The coordinates are continuous, so a tree is a point of a Euclidean
space rather than a combinatorial object; and a quartet's resolution is the
pairing attaining the \emph{smallest} of its three sums, so the discrete
structure is an $\arg\min$ of a linear function of those coordinates.
...
$D(\tree) = \sum_Q \ell_Q[r_\tree(Q)]$ exactly. The relaxation is an
extension of what it relaxes rather than a second model, which is the same
demand Section~\ref{sec:properties} makes of the Gumbel-softmax relaxation of
...
fixture's shortest internal branch. \emph{The relaxation is outer}: $F_\tau$
is defined on all of $\mathbb{R}^{\binom{n}{2}}$ and not on $\mathrm{Gr}(2,n)$,
a general dissimilarity resolves every quartet, and the collection of
resolutions need not come from any tree. Ascent may therefore leave the
variety, and the topology is read back off the point it stops at by neighbor
joining, which is exact on the variety.

$D$ is a quartet decomposition of the criterion and not the tree's
log-likelihood: it is a different surface, and substituting one for the other
is licensed by measuring that the two agree at the $\arg\max$, never by a
...
that builds a tree from a distance matrix
(Algorithm~\ref{alg:neighbor-joining}), and the climb over a neighbourhood
(Algorithm~\ref{alg:hill-climbing}). Each is standard and each is stated in
the appendix. What is not settled by citing them is which to spend a budget
on at a given taxon count, and that is what
Table~\ref{tab:phylo:regimes} answers.
...
compression saturates at $P \le k^{\,n}$
(Equation~\ref{eq:site-patterns}), so past a few thousand sites an
evaluation stops growing in cost while the estimate keeps sharpening: length
is the cheapest resolution on offer here, and the reason the declared range
reaches eleven thousand sites at a taxon count whose search is unrefereed.
Below a few hundred sites the same variance decides whether a distance
matrix sits inside the recovery radius at all, so a short alignment is not a
small problem but a different one.

\paragraph{What each method needs of the model.}
The recursion applies to any tree and any rate matrix, reversible or not.
The closed form Equation~\ref{eq:jc} is Jukes--Cantor's alone; the
log-det distance, Equation~\ref{eq:logdet}, is additive under any Markov
process on the branches and is the one distance that survives a model the
fit does not assume. The conjugation of Equation~\ref{eq:hadamard} is the
symmetric two-state model's, reaching four states only through the exact
pairing reduction and not through a general time-reversible one. The
tropical relaxation applies wherever a quartet carries a score, but is
\emph{outer}: ascent may leave the space of trees entirely, so it is a way
of moving and never a way of reporting, and the topology is read back by an
algorithm that is exact on the space.

\paragraph{What the validation assumes.}
Three application-specific properties, and every referee in
Section~\ref{sec:phylo:validation} rests on one of them. Sites are
independent and identically distributed given the tree, which is what makes
direct marginalization a per-column sum and pattern compression an identity
rather than an approximation. The model is time-reversible, which is the
whole content of the pulley principle: the invariant is not a general
property of a likelihood but a statement about $\Q = \SubMat
\operatorname{diag}(\bpi)$, and it is silent about a model that is not
reversible. And recovery assumes the data came from the model, so it
calibrates the pipeline on its own samples and transfers to nothing else ---
which is why it is the referee of last resort and not the first.
...
\citep{papadimitriou1998}. Three further directions each have a stated
prerequisite. A posterior over topologies rather than a flat-prior weight over
maximized likelihoods needs a chain that mixes over the move graph, whose
mixing is the object studied rather than assumed \citep{whidden2015,
altekar2004}; a differentiable topology needs a parameterization of the
space itself, which the subsplit constructions supply and which would give
Section~\ref{sec:tropical} an oracle it does not have \citep{zhang2018,
zhang2019}; and a learned ranking of the regraft neighbourhood is the
surrogate Section~\ref{sec:surrogates} ranks with, learned rather than derived
\citep{azouri2021}. Richer substitution models --- rate heterogeneity across
sites, a general time-reversible matrix fitted rather than fixed --- change
the evaluator's parameter vector and nothing about its referees
\citep{yang2014}. Established implementations are a comparison that could be
made and is not one made here \citep{minh2020, kozlov2019}: they are surveyed,
and admitting one is a decision on its own evidence.

\paragraph{What is deliberately not attempted.}
Empirical alignments, because there is no true tree behind one and a result on
it can be reported but not refereed --- which is the reason every fixture is
simulated. Model misspecification, since every recovery statement here is
calibration on the model's own samples and says nothing about data the model
does not describe. Insertions and deletions, which break the site
...
Hadamard conjugation proper, over the Klein four-group: the two-state
transform with the exact pairing reduction is carried, and the general one is
stated and left unbuilt.
...
semidefinite relaxation with its rounding. They are not alternatives to one
another in the ordinary sense, and the first thing to settle is which
question is being asked: a sampler estimates a distribution, a cut returns
one configuration, and message passing returns marginals and a free energy.
Scoring one against another's referee is the standing error here.
...
twice. Near a critical coupling the correlated regions span the lattice, so
a single-site sweep decorrelates slowly and the cluster move is what makes a
sampled estimate affordable --- but only at non-negative couplings, which is
exactly where the exact cut is also available for the ground state. Away
from criticality the cluster move's advantage narrows and its refusals
remain. That the two closed forms of Section~\ref{sec:potts-sampling} exist
is what lets an instance be placed \emph{at} a critical point rather than
near one, so the comparison is run where the difference is largest and not
where it happens to fall.

\paragraph{What the validation assumes.}
The referees in Section~\ref{sec:potts:validation} rest on three properties
of the model, all easy to lose in a move set. The chain must reach every
...
before it is run. And a temperature must act as a scaling of $J$ and $h$ and
in no other way, since that is what lets the \emph{unscaled} model's
enumeration referee every replica of a tempered ensemble at once.
...
\paragraph{What it would take to go further.}
The missing referee is an oracle for the normalizer of a lattice wider than
the transfer matrix reaches. A boundary contraction supplies one: a strip's
...
ceiling into a stated accuracy \citep{schollwoeck2011}. Two bounds would
extend what can be claimed without an oracle at all --- the tree-reweighted
upper bound of Equation~\ref{eq:spanning-tree-bound} has a maximum-a-posteriori
counterpart that bounds the ground state rather than $\log Z$
\citep{wainwright2005map}, and the characterization of which energies a cut
can minimize says exactly which multi-label instances reduce to the exact
solver rather than to expansion \citep{kolmogorov2004}. For the search itself,
population annealing carries a whole ensemble down a temperature schedule with
resampling and is the natural comparison for the tempering of
Section~\ref{sec:tempering} at equal sweeps \citep{machta2010}. A learned
ground-state proposal is admissible on the same terms as any other: a graph
network trained to output a configuration is a method \citep{schuetz2022}, and
the objection that it must be measured against a competently tuned greedy
baseline is part of the method rather than a criticism of it
\citep{angelini2023}.
...
draw of a whole path from its conditional
(Algorithm~\ref{alg:chain-block}). Unlike every other section here, none of
them has a size at which it stops being exact: the chain is a tree, so the
recursions cost $O(T k^2)$ and are exact at every length. The size question
is therefore not which algorithm evaluates but which referee is available,
and the two ceilings move for different reasons.

Below the path enumeration --- eleven positions at three states, seventeen
at two --- the term-by-term sum scores the evidence, both decodings and
every posterior, and it is the only thing that can: it is what separates a
decoder that computes the most probable path from one that computes the
per-position modes, so no decoder can referee it. That is why the fixture on
which the two decodings differ lives inside this ceiling, and why nothing
else is asked of it. Past the ceiling only recovery remains, and recovery
needs \emph{length} for a reason that has nothing to do with cost: a
transition matrix is identified by the number of transitions observed, so
the fixture's length is set by the variance of the estimate and its width by
the state count, while the recursion's cost stays linear in the length.

\paragraph{Which fit, and why both are carried.}
The alternation costs one pass per iterate and increases the likelihood at
every one, so it is what to run wherever the emission family's re-estimate
is a formula. Where the re-estimate is itself an optimization --- a
dispersion whose score involves a digamma, the case that says whether an
interface built around closed forms was built correctly --- the guarantee is
inherited only if the inner solve actually converged, and an unsettled
inner solve surfaces as a non-monotone outer likelihood several iterates
later. There the gradient fit over the unconstrained vector of
Section~\ref{sec:abstraction} is the direct route. Where both apply they
must reach the same optimum, and it is the agreement of two routes sharing
only the model that is the evidence, never the convergence of either.

\paragraph{Which decoding, and the choice is the caller's.}
The two answers are not an accuracy ordering. The most probable path
minimizes the probability of getting the whole path wrong; the per-position
modes minimize the expected number of wrong positions and may return a path
the model gives probability zero. Neither dominates, and the choice states
what the caller is scored on. Sampling the path posterior rather than
decoding it is the third answer and the one the coupled model's expectation
step needs, since a decoded path is a point estimate and an expectation is
not.
...
\paragraph{What it would take to go further.}
The chain has no missing oracle inside its ceiling and no oracle at all past
it, so the extensions are about the \emph{start} and the \emph{uncertainty}
rather than about the recursion. A method-of-moments start would give the fit
what the distance start gives the tree: the observable moments of short
windows invert to the parameters with a consistency guarantee and no surface
to climb \citep{hsu2012}, and the argument that carries it to a tree
\citep{mossel2006} is already cited in Section~\ref{sec:spectral-start},
which makes the chain the case where it is easiest to referee. A posterior
over parameters rather than a point estimate --- variational, or sampled by
Section~\ref{sec:hmc}'s chain --- would replace the delta-method interval
whose asymptotics Section~\ref{sec:abstraction} is explicit about
\citep{bishop2006, murphy2023}. And choosing the state count rather than
declaring it is a model-selection problem this document does not pose.
...
Section~\ref{sec:potts} at the sizes stated there. Under a categorical
emission the instance stays small --- a $10 \times 10$ lattice at $M = K = 2$
over four positions --- and is refereed by the planted labels alone, which is
the cell Section~\ref{sec:applicability} marks as wanting an oracle. Under
the paired count emission of Section~\ref{sec:emissionmixture} the sequential
extent is what grows: an $8 \times 8$ lattice at $M = K = 2$ over 1,000
positions per pull request, and a triangular $71 \times 71$ at $M = K = 10$
over 20,000 positions past it. There the reference implementation of the
posterior is itself the oracle, an accelerated one being pinned to it, and the
planted labels referee the fit on top of that.
...
The estimator is not chosen here, it is dictated: neither marginal of
Equation~\ref{eq:joint} is tractable, so the blocks are what remain, and
what is actually chosen is the label solver and the start. Three algorithms
are assumed --- the per-class chain marginalization of
Section~\ref{sec:hmm}, one of the three label solvers of
Section~\ref{sec:potts}, and the annealed start of
Algorithm~\ref{alg:graph-burnin}, whose cluster move is
Algorithm~\ref{alg:wolff-field}.

\paragraph{What each size decides.}
The enumerable instance is where the constructed graph's log-density and the
per-class posteriors are scored against a sum, and it decides nothing about
the label solvers: what it referees is that log-density and those posteriors,
and the solvers are separated past it. There the planted
labels are the referee and the choice between the solvers is a measurement
at an equal budget of sweeps, whose standing result is that on the planted
lattice the start decides the labelling and the move does not --- a result
that could not have been argued for and had to be run.

The two axes past enumeration cost differently, and confusing them wastes a
budget. The sequential extent enters the chain pass linearly and is what
identifies the emission parameters, so more positions buy sharper
components. The node count enters the label solve, which is not linear in
it, and buys spatial resolution and nothing else. An instance at five
thousand nodes and twenty thousand positions is therefore two hard problems
bolted together rather than one large one, and its referee is the planted
labelling for the first and a reference posterior for the second.

\paragraph{Which label solver, and what each needs.}
Descent needs nothing of the model and is what remains when the budget is
too small for either alternative; it freezes into disjoint clusters, which
is a property of a single-site move against a prior that penalizes
disagreement and not a defect. Expansion needs the pairwise term to be a
metric on labels, which $\delta$ is, and returns the factor-of-two
certificate of Equation~\ref{eq:alpha-expansion}. The cluster move needs
non-negative couplings --- which Equation~\ref{eq:spatial-prior} has by
construction, so the refusal that binds Section~\ref{sec:frustrated} does
not bind here --- and moves a whole region at once, at the price of being a
sampler and therefore of a schedule, not a solver.
...
approximation of it. A gate over more than one chain state would leave the
same code running and the same monotonicity holding, with the expectation
step no longer the posterior. Two weaker assumptions follow it: that classes
are identified only up to permutation, and that the planted labelling is
what a correct estimator recovers --- which the section states is strictly
weaker than the planted labelling being the maximum of
...
is what a correct estimator recovers, and nothing establishes that it
maximizes Equation~\ref{eq:label-ground-state}. An enumerated
maximum-posterior labelling on the canonical instance would close that,
turning a recovery statement into an optimality one at one size. Past it, a
bound on the joint evidence would say how far a block ascent's fixed point
sits from the best available --- the mean-field bound of
Equation~\ref{eq:mean-field-bound} from below and the tree-reweighted bound
of Equation~\ref{eq:spanning-tree-bound} from above, both stated for a
lattice and neither yet carried on the coupled graph. Two further directions
are model rather than referee. The spatial coupling is declared and could be
fitted, which makes it a parameter of the same block ascent and its
identifiability a question the enumerable instance can answer. And the label
update is a discrete search over a neighbourhood with a computable
improvement, which is exactly the decision process of
Section~\ref{sec:policy-gradient}, so a learned proposal is admissible here on
the same terms it is admissible for a topology.
...
criterion and not of an implementation. It is the strongest assumption in
this section, and the reason the zone is carried as a \emph{pair} with its
consistent counterpart --- an implementation that is merely broken fails
both, so only the pair separates a defect from a theorem the section is
asserting the code reproduces.
...
\paragraph{What it would take to go further.}
This is the one search in the document where an exact method past enumeration
is plausibly affordable. The score is an integer and it decomposes over sites,
so a partial tree admits a lower bound on every completion of it and a branch
and bound can discard a whole subtree of the topology space without scoring
it \citep{papadimitriou1998, clrs2022}; nothing about a likelihood offers the
same, which is why the criterion is where an exact search would be built
first. A weighted criterion is the other direction: the step matrix is
declared here, and estimating one from the alignment --- subject to the metric
condition the search needs --- would make the criterion a fitted model rather
...
These instances add no algorithm. Descent, annealing and replica exchange
are Section~\ref{sec:potts}'s unchanged (Algorithms~\ref{alg:heat-bath}
and~\ref{alg:parallel-tempering}), and what the section supplies is an
answer known from outside at every size. That is the point of it: it is
where a search is scored past the reach of every oracle the rest of this
document uses.

\paragraph{What each instance can and cannot referee.}
The two are not interchangeable, and the difference decides what a
measurement on either may say. The triangular antiferromagnet states an
\emph{energy} and not a configuration: the optimum is massively degenerate,
so attaining Equation~\ref{eq:triangular-ground-state} proves a search
succeeded and says nothing about which ground state it found, and a claim
about \emph{the} ground state is not a well-formed question there. The
planted glass states an \emph{upper bound} and not an energy: a search
returning more than Equation~\ref{eq:planted-energy} has failed, and a
search reaching it has not thereby succeeded, since at a large inverted
fraction the true optimum lies below the planted state. One instance can
acquit and the other can only convict.
...
count: at zero inversion the instance is a gauge transform of the
ferromagnet, a random start finds the answer and the comparison measures
nothing, and the range worth running begins where the planted state stops
being near-optimal. A success rate reported without the fraction it was
measured at describes no instance. On the triangular lattice the analogous
parameter is the extent, and it moves the degeneracy rather than the energy:
the closed form holds at every extent while the number of configurations
attaining it does not, which is why the extents are declared rather than
swept.
...
refusals. The exact cut refuses, the energy not being submodular; the
cluster moves refuse, $1 - e^{-\beta J}$ not being a probability at a
negative coupling. So the exact and the near-exact methods are gone before
the comparison starts, and what is compared is three searches with no
guarantee between any two of them, at an equal budget of sweeps over shared
seeds. Those refusals are checked as results rather than described in prose,
which is what keeps the boundary between exact and approximate drawn in one
place.
...
extend them is anything that turns a comparison into a certificate. The
...
convert ``the best energy found'' into a bracket, and the guarantee stops
being a statement about the ensemble. Population annealing is the search-side
extension --- an ensemble carried down a schedule with resampling, rather than
replicas exchanged along one \citep{machta2010} --- and it is the comparison
Section~\ref{sec:tempering} would be made against at equal sweeps. A learned
proposal is admissible on the terms Section~\ref{sec:potts:extensions} states,
and these instances are where it would have to be measured, since they are the
ones with a known answer at every size.

\paragraph{What is deliberately not attempted.}
A disorder average. Every statement here is about one instance at one seed;
averaging over the ensemble of couplings is a different quantity and would
need a different accounting, and the inverted fraction is swept precisely so
that a per-instance result is reported with the disorder it was measured at. A
phase diagram, for the same reason the thermodynamic limit is out of
Section~\ref{sec:potts}: the residual entropy is quoted for orientation and
asserted at no finite size. And the exact ground state of the glass, which is
not available at any size here --- Equation~\ref{eq:planted-energy} is an
upper bound and the section is explicit that it can convict a search and never
acquit one.
...
Algorithm~\ref{alg:baum-welch} with the chain removed, the quasi-Newton fit
of Section~\ref{sec:abstraction} over the unconstrained vector, and the
seeding of Algorithm~\ref{alg:kmeanspp}. The interesting size here is not
$N$ and not $K$ but the dimension, and it is one by choice rather than by
convenience.

\paragraph{Why one dimension is a decision about the referee.}
Enumerating the $K^N$ labellings is out of reach at every declared size, so
the expectation step has no enumeration behind it and is refereed instead by
the chain recursion of Section~\ref{sec:hmm} on the same responsibilities
--- the one problem in this document whose oracle is borrowed from a
neighbouring class. The seeding's referee is the exact clustering cost, a
dynamic programme over cut positions in the sorted observations. That
programme exists because the observations are totally ordered; in higher
dimension the same quantity is NP-hard, so raising the dimension would not
make the fit harder, it would delete the only oracle the initializer has and
leave Equation~\ref{eq:kmeanspp-bound} with nothing to be checked against.
Growing $N$ buys precision and changes no referee, which is why the
simulator is checked at hundreds of thousands of draws and the method
comparison at hundreds.

\paragraph{Which fit.}
The alternation is cheaper per iterate and monotone; the gradient fit is
neither, and is carried because two routes that share the model and nothing
else are evidence where one route's convergence is not. Where the components
overlap the alternation approaches its fixed point linearly, so an iterate
stopped on a count sits above its basin's optimum by an amount a polish
removes --- and an equal-budget comparison that charges the polish to one
method and not to the others is comparing stopping rules.

\paragraph{What the model does not have.}
There is no maximum. A component contracting onto a single observation sends
the likelihood to infinity, so what any method here finds is a local optimum
by construction, and the floor that refuses such a fit is a modelling
decision rather than a numerical guard --- it decides which optima are
admissible. The count-emission family of Section~\ref{sec:emissionmixture}
is the same mixture without this pathology, both of its channels being
discrete, and the comparison between them is the clearest statement of what
the Gaussian's unboundedness costs.

\paragraph{What the validation assumes.}
Independence across observations, which is what makes the responsibilities a
per-observation normalization at all; and separation enough that ordering
the components resolves the permutation. Neither transfers: a mixture whose
components overlap past the ordering's reach fails the recovery test for a
reason that is a property of the instance and not a defect of the fit, so
the separation is declared with the instance rather than discovered from it.
...
\paragraph{What it would take to go further.}
Raising the dimension is the obvious extension and it costs the only oracle
the initializer has, so it is not free: the exact clustering cost is a
dynamic programme over cut positions in sorted observations and has no
counterpart above one dimension. What would replace it is either a certified
bound on the seeding cost that does not need the optimum, or a regime where
the planted labels alone referee and the section says so --- the second is
what Section~\ref{sec:emissionmixture} already does at ten components. The
unbounded likelihood is the other direction: a prior on the scales, or a
penalized objective, gives a maximum that exists rather than a floor that
refuses, and turns a modelling decision into a stated one \citep{bishop2006,
murphy2023}. Choosing $K$ rather than declaring it is model selection, and
the enumerable instance is where a criterion for it could be refereed at all.

\paragraph{What is deliberately not attempted.}
A component count fitted from the data, a non-diagonal covariance, and any
dimension above one --- each for the same reason, which is that this section
exists to referee an emission family and an initializer against exact
quantities, and each of the three removes one. The collapsing component is
kept rather than removed: it is the standing example of a likelihood with no
maximum, and a section that regularized it away would lose the case
...
compares two quantities that share no units.

\paragraph{The cost is the family's, not the sample's.}
Both shape parameters are found by bisection, at a few thousand score
evaluations per component per iterate, so the budget is spent on iterations
and on components and barely on observations. That is what makes the
ten-component instance a hard case rather than a larger one: $N$ in the low
thousands is not what the budget goes on. It also decides where a failure
appears. An inner solve that did not settle is
a local event, and a mixture that discarded its report would surface it as a
non-monotone outer likelihood several iterates later, at which point nothing
in the trace says which component or which iterate produced it --- so the
...
It runs over a prefix of ten observations, and that is licensed by exactly
one property: the posterior factorizes across observations, so the marginal
at each of ten is the same quantity the responsibilities of the whole
dataset carry. The prefix is therefore not a sample of the test but the test
itself, and it is at its sharpest precisely when the factorization is the
thing in doubt --- which is the property every step in this section rests
on and the one an implementation can break without any other check noticing.
...
different enumerations; and separation on depth and allele fraction
sufficient for an ordering to resolve the permutation, which is what a
declared instance supplies by construction and what makes an ordering
admissible where enumerating the alignments of ten components would not be.
Two flat directions bound what a fit may report at all: the dispersion
toward the Poisson limit and the concentration toward the binomial one each
have a value above which this much data cannot distinguish one value from
infinity, so an estimate arriving there has found a bound rather than a
maximum.
...
\paragraph{What it would take to go further.}
The flat directions are the sharpest gap. An estimate that reaches its
identifiability bound is reported as a bound, and what would say more is a
profile likelihood in that direction --- the curvature being absent, the
interval has to come from the objective's own shape rather than from the
observed information Section~\ref{sec:abstraction} pushes through the delta
method. The M step is the second: both shape parameters are found by
bisection, and a closed form for either would move the cost of an iterate by
the factor Section~\ref{sec:emissionmixture:discussion} says the budget is
spent on. The third is structural and already built one section over: putting
a chain over the component labels is exactly the coupled model of
Section~\ref{sec:coupled}, so what this section carries as an independent
mixture is the same family with its sequential structure removed, and the two
share a maximization step by construction.
...
Four methods are assumed and none is stated here: the quasi-Newton fit of
...
Algorithm~\ref{alg:random-restart}. The
functions are carried to separate those methods, not to be solved, so the
question the other sections ask --- which method to spend a budget on --- is
here asked in reverse: which failure does each surface expose.
...
The minimizers are closed forms, so the referee costs nothing at any
dimension and there is no per-tier size limit to state. What grows with $d$
is not the cost of checking but the number of basins a multi-start must
find, so the quantity that scales is a \emph{rate} --- the fraction of
starts reaching the global minimum --- and it is asserted low rather than
asserted to succeed. That is the bound on what a single fit of any
multimodal surface in this document may claim, and it is measured here
because here it can be.
...
\paragraph{What it would take to go further.}
The comparison here is restarts of a local fit against an annealed chain, and
the method it has not been run against is a population one: an evolution
strategy that adapts a covariance from its own samples is the standard
baseline for a rugged continuous surface and would be scored at the same
budget of objective evaluations \citep{hansen2001}. Two reporting extensions
matter more than they sound. A rate over starts is a statistic with an
interval, and stating it with the interval rather than as a number is what
makes two methods separable at all \citep{agarwal2021}; and the seeds, the
budget and the number of runs behind such a rate are part of the claim rather
than of its provenance \citep{henderson2018}. Both apply to every
equal-budget comparison in this document, and this is the surface where they
can be checked against a known answer.

\paragraph{What is deliberately not attempted.}
Constraints, which would make the constraint map of
Section~\ref{sec:abstraction} the thing under test rather than the optimizer.
A noisy objective, since these functions are carried precisely because their
value is exact and a fit that stopped early is distinguishable from a
parameter that is not identified. And high dimension: what grows with $d$ is
the number of basins, so a larger $d$ makes the rate smaller and the
measurement no sharper.
...
\paragraph{What is deliberately not attempted.}
Non-binary codes, since a binary variable's message being one number is what
makes Equation~\ref{eq:tanh-rule} a closed form rather than a sum. A channel
