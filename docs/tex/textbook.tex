...
Equation~\ref{eq:external-field} in its accept step. Its $H_{nm}$ is that
equation's field, an energy the labels minimize, so the step accepts on
$e^{-\beta \Delta H}$; the samplers of Section~\ref{sec:potts-sampling} take
a log-weight $h$ and accept on Equation~\ref{eq:cluster-accept}, so they are
handed $h = -H$, and the two acceptances are one. At $\beta \to 0$ no bond
