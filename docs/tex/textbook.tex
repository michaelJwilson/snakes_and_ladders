...
\title{Inference on Graphical Models: Problems and Algorithms \\
\large The problems, the algorithms, and the mathematical theorems and
properties that validate them}
\author{M.\ J.\ Wilson}
\date{\today}
...
\begin{abstract}
\noindent
The inference problems this project poses, the algorithms that answer them,
and the mathematical properties each answer is validated against. The problems
are stated first and ordered by complexity, from a continuous objective in a
handful of coordinates to a lattice, a phylogeny and an iteratively decoded
code; the algorithms follow, stated once and cited from every problem they
serve; and the validation is a ladder, from enumeration where it is affordable
through an inefficient algorithm where it is not to the capable one, scored
against both. This is a document about algorithms and not about an
implementation: no statement in it depends on how any of it is coded, and the
companion paper carries the measurements.
\end{abstract}
...
A consistent notation across the problems and the algorithms this text
describes, and across the companion paper: one symbol, one meaning.
Table~\ref{tab:notation} states it in two halves. Above the rule are the
symbols every section uses; below it are those belonging to one application,
which appear nowhere else and carry no meaning outside it.

Two conventions govern what follows. Optimization is minimization of a
negative log-likelihood: every objective $\objective$ is a quantity to be
made small, and a likelihood enters as $-\ln \Prob(\data \mid \cdot)$.
And $\objective$ is a function of $\params$, the unconstrained vector, rather
than of the parameters a person names; $\constrained$ connects the two, and
Section~\ref{sec:abstraction} says why the distinction is not cosmetic. A
probability is written $\Prob(\cdot)$ and its logarithm $\ln \Prob(\cdot)$,
the logarithm being natural throughout.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{ll}
    ...
    Symbol & Meaning \\
    \midrule
    \multicolumn{2}{l}{\emph{The factor graph, its parameters, and its objective}} \\
    ...
    $Z$            & a partition function; $\ln Z$ the evidence \\
    $F$            & a free energy, $F = -\ln Z$; a subscript names an approximation of it \\
    $q(\cdot)$     & a variational distribution over the latent variables \\
    $\graph$       & a general undirected graph \\
    $\tree$        & a tree topology \\
    $\rootnode$    & the root of a tree \\
    $\neighbourhood(\cdot)$ & the neighbourhood reachable in one step \\
    ...
    $\energy$      & a Hamiltonian: the energy of a configuration, script \\
    $\Prob(\cdot)$ & a probability; $\ln \Prob(\cdot)$ its logarithm, natural throughout \\
    $T,\ \beta = 1/T$ & a temperature and its inverse; a schedule is $T(t)$ over a budget of $t$ steps \\
    $\indicator[\,\cdot\,]$ & one where its condition holds and zero where it does not \\
    \midrule
    \multicolumn{2}{l}{\emph{Statistical physics: spins on a graph} (Sections~\ref{sec:potts}, \ref{sec:frustrated})} \\
    $\sigma_i$     & a spin, the variable of a Potts graph: $x_i \equiv \sigma_i$ \\
    $q$            & the number of Potts states \\
    $J_{ij}$       & the coupling on the edge $\langle i, j \rangle$ \\
    $h_i$          & the external field at node $i$ \\
    $\transfer$    & the transfer matrix between adjacent columns of a lattice \\
    \addlinespace
    \multicolumn{2}{l}{\emph{Sequential models: a chain and its emissions} (Sections~\ref{sec:hmm}, \ref{sec:coupled})} \\
    $z_t$          & a hidden state, the variable of a chain: $x_t \equiv z_t$ \\
    $\hidden$      & a hidden configuration, $(z_1, \dots, z_T)$ \\
    $\mathbf{A}$   & a transition matrix \\
    $\emission$    & an emission family \\
    \addlinespace
    \multicolumn{2}{l}{\emph{Phylogenetics: a tree and its substitution model} (Sections~\ref{sec:phylo}, \ref{sec:parsimony})} \\
    ...
    $\branch$      & branch lengths, in expected substitutions per site \\
    $\hadamard$    & the Sylvester--Hadamard matrix, italic \\
    $F(\tree),\ F_s$ & a parsimony score: the changes a topology implies, over the alignment and at one site \\
    \addlinespace
    \multicolumn{2}{l}{\emph{Error correction: a code and its channel} (Sections~\ref{sec:ldpc}, \ref{sec:turbo})} \\
    $\parity$      & a parity-check matrix, bold; the codewords are the $c$ with $\parity c = 0$ \\
    $L_i$          & the channel log-likelihood ratio at bit $i$ \\
    \bottomrule
  \end{tabular}
  \caption{The notation of this document and the companion paper. The symbols
  above the rule are general and mean the same thing in every section; each
  group below it belongs to the application named beside it, where the general
  variable $x_i$ is identified with the name that application's readers arrive
  with. Three symbols share the letter $H$ and are kept apart by their face: a
  Hamiltonian is script, a parity-check matrix bold, and the
  Sylvester--Hadamard matrix italic. Two more carry a second meaning in one
  application and are listed below the rule where they do: $q$ is the Potts
  state count and $F$ a parsimony score.}
  \label{tab:notation}
\end{table}

\section{Graphical Models, Likelihood and the Partition Function}
\label{sec:foundations}

Every problem stated below is a distribution over discrete variables with
parameters attached, and everything asked of one --- a likelihood, an evidence,
an expectation step, a bound, a sample --- is read off a single sum. This
section states that sum once and reads the rest off it, so that a problem
statement says what its own sum runs over and assumes the remainder. Nothing
here is new \citep{koller2009, wainwright2008, bishop2006, mackay2003}; stating
it once is what lets the sections after it be short.

\subsection{Graphical models}

A directed model factorizes over conditionals and an undirected one over
potentials on the cliques of a graph,
\[
  \Prob(x) = \prod_i \Prob\big(x_i \mid x_{\mathrm{pa}(i)}\big),
  \qquad
  \Prob(x) = \frac{1}{Z} \prod_c \psi_c(x_c),
\]
the first normalized by construction, every conditional summing to one over its
own variable, and the second only through $Z$. The distinction does not survive
conditioning: fixing an observed child leaves its parents coupled by a product
that no longer sums to one, so a directed model with data in it is an
undirected problem with a normalizer to find. Hammersley--Clifford closes the
other direction --- a strictly positive distribution whose conditional
independences are those of $\graph$ factorizes over the cliques of $\graph$
\citep{koller2009}.

Both forms are products of non-negative functions of subsets of the variables,
which is the factor graph of Equation~\ref{eq:factor-graph}, and it is the one
form this document works in. Six constructions carry the problems below onto
it: a Potts graph in a field, a hidden Markov chain, a rooted tree under a
substitution model, the coupled spatio-sequential model, a parity-check matrix
with its channel likelihoods, and a convolutional trellis. Each problem
section's \emph{As a factor graph} says which factors it has, and that
statement is the whole of its structure; what follows here is true of all six
at once.

\subsection{The partition function}

Writing the factors' dependence on the parameters makes the sum of
Equation~\ref{eq:factor-graph} a function of them:
\begin{equation}
  Z(\params) = \sum_x \tilde{p}(x; \params),
  \qquad
  \tilde{p}(x; \params) = \prod_a \psi_a(x_{\partial a}; \params),
  \qquad
  F(\params) = -\ln Z(\params),
  \label{eq:partition}
\end{equation}
with $\tilde{p}$ the unnormalized measure and $F$ the free energy. It is
Equation~\ref{eq:factor-graph}'s normalizer with the parameters written in, and
it is the object the rest of this section differentiates, bounds and
interpolates.

Where the factors are exponential in the parameters,
$\psi_a(x_{\partial a}; \params)
 = \exp\big(\params^{\top} s_a(x_{\partial a})\big)$, $\ln Z$ is the cumulant
generating function of the sufficient statistics $s = \sum_a s_a$: its first
derivative is $\mathbb{E}[s]$, its second the covariance of $s$, and $\ln Z$ is
convex in $\params$ because a covariance is positive semi-definite
\citep{wainwright2008}. The first derivative needs no exponential form,
\begin{equation}
  \frac{\partial \ln Z(\params)}{\partial \params}
    = \sum_a \mathbb{E}_{p(x_{\partial a})}
      \Big[ \frac{\partial \ln \psi_a(x_{\partial a}; \params)}
                 {\partial \params} \Big],
  \label{eq:logz-gradient}
\end{equation}
the sum over factors of the derivative of a factor's own log-potential averaged
under the belief at that factor (Appendix~\ref{app:logz}). One marginalization
therefore serves the inference and the fit alike, which is what
Section~\ref{sec:factor-graph} builds the mixed optimization on.

\paragraph{Where the sum is available.}
Three structures give $Z$ exactly and one common structure gives none of them.
On a chain the sum collapses into a product of matrices, which is the transfer
matrix of Equation~\ref{eq:transfer} at a column of one site and the forward
recursion of Equation~\ref{eq:forward} written out. On a tree one leaf-to-root
pass of Equation~\ref{eq:sum-product} does the same, which is pruning,
Equation~\ref{eq:pruning}. On a graph small enough the sum is taken term by
term, which is the base rung of Section~\ref{sec:ladder} and is refused past
200,000 states. A lattice is none of the three: computing $Z$ on a general
graph is \#P-hard \citep{koller2009}, so what is reported there is a bracket
--- Equation~\ref{eq:mean-field-bound} below and
Equation~\ref{eq:spanning-tree-bound} above --- or a stationary point of an
approximate free energy, Equation~\ref{eq:bethe}, reported as one.

\subsection{Maximum likelihood}

Equation~\ref{eq:partition} splits the log-probability of a configuration into
a term that is local and a term that is not,
\[
  \ln \Prob(x \mid \params)
    = \ln \tilde{p}(x; \params) - \ln Z(\params),
\]
the first a sum over the factors $x$ touches and the second a sum over every
configuration there is. Maximum likelihood maximizes this over $\params$; its
cost is entirely in the second term, and every algorithm here that fits
parameters is an arrangement for not paying that cost twice.

Differentiating $\sum_x \Prob(x \mid \params) = 1$ under the sum gives the
score identity
\begin{equation}
  \mathbb{E}_{\Prob(\cdot \mid \params)}
    \big[ \nabla_{\params} \ln \Prob(x \mid \params) \big] = 0,
  \label{eq:score}
\end{equation}
which holds at every $\params$ and not only at the generating one: a fitted
gradient at zero is evidence of a stationary point and never of correctness.
The score's variance is the Fisher information
$I(\params)
 = \mathbb{E}\big[\nabla \ln \Prob \; \nabla \ln \Prob^{\top}\big]
 = -\,\mathbb{E}\big[\nabla^2 \ln \Prob\big]$,
the second equality being the information identity, and it is what makes the
observed information at an optimum an estimate of the curvature
Section~\ref{sec:abstraction} pushes through $\constrained$.

Misspecification breaks that identity. For an objective that is not the data's
own log-likelihood the two sides differ and the estimator's asymptotic variance
is the sandwich, or Godambe, form $A^{-1} B A^{-1}$ --- $A$ the expected
Hessian of the objective, $B$ the variance of its gradient --- which collapses
to $I^{-1}$ exactly when $A = -B$. Every fit stated below maximizes the model's
own likelihood, with one structural exception: on a loopy graph the quantity
maximized is an approximate evidence (Section~\ref{sec:factor-graph}), nothing
makes $A = -B$ there, and an interval read from the observed information alone
is not justified by Equation~\ref{eq:score}.

\subsection{Bayesian inference}

A prior $\Prob(\params)$ and a likelihood $\Prob(\data \mid \params)$ give a
posterior $\Prob(\params \mid \data) \propto \Prob(\data \mid \params)\,
\Prob(\params)$, and the constant of proportionality is a partition function
again. Write the prior as an unnormalized measure $\tilde{p}_0$ with normalizer
$Z_{\mathrm{prior}} = \int \tilde{p}_0(\params)\, \mathrm{d}\params$ and the
posterior's measure as $\tilde{p}_0\, \Prob(\data \mid \cdot)$ with normalizer
$Z_{\mathrm{post}}$; then
\begin{equation}
  \Prob(\data) = \frac{Z_{\mathrm{post}}}{Z_{\mathrm{prior}}},
  \label{eq:evidence-ratio}
\end{equation}
so the evidence is one object taken twice and every statement above about $Z$
applies to it. The marginal likelihood of a latent-variable model is the same
ratio with the latent variables summed rather than the parameters integrated,
which is why one section states both.

Raising a posterior to a power $\beta$ and letting $\beta \to \infty$
concentrates it on its mode, so a maximum a posteriori configuration is the
zero-temperature limit of the same measure --- the limit in which sum-product
becomes max-product (Section~\ref{sec:factor-graph}) and the heat bath becomes
iterated conditional modes (Section~\ref{sec:tempering}). A weight over
structures is not a posterior over them until the prior is stated:
Section~\ref{sec:phylo:extensions} is where that matters, contrasting a
posterior over topologies with the flat-prior weight over maximized likelihoods
that Equation~\ref{eq:tempered-weight} estimates.

\subsection{Latent variables and expectation--maximization}

Where part of a configuration is unobserved, the complete-data likelihood
$\Prob(x, \data \mid \params)$ is the product of the factors and the
observed-data likelihood is its sum over the latent $x$ --- a $\ln Z$ again,
with the observations folded into the factors as
Section~\ref{sec:factor-graph} folds them. Expectation--maximization alternates
on that sum \citep{dempster1977}. The E step forms
\[
  Q(\params \mid \params')
    = \mathbb{E}_{\Prob(x \mid \data, \params')}
      \big[ \ln \Prob(x, \data \mid \params) \big],
\]
an expectation under the posterior at the current parameters, which is what one
marginalization returns; the M step maximizes $Q$ over $\params$, and by
Equation~\ref{eq:logz-gradient} its gradient is the posterior-weighted sum that
marginalization has already computed. The two steps therefore share one pass
and not two.

Monotonicity is Jensen's inequality and nothing more: the change in the
observed-data log-likelihood over a round is the gain in $Q$ plus a
Kullback--Leibler divergence between the old posterior and the new, both
non-negative, so a step that does not decrease $Q$ does not decrease the
likelihood. That is an ascent and not a convergence result --- the fixed point
is stationary and need not be the maximum --- which is why every alternation
below is held to monotonicity at each iterate \emph{and} has its fixed point
checked against an independent gradient fit of the same likelihood.

Replacing the posterior expectation by its mode is hard, or classification,
expectation--maximization. It ascends the complete-data likelihood rather than
the marginal, and the two agree only where the posterior is nearly degenerate.
Section~\ref{sec:coupled:algorithms} runs that limit, and runs it in one block
only: the labels are maximized, as the ground state of
Equation~\ref{eq:label-ground-state} under iterated conditional modes or alpha
expansion, while the chain states stay summed. A hard step in a block whose
posterior is not degenerate is a different objective and not a cheaper E step.

\subsection{The evidence lower bound}

For any distribution $q$ over the latent variables,
\begin{equation}
  \ln \Prob(\data) \;\ge\;
    \mathbb{E}_q \big[ \ln \Prob(\data, x) \big]
    - \mathbb{E}_q \big[ \ln q(x) \big],
  \label{eq:elbo}
\end{equation}
the gap being the Kullback--Leibler divergence from $q$ to the posterior and
the equality holding exactly at $q = \Prob(x \mid \data)$.
Expectation--maximization is coordinate ascent on Equation~\ref{eq:elbo}: the E
step maximizes over $q$ and closes the gap, the M step maximizes over $\params$
\citep{neal1998}. Reading it that way is what admits the variants --- a partial
E step, an incremental one over part of the data --- each still an ascent of
one bound, and it is why an approximate E step returns a bound rather than an
evidence.

Restricting $q$ to a family makes the inequality strict and the gap a bias
rather than an error. Under a factorized $q = \prod_i q_i$,
Equation~\ref{eq:elbo} is the mean-field bound of
Equation~\ref{eq:mean-field-bound}, tightest at its fixed point and below
$\ln Z$ everywhere. The Bethe free energy of Equation~\ref{eq:bethe} relaxes
further, from a distribution to a set of locally consistent beliefs, and loses
the inequality with it: a loopy fixed point is a stationary point of an
approximate free energy and is reported as one, never as a bound
(Appendix~\ref{app:bethe}). The upper end comes from convexity instead,
Equation~\ref{eq:spanning-tree-bound}. Section~\ref{sec:surrogates} states the
three with what each costs, and nothing here re-derives them.

\subsection{The correspondence}

Table~\ref{tab:partition-functions} is the same sum read once per problem: what
$Z$ is, whether it is available exactly, and which rung of
Section~\ref{sec:ladder}'s ladder referees it. A row saying \emph{bounded} is a
row where the number reported carries a bracket or an approximation and is
reported as such, never as the sum.

\begin{longtable}{@{}p{0.20\textwidth}p{0.31\textwidth}p{0.17\textwidth}p{0.21\textwidth}@{}}
  \caption{The partition function of each problem this document states, what it
  is a sum over, whether it is exact at the supported sizes, and the rung of
  the validation ladder that referees it. The rungs are named as
  Section~\ref{sec:ladder} orders them, an exact enumeration at the base and
  the capable algorithm above it; where a row names two, the first is the
  oracle and the second the referee that carries past
  it.}\label{tab:partition-functions}\\
  \toprule
  Problem & What $Z$ is & Exact or bounded & Rung that referees it \\
  \midrule
  \endfirsthead
  \toprule
  Problem & What $Z$ is & Exact or bounded & Rung that referees it \\
  ...
  Phylogenetic tree, Jukes--Cantor
  & One site's likelihood; the alignment's evidence is the product over $L$
    sites
  & Exact: the graph is a tree
  & pruning \\
  Phylogenetic tree, general time-reversible
  & The same sum with a fitted rate matrix in the branch factors
  & Exact
  & pruning \\
  Phylogenetic tree, parsimony
  & No sum: the score counts changes, the zero-temperature limit of the same
    marginalization
  & Exact
  & parsimony, Fitch \\
  Potts chain in a field
  & The Boltzmann normalizer over $q^{|V|}$ configurations
  & Exact
  & enumeration; transfer matrix \\
  Potts lattice and Markov random field
  & The same normalizer on a loopy graph
  & Bounded
  & enumeration; variational bounds \\
  Potts lattice in a per-site field
  & The same, the field varying by site
  & Exact to the strip width, bounded above it
  & transfer matrix \\
  Potts lattice in a tiling field
  & The same, the field rewarding one planted state per tile
  & Exact to enumeration, bounded above it
  & enumeration \\
  Frustrated Potts, triangular antiferromagnet
  & $Z(\beta)$, whose $\beta \to \infty$ limit carries the ground-state energy
  & Bounded past enumeration
  & enumeration \\
  Planted spin glass
  & The same, with the ground state fixed by construction
  & Bounded past enumeration
  & enumeration \\
  Low-density parity-check code, Gallager
  & The sum over codewords of the channel likelihood; per bit, the normalizer
    of that bit's posterior
  & Exact where the graph is cycle-free
  & brute-force ML \\
  Low-density parity-check code, bicycle
  & The same sum on a graph constructed to carry cycles
  & Bounded: a Bethe fixed point
  & brute-force ML \\
  Calderbank--Shor--Steane code over a bicycle matrix
  & The sum over an error coset of one syndrome
  & Bounded: a Bethe fixed point
  & CSS / syndrome \\
  Turbo code, two $(7,5)$ encoders
  & Per constituent, the trellis sum the forward recursion normalizes
  & Exact per constituent, iterative jointly
  & BCJR \\
  Polar code
  & The sum over inputs consistent with the frozen set, taken by the recursive
    transform
  & Exact per decision, bounded by the list
  & polar SC / SCL \\
  Hidden Markov model
  & The forward recursion's normalizer, $Z = \Prob(\data)$
  & Exact: the chain is a tree
  & path enumeration \\
  Coupled spatio-sequential model
  & The joint normalizer over $M^{N} K^{SM}$ states, neither marginal tractable
    alone
  & Exact at the enumerable instance, bounded above it
  & coupled model, exact \\
  Count-pair coupled spatio-sequential model
  & The same with a count-pair emission
  & Exact at the enumerable instance, bounded above it
  & coupled model, exact \\
  Gaussian mixture
  & The responsibilities' normalizer, $\sum_k w_k\,
    \mathcal{N}(y_i; \mu_k, \sigma_k^2)$, per observation
  & Exact
  & assignment enumeration \\
  Mixture of two-channel count emissions
  & The same with the count family in place of the Gaussian
  & Exact
  & assignment enumeration \\
  Continuous test functions
  & None: the objective is evaluated and nothing is marginalized
  & Not applicable
  & --- \\
\end{longtable}

Two readings of the table are used throughout and are worth naming here. A
temperature turns one of these sums into a path of them, $Z(\beta)$ from a
normalizer that can be counted to the one that is wanted, and the annealed
estimators of Section~\ref{sec:tempering} read $\ln Z$ off the work done along
that path \citep{neal2001} --- Equation~\ref{eq:evidence-ratio} being the same
ratio taken in a single step. And sampling is the other reading of the same
object: every chain below draws from the measure $Z$ normalizes and never needs
$Z$ itself, which is why a sampler reaches sizes at which no evaluator of the
sum does.

\section{Mixed Discrete and Continuous Optimization}
\label{sec:shape}

Each problem below couples a \emph{combinatorial} search over a structure to a
\emph{continuous} optimization of parameters given that structure. The two are
not interleaved steps of one optimizer: a structural move changes what the
parameter vector \emph{means} and how long it is, so it constructs a new
objective rather than stepping inside an existing one. Everything else here
follows from that division.

\subsection{Message Passing on Factor Graphs}
...
\paragraph{The evidence is the objective.}
Where the factors carry parameters $\params$, the partition function of
Equation~\ref{eq:factor-graph} is the marginal likelihood of the observations
folded into them, $Z(\params) = \Prob(\data \mid \params)$, so maximum
likelihood over a model with latent variables \emph{is} maximization of
$\ln Z$ --- the mixed case in which the discrete variables are summed out and
the continuous ones fitted. Its gradient is the posterior expectation of
Equation~\ref{eq:logz-gradient}, stated with the rest of the foundations in
Section~\ref{sec:foundations}. Two things
follow, and both are used throughout. The marginalization that
Equation~\ref{eq:sum-product} performs is what an optimizer of the continuous
parameters needs, so one pass serves both --- the expectation step of an
expectation--maximization is exactly the beliefs, and the maximization step
reads Equation~\ref{eq:logz-gradient}. And where the graph is loopy the beliefs
are the Bethe approximation of Equation~\ref{eq:bethe}, so the fitted optimum
is the stationary point of an approximate evidence and is reported as one.

\paragraph{An application enters as a factor.}
A leaf's observed character, an emission, a parity check and a gated emission
are each one factor of Equation~\ref{eq:factor-graph} and nothing more; the
branch lengths of a phylogeny are the parameters of the pairwise factors along
its edges. Each problem statement below says which factors it has, and that
statement is the whole of its structure.
...
\section{The Ladder of Validation}
\label{sec:ladder}

Every claim below is refereed, and what referees it is decided by the size of
the instance rather than by the method. The arrangement is a ladder, and each
problem statement climbs as much of it as its sizes allow.

At the base is \emph{enumeration}: the sum over every configuration, every
path, every internal labelling or every topology. It is exponential, which is
what makes it an oracle rather than a second opinion, and it fixes the
smallest fixtures --- a size is chosen because enumeration reaches it, and
every enumeration here refuses more than 200,000 states rather than attempting
them.

Above it are the \emph{simple, inefficient} algorithms: a transfer matrix over
a strip, a direct marginalization that shares no recursion with the recursion
it scores, a reference implementation with no acceleration in it. They reach
sizes enumeration does not and are exact where they apply, so they carry the
referee one rung further at a cost that is polynomial rather than affordable.

Above them are the \emph{capable} algorithms, the ones a result is actually
produced with. They are scored against the two rungs below wherever those
reach, and past that against each other and against the simulated truth --- the
generating parameters or structure of a seeded fixture, to a stated tolerance
or coverage. Agreement between two capable algorithms is agreement and not
truth (Section~\ref{sec:properties}).

Which rung runs when is a second axis, and the two are independent: a check
is refereed somehow and separately gated somewhere. Four gates are used, and
each problem's supported sizes name theirs. The \emph{per pull request} tier
carries the sizes a merge waits for and is bounded by a stated duration. The
\emph{stress} tier carries the same claim one size up, past that bound and
inside a developer's. The \emph{release} tier carries everything, and a check
too slow for the first tier is moved here rather than waited for. And the
\emph{key} fixture is one instance per problem --- the largest declared one
whose whole simulate, fit and assert run fits a stated cap --- run in full so
that at least one instance of every problem is exercised end to end rather
than in pieces. A claim moved up a tier keeps a small sibling below it where
one exists, and where none does the gap is named rather than left implicit.

A problem states its own ladder in its validation, and the Potts lattice of
Section~\ref{sec:potts} is where all three rungs are visible at once:
enumeration of $q^{|V|}$ configurations up to eleven nodes at three states,
the transfer matrix over a strip of any length at a width that fits, and
belief propagation, sampling and the cut solvers scored against both and then
run where neither reaches. What the ladder does not reach is named rather than
covered: Section~\ref{sec:applicability} marks each such cell and says which
oracle would close it.


\begin{center}\rule{0.45\linewidth}{0.4pt}\end{center}
\addcontentsline{toc}{section}{\textit{The General Problems}}
\noindent\textit{The General Problems.} The four problems below are stated in the language of
Section~\ref{sec:factor-graph} and in nothing else: an objective over a handful
of continuous coordinates, a mixture of two kinds, and a chain. None carries an
application, and every algorithm any of them uses is general.
\begin{center}\rule{0.45\linewidth}{0.4pt}\end{center}

\section{Continuous Test Functions}
...
Section~\ref{sec:testfunctions:algorithms} states them: the
quasi-Newton fit of Section~\ref{sec:abstraction} under the identity
constraint map, the multi-start of Algorithm~\ref{alg:random-restart}, the
Hamiltonian sampler of Appendix~\ref{app:hmc} read as an energy, and the
annealing of Appendix~\ref{app:tempering} on a schedule. Nothing here is
specific to these surfaces, which is why they referee the machinery the
likelihood problems share.

\section{The Gaussian Mixture}
\label{sec:mixture}
...
\subsection{The algorithms}
Section~\ref{sec:mixture:algorithms} states them: the
alternation of Algorithm~\ref{alg:baum-welch} with no chain in it, the
gradient fit over the unconstrained vector of Section~\ref{sec:abstraction},
and the $k$-means$++$ seeding of Algorithm~\ref{alg:kmeanspp}, which is the
initializer of Section~\ref{sec:initializers} in its data-sampled kind.

\section{The Count-Emission Mixture}
...
\subsection{The algorithms}
Section~\ref{sec:emissionmixture:algorithms} states them: the
same alternation, Algorithm~\ref{alg:baum-welch}, over an emission family whose
maximization step is itself an optimization, and the family-aware seeding that
replaces the squared distance of Algorithm~\ref{alg:kmeanspp} by the family's
own divergence.
...
A \emph{batch} of such paths is not one longer path. Where the data is
segmented into $S$ chains of lengths $T_1, \dots, T_S$, each chain restarts at
$\bpi$ rather than continuing through $\mathbf{A}$, so the batch takes
$\sum_s (T_s - 1) = T - S$ transitions and not $T - 1$. The segmentation is
therefore part of the model and not a detail of its storage: at a fixed total
$T$, a single chain and twenty short ones assign different probabilities to the
same observations, because a state change \emph{across} a boundary is free
where the same change one position earlier costs $\log A_{ij}$. A segment of
one position carries an initial distribution and no transition, and is excluded
as degenerate.
...
\paragraph{Chains of unequal length.}
\label{par:hmm:ragged}
A batch need not be rectangular. Writing $y^{(r)}_{1:T_r}$ for the $r$th
sequence, the likelihood of $R$ sequences of lengths $T_1, \dots, T_R$ is the
product
\begin{equation}
  p(\data \mid \params)
    = \prod_{r=1}^{R} p\big(y^{(r)}_{1:T_r} \mid \params\big),
  \label{eq:hmm-ragged}
\end{equation}
each factor the forward recursion of Equation~\ref{eq:forward} run to its own
length $T_r$. Nothing in the model couples the sequences, so this is a statement
about the data and not a second model: the parameters are shared and the
lengths are not.

\emph{Padding is an implementation of Equation~\ref{eq:hmm-ragged} and never
a model.} A batch padded to its longest member and masked computes the same
product, and what it costs is the padding: the work grows as
$R \cdot \max_r T_r$ where the sum $\sum_r T_r$ is what the likelihood needs.
The two agree to floating point and the gap between them is the padding
fraction, which is a property of the batch rather than of either route. An
unmasked padded position is a different likelihood, so every consumer masks
--- in the expectation step and in the emission re-estimation alike.
...
\subsection{The algorithms}
Section~\ref{sec:hmm:algorithms} states them: the two-pass
marginalization of Equation~\ref{eq:forward} and Equation~\ref{eq:posterior},
which is Equation~\ref{eq:sum-product} on a chain; the alternation that fits
it, Algorithm~\ref{alg:baum-welch}; the two decodings, of which
Equation~\ref{eq:viterbi} is one; and the block draw of a whole path from its
conditional, Algorithm~\ref{alg:chain-block}.

\begin{center}\rule{0.45\linewidth}{0.4pt}\end{center}
\addcontentsline{toc}{section}{\textit{Application: Phylogenetics}}
\noindent\textit{Application: Phylogenetics.} The two problems below are the factor graph of
Section~\ref{sec:factor-graph} on a tree, and the tree is what an application
supplies. Everything application-specific in this document --- a substitution
model, a taxon, a parsimony score --- enters here, in
Section~\ref{sec:coupled}, and in Sections~\ref{sec:ldpc}
and~\ref{sec:turbo}; the sections between them carry none of it.
\begin{center}\rule{0.45\linewidth}{0.4pt}\end{center}
...
indicator per leaf, $\psi_\ell(x_\ell) = \indicator[x_\ell = \text{observed}]$,
...
\subsection{The algorithms}
Section~\ref{sec:phylo:algorithms} states them: the pre-order
simulation of Algorithm~\ref{alg:simulate}; the post-order marginalization of
Equation~\ref{eq:pruning} and the adjoint that differentiates it,
Algorithm~\ref{alg:pruning-backward}; the climb over a topological
neighbourhood, Algorithm~\ref{alg:hill-climbing}; the distance, agglomeration
and spectral starts of Section~\ref{sec:spectral-start}, which are
Algorithm~\ref{alg:neighbor-joining} and Algorithm~\ref{alg:hadamard-start};
and the tropical relaxation of Section~\ref{sec:tropical}.

\section{Large Parsimony}
\label{sec:parsimony}
...
\subsection{The algorithms}
Section~\ref{sec:parsimony:algorithms} states them: the same
climb, Algorithm~\ref{alg:hill-climbing}, with one post-order pass in place of
the fit, over the neighbourhoods Section~\ref{sec:phylo:search} defines.

\begin{center}\rule{0.45\linewidth}{0.4pt}\end{center}
\addcontentsline{toc}{section}{\textit{The General Problems, Continued: Spins on a Graph}}
\noindent\textit{The General Problems, Continued: Spins on a Graph.} A Potts model in an
external field is the general discrete problem at every size, and a frustrated
lattice is the instance that separates the searches for it. Neither carries an
application; both are cited by the two that do.
\begin{center}\rule{0.45\linewidth}{0.4pt}\end{center}

\section{Potts Models in an External Field}
\label{sec:potts}
...
is that the covariate still tilts the labels. The tiling field of
Section~\ref{sec:alpha-expansion} is declared on a triangular $4 \times 3$ at
three states, $3^{12} = 531{,}441$ labellings enumerated for the ground state,
and on the same $71 \times 71$ at ten states, where the planted states are
what is asserted. The general graph carries
...
\subsection{The algorithms}
Section~\ref{sec:potts:algorithms} states them: the message
iteration of Equation~\ref{eq:bp-message} and the free energy of its fixed
point; the minimum cut and the expansion cycle built on it,
Algorithm~\ref{alg:alpha-expansion}; the single-site and cluster updates,
Algorithms~\ref{alg:heat-bath} and~\ref{alg:cluster-move}; and the replica
exchange over them, Algorithm~\ref{alg:parallel-tempering}, whose ladder is
Appendix~\ref{app:tempering}.

\section{Frustrated Lattices}
...
\subsection{The algorithms}
Section~\ref{sec:frustrated:algorithms} states them, and they
are Section~\ref{sec:potts:algorithms}'s unchanged: enumeration and the maximum
cut where they reach, the semidefinite relaxation at any size, single-site
descent, which is Algorithm~\ref{alg:heat-bath} at zero temperature, and the
annealing and replica exchange of Algorithm~\ref{alg:parallel-tempering}.

\begin{center}\rule{0.45\linewidth}{0.4pt}\end{center}
\addcontentsline{toc}{section}{\textit{Application: Spatial Transcriptomics}}
\noindent\textit{Application: Spatial Transcriptomics.} The coupled model joins three of the
general problems above into one graph, and is where a lattice, a chain and a
mixture are fitted together.
\begin{center}\rule{0.45\linewidth}{0.4pt}\end{center}

\section{The Coupled Spatio-Sequential Model}
\label{sec:coupled}
...
parameters $\theta_m$. The joint is
\begin{equation}
  \begin{split}
    \log p(\ell, k, x)
      &= \beta \sum_{\langle n,n' \rangle} J_{nn'}\,
         \delta(\ell_n, \ell_{n'}) - \log Z_{\mathrm{Potts}} \\
      &\quad + \sum_m \Big[ \log \pi_m(k_{1,m})
         + \sum_{s > 1} \log A_m(k_{s-1,m}, k_{s,m}) \Big] \\
      &\quad + \sum_n \sum_s
         \log P(x_{sn} \mid k_{s,\ell_n}, \theta_{\ell_n}),
  \end{split}
  \label{eq:joint}
\end{equation}
Equation~\ref{eq:factor-graph} with pairwise, chain and degree-two gated
factors, and no fourth code path.

Two of these terms admit a quantity the model conditions on rather than fits.
An emission may be scored against a per-observation \emph{covariate}
$c_{sn}$ --- what the observation was measured against, an exposure for a rate
or a number of trials for a bounded count --- so the emission term reads
$\log P(x_{sn} \mid k_{s,\ell_n}, \theta_{\ell_n}, c_{sn})$. A family whose
observation has several channels takes one covariate per channel, because the
channels condition on different kinds of quantity and a single value names
neither. Setting $c_{sn}$ to each channel's neutral value recovers the
uncovaried model exactly; the neutral value is a family's to define and is not
one for every channel. A covariate that varies and is not conditioned on is
not ignored --- it is absorbed, and the fit recovers the average of what it
should have separated.

The chain term likewise admits $A_m(k_{s-1,m}, k_{s,m})$ varying with $s$: a
kernel per transition rather than one for the chain. It carries
$(S-1)K(K-1)$ free values against $S-1$ transitions per chain, so it is
conditioned on and not estimated, and what a caller assembles it from --- a
distance between positions, a product over two latents --- is outside this
statement. Its three terms are the three problem
classes above: a Potts prior on the labels,
\begin{equation}
...
\end{equation}
ferromagnetic, so that neighbouring nodes prefer one class. Written as a
penalty on disagreement it is $-\beta \sum J_{nn'}\,
\indicator[\ell_n \ne \ell_{n'}]$, the same distribution up to a constant.
Next, one hidden Markov chain per class, in the simplest case with a
circulant transition,
\begin{equation}
...
and a \emph{gated} emission, $\indicator[\ell_n = m]\,\log p(x_{sn} \mid k_{s,m},
...
The chains are per class and not per spot: each of the $N$ spots carries one
label $\ell_n \in \{1, \dots, M\}$, and each of the $M$ classes carries one
chain of $S$ states drawn from $K$, shared by every spot the labels assign to
it. So the labels contribute $M^{N}$ and the chains $K^{SM}$, and the joint
has $M^{N} K^{SM}$ states. The count is \emph{not} $K^{SN}$, which is the
different model in which every spot owns a private chain; there the classes
would carry no information and the coupling this section is about would not
exist. The gated emission above is what makes the
distinction observable: a spot reads the profile of its own class, so two
spots sharing a label share a profile exactly.
The enumerable instance is therefore small: the
...
\paragraph{The count-pair instance, and what a covariate conditions on.}
\label{par:coupled:pairs}
Under the paired count emission a spot's observation at a position is two
counts, a total and a successes, and each channel may carry a
\emph{covariate}: a per-observation quantity the model conditions on rather
than fits. The two channels take different ones, and the difference is the
whole of the contract:
\begin{itemize}
  \item an \emph{exposure} $c_{s,n} > 0$ on the total channel multiplies the
    rate the count is drawn at, so a negative-binomial total at mean $\mu$
    becomes one at mean $c_{s,n}\mu$. The identity that makes this a
    conditioning and not a reparameterization is
    $c\,\Gamma(r, \mu/r) = \Gamma(r, c\mu/r)$: the shape is untouched, so the
    dispersion the fit estimates is the same parameter it would estimate at
    unit exposure;
  \item a \emph{trial count} $m_{s,n}$ on the successes channel
    \emph{replaces} the declared number of trials, so the beta-binomial is
    scored out of what was actually observed rather than out of a constant.
\end{itemize}
A covariate that varies and is not conditioned on is not neutral: the fit then
estimates the rate averaged over the exposures. On the fixture below, planted
rates of $24$ and $72$ with exposures spanning a factor of sixteen are
recovered as $23.3$ and $71.3$ when the fit is told them, and as $47.9$ and
$151.7$ when it is not --- factors of $1.99$ and $2.11$ against the
$\mathbb{E}[U(0.25, 4)] = 2.125$ the exposures average to.

Two instances are declared. The per-pull-request one is an $8 \times 8$
lattice at $M = K = 2$ over $1{,}000$ positions; past it a triangular
$71 \times 71$ at $M = K = 10$ over $20{,}000$ positions, which is
$1.008 \times 10^{8}$ count pairs and past every oracle in this document ---
there the reference implementation is the referee for the accelerated one, and
the planted labels for the fit.

\paragraph{A continuous covariate.}
\label{par:coupled:covariate}
A third instance is the per-pull-request one above with a covariate on every
observation: each position, vertex and channel draws its own log-normal,
$c = \exp(\sigma z)$ with $\sigma = 0.5$ for the exposure and $40 \exp(\sigma
z)$ with $\sigma = 0.25$, rounded to a whole number, for the trial count. The
exposure then takes one value per observation, $64{,}000$ at the
per-pull-request size, so a tabulation of the density by count and distinct
covariate value is no smaller than the observations it tabulates. It is
declared at the fine resolution only, since a sum of negative binomials at
unequal exposures is not negative binomial and binning would misspecify it;
past it the $71 \times 71$ lattice at $M = K = 10$ is declared over $2{,}000$
positions under the same covariate.

\paragraph{Chains of unequal length.}
\label{par:coupled:ragged}
The chains of Equation~\ref{eq:joint} need not share a length. Class $m$
carries $S_m$ states and the joint is the same product with $S$ replaced by
$S_m$ per class, exactly as Equation~\ref{eq:hmm-ragged} states for a batch of
sequences: the classes are coupled through the labels, not through their
lengths. Padding to the longest chain and masking computes the same joint and
is an implementation of it, never the model --- and what it costs is the
padding fraction of the batch, not a property of either route.
...
posterior --- the failure
Section~\ref{sec:coupled:discussion} records. Joint marginalization of labels and chains, which is intractable and is
...
\subsection{The algorithms}
Section~\ref{sec:coupled:algorithms} states them: the block
ascent the structure dictates, with forward--backward per class as its
expectation step; one of the three label solvers of
Section~\ref{sec:potts:algorithms}, of which the cluster move in a field is
Algorithm~\ref{alg:wolff-field}; and the annealed, data-sampled start of
Algorithm~\ref{alg:graph-burnin}.

\begin{center}\rule{0.45\linewidth}{0.4pt}\end{center}
\addcontentsline{toc}{section}{\textit{Application: Error Correction}}
\noindent\textit{Application: Error Correction.} A code is a factor graph with a hard
constraint per check, and decoding is Equation~\ref{eq:sum-product} on it. The
two problems below are the classical and the iteratively decoded case. A
quantum code enters as the bicycle construction of
Section~\ref{sec:ldpc:bicycle}, a second construction of the same problem
rather than a problem of its own.
\begin{center}\rule{0.45\linewidth}{0.4pt}\end{center}
...
\begin{equation}
  ...
  = \begin{cases}
      (1 - 2 y_i) \ln \dfrac{1 - p}{p}
        & \text{binary symmetric, flip } p, \\[1.0ex]
      0 \text{ if erased, else } \pm\infty \text{ signed by } 1 - 2 y_i
        & \text{binary erasure, erasure } \epsilon, \\[0.6ex]
      2 y_i / \sigma^2
        & \text{binary-input Gaussian,}
    \end{cases}
  \label{eq:ldpc-llr}
\end{equation}
positive when the received symbol favours zero, the Gaussian channel sending
$y_i = (1 - 2 c_i) + \sigma z_i$ for standard normal $z_i$. The posterior over the code
is then
\begin{equation}
  p(c \mid y) \;\propto\; \indicator[\parity c = 0]\, \exp(-c \cdot L),
  \label{eq:ldpc-code}
\end{equation}
...
\subsection{The algebraic codes, and a decoder that does not iterate}
\label{sec:ldpc:algebraic}
Every code above is decoded by running Equation~\ref{eq:tanh-rule} to a fixed
point, and refereed by enumerating its $2^{k}$ words, which stops where the
exponent does. The codes here are decoded in closed form at every length, so
each is a referee whose cost does not grow with the block
\citep{blahut2003, mackay2003}.

A code of minimum distance $d$ corrects $t = \lfloor (d-1)/2 \rfloor$ errors,
since the Hamming spheres of that radius around its words are disjoint. A
\emph{perfect} code is one whose spheres also cover, leaving nothing over:
\begin{equation}
  2^{k} \sum_{i=0}^{t} \binom{n}{i} = 2^{n},
  \label{eq:sphere-packing}
\end{equation}
an equality over integers rather than a bound, and the sharpest structural
invariant available here --- a dropped row or a permuted column fails it at
once. The binary perfect codes are the odd repetition codes, the Hamming
codes $(2^{m}-1, 2^{m}-1-m)$ and the Golay code $(23, 12)$; the extended
Golay $(24, 12)$ is not perfect, and buys $d = 8$, which corrects three
errors and detects a fourth.

Reed--Solomon takes the other route. Over $\mathrm{GF}(2^{m})$ with
$n = 2^{m} - 1$, the generator polynomial's roots are
$\alpha^{1}, \dots, \alpha^{n-k}$, and the code meets the Singleton bound
\begin{equation}
  d \leq n - k + 1
  \label{eq:singleton}
\end{equation}
with equality, which no code above does: it is the first maximum-distance
separable code here. Its decoder is algebraic --- Berlekamp--Massey for the
error locator, a Chien search for the locator's roots, Forney for the
magnitudes \citep{blahut2003} --- and is \emph{bounded distance}: it returns
a word within $t$ symbols of what arrived, or refuses.

\paragraph{A bounded-distance decoder's error rate is an equality, not a
bound.} Past $t$ the transmitted word is farther than $t$ from what arrived,
so a decoder that only returns words within $t$ cannot return it. The block
error probability is therefore exactly the binomial tail
\begin{equation}
  P_{\mathrm{block}} = 1 - \sum_{i=0}^{t} \binom{n}{i} q^{i} (1-q)^{n-i},
  \label{eq:bounded-distance}
\end{equation}
where $q$ is the probability that one symbol is wrong: the crossover $p$ for
a binary code, and $1 - (1-p)^{m}$ for a code over $\mathrm{GF}(2^{m})$ whose
symbols are sent as $m$ bits. Equation~\ref{eq:bounded-distance} is also the
\emph{maximum-likelihood} rate for a perfect code, where every received word
has a unique nearest codeword. That is what makes these codes oracles rather
than examples, and it is what the implementations here are pinned against.

\subsection{Capacity, which is nobody's decoder}
\label{sec:ldpc:capacity}
Every rate above is what one decoder achieves. Capacity is what no decoder of
that rate achieves: a converse, not a construction \citep{cover2006}. For the
three channels of Section~\ref{sec:ldpc:model}, in bits per channel use,
\begin{align}
  C_{\mathrm{BEC}}(\epsilon) &= 1 - \epsilon, \label{eq:capacity-bec} \\
  C_{\mathrm{BSC}}(p) &= 1 - \mathrm{H}_{2}(p), \label{eq:capacity-bsc} \\
  C_{\mathrm{BIAWGN}}(\sigma) &= 1 - \mathbb{E}
    \log_{2}\!\left(1 + e^{-2y/\sigma^{2}}\right),
    \quad y \sim \mathcal{N}(1, \sigma^{2}),
  \label{eq:capacity-biawgn}
\end{align}
with $\mathrm{H}_{2}$ the binary entropy in bits. The first two are closed
forms; the third is an integral, taken by Gauss--Hermite quadrature, and
carries the published rate-$1/2$ limit $\sigma^{\star} = 0.9787$
\citep{richardson2008} as its pin --- 0.49999606 against one half, and
unmoved to $3 \times 10^{-10}$ by halving or doubling the node count.
Inverting each of the three for the noise at which capacity equals a rate is
a bisection, on the same monotonicity argument as
Appendix~\ref{app:density-evolution}'s threshold.

\paragraph{What the gap says, and what it does not.} A threshold and a
capacity are the same kind of statement --- both are about infinite length ---
and their ratio is the ensemble's. The $(3,6)$ ensemble's erasure threshold
$\epsilon^{\star} = 0.4294$ sits below $C_{\mathrm{BEC}}^{-1}(1/2) = 0.5$,
leaving $14.1\%$ of that channel unused; on the symmetric channel its
threshold $0.084$ against $p^{\star} = 0.1100$ is a factor of $1.31$. A
finite-length code's crossing of a chosen error rate is \emph{not} the same
kind of statement, and the distance from it to the boundary is part length
and part decoding: at a block error rate of $10^{-2}$, the repetition,
Reed--Solomon, Golay and Hamming pairs of
Section~\ref{sec:ldpc:algebraic} give up factors of $3.0$, $5.6$, $2.7$ and
$3.9$ against Equation~\ref{eq:capacity-bsc}, on blocks of 3 to 23 bits.
Lowering the rate does not buy the gap back: repetition has the mildest limit
of the four, $p^{\star} = 0.174$, and still gives up $3.0$.
...
$\psi_a(c_{\partial a}) = \indicator[\bigoplus_{i \in \partial a} c_i = 0]$
...
\begin{itemize}
  ...
  \item \emph{Equation~\ref{eq:sphere-packing}} referees the algebraic
    constructions as an equality over integers, and
    Equation~\ref{eq:bounded-distance} referees their decoders as a closed
    form no length outruns: the implementations are held to it by Monte
    Carlo at a stated draw count, agreeing within 1.7 standard errors on
    every pair measured. It establishes that each code is the one named and
    that its decoder achieves what bounded distance guarantees; it
    establishes nothing about what a better decoder would achieve on the
    same code, which for a perfect code is the same number and for
    Reed--Solomon is not.
  \item \emph{Capacity} referees every threshold and operating point from
    above, and is the only referee here that does so without running
    anything: Equation~\ref{eq:capacity-bsc} and its two siblings forbid a
    rate at a noise level, so a threshold that passed one would be a mistake
    in the threshold or in the capacity. It establishes that the numbers
    reported are not impossible, and nothing about any of them being right.
...
\subsection{The algorithms}
Section~\ref{sec:ldpc:algorithms} states them:
Equation~\ref{eq:sum-product} specialised to one number per message, run in the
log domain by Algorithm~\ref{alg:sum-product-decode}, with the min-sum
approximation of Equation~\ref{eq:min-sum} beside it.
...
\end{equation}
where the edge is the transition $s_t \to s_{t+1}$, $u$ and $p$ are the input
and parity bits labelling it, $L^{s}$ and $L^{p}$ are the channel ratios of
the systematic and parity bits, and $L^{a}$ is an a priori ratio on the
input, zero for a stand-alone decoding. The input bit needs no variable of its own: the two edges leaving
...
\subsection{The algorithms}
Section~\ref{sec:turbo:algorithms} states them: the BCJR
recursion of Equation~\ref{eq:bcjr} on each constituent trellis, which is
Equation~\ref{eq:forward} in the log domain, and the extrinsic exchange between
them, Algorithm~\ref{alg:turbo-decode}.

\section{Polar Codes}
\label{sec:polar}

\subsection{The model}
\label{sec:polar:model}
A polar code is the image of one fixed matrix under a choice of which inputs
carry information \citep{arikan2009}. With
$F = \begin{bmatrix} 1 & 0 \\ 1 & 1\end{bmatrix}$ and $N = 2^{n}$,
\begin{equation}
  G_N = F^{\otimes n}, \qquad c = u G_N \pmod 2,
  \label{eq:polar-transform}
\end{equation}
where $u$ carries the $k$ message bits on an \emph{information set}
$\mathcal{I}$ and zeros on its complement, the \emph{frozen set}. $F$ is its
own inverse over $\mathrm{GF}(2)$ and so is every Kronecker power of it, which
is how a decoder recovers $u$ from $c$ --- by applying $G_N$ a second time
rather than inverting it.

\paragraph{Which inputs carry information is the whole construction.}
Under a decoder that decides $u_i$ in order, knowing $u_0, \dots, u_{i-1}$,
input $i$ sees a \emph{synthetic} channel $W_i$: the same physical channel,
$N$ uses of it, and a different effective quality per input. The transform
moves quality between inputs and creates none,
\begin{equation}
  \sum_{i=0}^{N-1} I(W_i) = N \, I(W),
  \label{eq:polar-capacity}
\end{equation}
so a rule that freezes the worst $N - k$ inputs spends the channel where it is
worth spending. On the erasure channel the Bhattacharyya parameter obeys
Ar\i kan's exact recursion, one kernel application at a time,
\begin{equation}
  Z(W^{-}) = 2 Z(W) - Z(W)^{2}, \qquad Z(W^{+}) = Z(W)^{2},
  \label{eq:polar-bec}
\end{equation}
with $I = 1 - Z$ there exactly, which makes Equation~\ref{eq:polar-capacity}
checkable rather than cited. Off the erasure channel no closed recursion
exists and the standard Gaussian approximation is used instead: one mean per
input, $Z = e^{-m/4}$. It is an approximation, and only the \emph{order} it
induces matters, since the frozen set depends on nothing else.

\paragraph{Reed--Muller is the same transform under a different rule.}
$\mathrm{RM}(r, n)$ freezes by row weight --- keep the rows of $G_N$ of weight
at least $2^{n-r}$ --- where a polar code freezes by reliability. At
$N = 8$, $k = 4$ the two rules pick the same information set, $\{3,5,6,7\}$,
which is the extended Hamming $(8,4)$ code ---
the same object Section~\ref{sec:ldpc}'s algebraic constructions reach by a
different route; at $N = 16$ they part. One transform, two
rules, and no second implementation of either.

\paragraph{Successive cancellation is not maximum likelihood, and that is the
point.} It decides each input in order and cannot revisit one, so a single
early mistake propagates. The synthetic channels of
Equation~\ref{eq:polar-capacity} are \emph{defined} by that decoder, which is
what makes the construction coherent and also what caps it: the decoder is
weaker than the code. A list decoder carries $L$ candidate prefixes and prunes
by an exact path metric \citep{talvardy2015}, and at $L = 2^{k}$ it prunes
nothing and is therefore maximum likelihood.

\paragraph{Supported sizes.}
\label{par:polar:sizes}
Block lengths $N = 16$, $256$ and $2{,}048$ at rate $1/2$ carry the refereed
claims. Three ceilings separate what is refereed by what.
Equation~\ref{eq:polar-bec} is closed form at \emph{every} length, so the
construction is exact at all three. Enumerating the $2^{k}$ codewords is
refused past 200,000 configurations, so the maximum-likelihood referee ---
Section~\ref{sec:ldpc}'s enumeration, reached by taking the dual of
Equation~\ref{eq:polar-transform}'s information rows --- covers $N = 16$ at
rate $1/2$ and no more, and the exhaustive list $L = 2^{k}$ with it. A dense
$G_N$ is refused past $N = 4{,}096$; the decoders need none, being recursive
over $\log_2 N$ stages. $N = 8$ was the first candidate for the enumerable
size and is too small to measure the gap this section is about: at $k = 4$ it
is the extended Hamming code, where successive cancellation already agrees
with enumeration on most draws.

\subsection{As a factor graph}
The graph is Equation~\ref{eq:polar-transform} written out: $N(n+1)$ binary
variables in $n+1$ columns, and one parity factor per butterfly joining a pair
in one column to a pair in the next (Figure~\ref{fig:polar:sketch}). It is
loopy for $n \ge 2$, so Equation~\ref{eq:sum-product} on it is the Bethe
approximation and not exact --- and successive cancellation is not that
approximation either. It is a \emph{schedule}: decide $u_0$ from a
leaf-to-root pass, hold it fixed, decide $u_1$, and so on, each pass exact
given the decisions before it. The two operations the pass needs are the
check-node rule of Equation~\ref{eq:tanh-rule} on a pair, and a sign
correction by the decision already made,
\begin{equation}
  f(a, b) = 2 \operatorname{artanh}\!\big(\tanh(a/2)\tanh(b/2)\big),
  \qquad g(a, b, \hat u) = b + (1 - 2\hat u)\, a.
  \label{eq:polar-fg}
\end{equation}
...
\subsection{Validation}
\label{sec:polar:validation}
\begin{itemize}
  \item \emph{Equation~\ref{eq:polar-capacity}} referees the construction at
    every length, as an equality to $10^{-12}$ at $n = 3, 5, 8, 11$: the
    recursion is closed form, so there is nothing to sample. It establishes
    that the transform conserves what it redistributes and nothing about any
    decoder.
  \item \emph{Enumeration of the $2^{k}$ codewords} referees both decoders at
    $N = 16$: the maximum-likelihood codeword is what successive cancellation
    is measured against, and the gap is \emph{reported} rather than asserted
    away --- at $\sigma = 1$ over 200 shared draws, 98 block errors for
    successive cancellation, 64 for a list of 4, and 62 for maximum
    likelihood. It establishes the size of what the decoder gives up, and it
    reaches no further than $k \le 17$.
  \item \emph{A naive recursive reference} referees the list implementation's
    layout: same arithmetic, same order, one Python call per node and no path
    axis, pinned \emph{bitwise} over 900 draws across the three channels. It
    establishes that the path-array form computes the same decisions; a
    tolerance there would hide the one bug it exists to catch, a gather that
    reorders the traversal.
  \item \emph{The two ends of the list} referee the fork-and-prune machinery:
    $L = 1$ must reproduce plain successive cancellation bit for bit, and
    $L = 2^{k}$ prunes nothing and must therefore equal the enumerated
    maximum-likelihood answer. Each establishes one end and neither the
    middle, which is what the monotonicity below covers.
  \item \emph{Monotonicity in $L$ over shared seeds} referees the metric's
    ordering: a larger list explores a superset only if the metric ranks
    candidates consistently, which is a property of this implementation and
    not of the theory. It establishes an ordering and no rate.
  \item \emph{A measured ordering of the synthetic channels} referees the
    Gaussian approximation, which has no closed form to be held to: per-input
    error rates of successive cancellation on the rate-one code agree with it
    at Spearman $0.897$, against $0.903$ for the exact erasure recursion
    measured the same way, and the best and worst inputs agree exactly. It
    establishes that the approximation orders the inputs as a decoder does; it
    is not the genie-aided ordering the synthetic channels are defined by, so
    the agreement is high rather than perfect and is reported as such.
\end{itemize}

\subsection{Extensions, and what is not attempted}
\label{sec:polar:extensions}
\paragraph{The check, and what it would take to go further.}
A cyclic redundancy check appended to the message and carried through the list
turns the metric's best survivor into the best \emph{passing} survivor, which
is what makes list decoding competitive in practice \citep{tal2015}; the check
is polynomial division over $\mathrm{GF}(2)$, which the elementary codes carry.
The combination is built: at $N = 16$ with a three-bit check and the exhaustive
list it returns the maximum-likelihood codeword of the outer code, enumerated,
and it falls back to the metric's best survivor when no survivor passes. What
it buys at $N = 256$ is measured rather than assumed: lists of eight and of
thirty-two fail by pruning the sent path, never by ranking it second, so the
check rescues nothing there and the gain waits on a list the pruning does not
reach first.
Beyond it: a systematic encoder, so the message is readable off the codeword
without inverting anything; puncturing and shortening, which is how a length
that is not a power of two is reached; and the Gaussian approximation against
Monte Carlo at $N \ge 256$, where a disagreement would change the frozen set
and the fixture would have to carry the measured one --- a decision, since it
doubles the construction cost, and not a default.

\paragraph{What is deliberately not attempted.}
The polarisation theorem, which is asymptotic: what is reported instead is the
fraction of inputs that are neither good nor bad at each length, falling from
$0.75$ at $N = 8$ to $0.063$ at $N = 16{,}384$, and the trend is asserted
monotone rather than the limit claimed. And a min-sum $f$: the hardware
approximation of Equation~\ref{eq:polar-fg}, which would need the exact form
kept beside it as its oracle, and nothing here asks for it yet.

\subsection{The algorithms}
Section~\ref{sec:polar:algorithms} states them: the recursive pass of
Equation~\ref{eq:polar-fg} per input, Algorithm~\ref{alg:successive-cancellation},
and the list that carries $L$ prefixes through it,
Algorithm~\ref{alg:polar-list}.
...
Table~\ref{tab:ladder} is the third reading, method against method rather than
method against problem: per problem the algorithms ordered from exact to
efficient, each rung beside the rung it is pinned against and the test pinning
the pair, and a test cell reading \emph{ticket} is an issue rather than a
silence.

\paragraph{A blank is not built, not inapplicable.}
The tables are generated from the catalogue and the suite, both of which are
records of what the tree holds, so an empty cell says that this problem
carries no method of that family \emph{here} and says nothing about whether
one could exist. The two are different claims and only the first is generated:
not built is a gap with a ticket behind it, and not applicable is a statement
about the mathematics that has to be argued rather than read off a table. Four
pairings are the second, and they are the only four. A surrogate for the
continuous test functions stands in for an evaluation that is already three
arithmetic operations, so there is nothing for it to save
(Section~\ref{sec:surrogates}). A cut-based ground state needs a submodular
energy, which the frustrated lattice's mixed-sign couplings do not give
(Section~\ref{sec:potts-cuts}). A single-spin cluster move built on the
Fortuin--Kasteleyn bond needs a ferromagnetic coupling, so Swendsen--Wang and
Wolff are defined on Section~\ref{sec:potts}'s lattice and not on
Section~\ref{sec:frustrated}'s. And a branch-length optimizer has nothing to
optimize under parsimony, whose score is a count and carries no continuous
parameter (Section~\ref{sec:parsimony}). Every other blank is work not done.

Table~\ref{tab:declared-instances} then says what each problem is declared
\emph{at}. Every claim in this document is made on an instance, and an
instance is a size and a set of parameters rather than a name: the tiers of one
problem differ in more than one quantity at once --- extent and state count,
taxa and sites, chain length and transition rate --- so a reader who knows only
that a result was taken at the smaller of two instances knows almost nothing.
The table states each in the words its own declaration uses, and it is
generated, so an instance cannot be described here and declared otherwise.

\input{generated/problems_tables}

\part{The Algorithms}

Every algorithm this document uses is stated once. Where one serves several
problems it is stated in general terms in Appendix~\ref{app:algorithms} and
specialised here, problem by problem, in the order the problems were stated.
Where one serves a single problem it is stated in that problem's section
below.
...
Every search and every fit below starts somewhere, and where it starts is a
declared part of the method rather than a detail of a run. Four kinds are
carried, and they differ in what they read: a start that reads nothing, a
start that reads the data, a start that reads a moment of the data and inverts
it, and a start that reads the objective through a sampler. The kinds are not
interchangeable, because what each can be held to is different.

\paragraph{Model-free restarts.}
A draw from a declared generator around a declared point, repeated,
Algorithm~\ref{alg:random-restart}. It reads nothing and assumes nothing, so
it applies to every problem here and is the baseline every other start is
measured against at an equal budget of objective evaluations. On a multimodal
surface it is also the only start with a \emph{rate} attached --- the fraction
of starts from which the fit reaches the global optimum, which
Section~\ref{sec:testfunctions:algorithms} measures rather than asserts. Its
cost is the restart count, and that count is part of the budget the comparison
charges.

\paragraph{Data-sampled seeds.}
A start drawn from the observations, proportionally to how badly the current
parameters explain them: $k$-means$++$ on a mixture,
Algorithm~\ref{alg:kmeanspp}, and its family-aware form on an emission
mixture, where the family's own negative log-density replaces the squared
distance. What it inherits from the published bound is exactly the case the
bound was proved for: the $8(\ln K + 2)$ expected-cost guarantee is a statement
about squared Euclidean distance \citep{arthur2007}, so a seeding under another
divergence is measured against a uniform start over seeds and reported whether
it wins or not \citep{banerjee2005}.

\paragraph{Moment and spectral starts.}
A start that inverts an observable moment to the parameters, with a consistency
guarantee and no surface to climb: the pairwise distances and the agglomeration
over them for a tree (Algorithm~\ref{alg:neighbor-joining}), the exact
two-state spectrum of Equation~\ref{eq:hadamard}
(Algorithm~\ref{alg:hadamard-start}), and the window moments of a chain
\citep{hsu2012, mossel2006}. This is the only kind carrying an argument rather
than a measurement --- Atteson's radius says when the agglomeration recovers
the topology, and the conjugation is exact on an exact spectrum --- and the
only kind that stops existing above a size, which is why
Section~\ref{sec:phylo:algorithms} states the taxon count at which it does.

\paragraph{Annealed and sampled starts.}
A start produced by running the sampler itself at a temperature, and lowering
it: the data-sampled burn-in of Algorithm~\ref{alg:graph-burnin}, which anneals
in $\beta$ from zero so the labels are nearly free while the emission
parameters are fitted, and simulated annealing read as an initializer for a
descent that polishes what it returns. A schedule is one object with both
endpoints reached exactly at the declared step count
(Appendix~\ref{app:tempering}), so an annealed start is reproducible in the
sense the others are: one seed, one schedule, one result. Whether annealing
beats restarts of descent at an equal budget of sweeps is a property of the
instance and is measured on Section~\ref{sec:frustrated}'s, never asserted in
general.

A fifth kind separates two readings of one run. A tempered ensemble
estimates a \emph{distribution}, so its cold replica's weights,
Equation~\ref{eq:tempered-weight}, are an answer and not a starting point;
the lowest-valued point the ensemble visits is a start, and
the tempering start hands over that point and nothing of the weights.
Reading the weights as a start is the standing error about
Appendix~\ref{app:tempering}.

\section{Relaxing a Discrete Decision}
...
distribution $q = \prod_i q_i$ --- Equation~\ref{eq:elbo} under a factorized
$q$ --- tightest at its fixed point:
\begin{equation}
...
\end{equation}
Both of those cost one exact tree pass per edge, which a lattice of thousands
of sites does not admit. Two looser ones cost $O(N + |E|)$ and so reach any
size. Write
\begin{equation}
  E_{\mathrm{dec}} \;=\; -\sum_i \max_\sigma h_i(\sigma)
      \;-\; \sum_{\langle i,j \rangle} \max(J_{ij}, 0)
  \;\le\; E_{\min}
  \;\le\; \energy(\sigma) \quad \text{for every } \sigma ,
  \label{eq:decoupled-energy-bound}
\end{equation}
each term of the energy being at least its own minimum, and any
configuration's energy being at least the least. The same quantity carries a
bracket on the partition function, the left end holding where every coupling
is non-negative:
\begin{equation}
  \sum_i \log \sum_\sigma e^{h_i(\sigma)} \;\le\; \log Z
    \;\le\; N \log k - E_{\mathrm{dec}} .
  \label{eq:saturated-bound}
\end{equation}

\section{Learned Proposals for a Discrete Search}
\label{sec:policy-gradient}

The structural searches above choose a move from a neighbourhood by a rule
fixed in advance --- take the best, or take a random one. A learned proposal
replaces the rule by a policy trained on the searches already run, and enters
this document in one position only: it chooses \emph{what to evaluate}, and the
thing evaluated is still the objective the problem defines. That is what keeps
it refereeable. A proposal that fails is slower; an answer that fails is wrong.

The search is a Markov decision process \citep{suttonbarto2018}: the state is
the current structure with its fitted parameters, an action is a move from the
neighbourhood $\neighbourhood(\cdot)$, and the reward is the improvement in the
objective. An episode ends on a step budget or when no move improves the
score --- a property of the state, and the stopping rule the greedy baseline
obeys, which is what makes the two comparable. Because the reward is a
difference, the undiscounted return telescopes to the improvement between the
first and last state whatever path joined them,
Equation~\ref{eq:return}: a discount would prefer improvement found early over
the same improvement found late, and a truncation landing between a sacrifice
and its payoff teaches the opposite of the truth, so a truncated episode is
recorded as one. Appendix~\ref{app:rl} states the algorithms that train such a
policy, from dynamic programming to the actor--critic, and which of them a
state space of this shape admits.

Four things about \emph{this} decision process are application-specific, and
each is a place a measurement goes wrong quietly.
...
It is trained by the score-function estimator of
Equation~\ref{eq:reinforce} with a baseline, and the comparison against the
...
\section{Algorithms: Continuous Test Functions}
\label{sec:testfunctions:algorithms}
...
\section{Algorithms: The Gaussian Mixture}
\label{sec:mixture:algorithms}

\subsection{Expectation--maximization and the gradient fit}
...
The alternation may be tempered first \citep{ueda1998}: at temperature $T$
the E step takes $r_{ik} \propto (w_k\,p_k(y_i))^{1/T}$, and the M step is
unchanged. At fixed $T$ the pair ascends the free energy
\begin{equation}
  F_T(\theta) = \sum_i T \log \sum_k \big(w_k\,p_k(y_i)\big)^{1/T}
  = \max_{q}\, \sum_i \Big[\sum_k q_{ik} \log\big(w_k\,p_k(y_i)\big) + T\,H(q_i)\Big],
  \label{eq:da-em}
\end{equation}
the maximum attained at the tempered responsibilities, which is the monotone
claim a fixed temperature is held to; at $T = 1$ it is the log-likelihood and
the step is expectation--maximization's own, and as $T \to \infty$ every
responsibility is $1/K$ and one M step fits each component to the pooled data.
A schedule falling to one ends in the plain alternation. Whether it reaches a
higher optimum than the plain alternation from the same start is measured,
not assumed: on the three-component count-pair instance of
Section~\ref{sec:mixture} both reach one maximum from every start tried,
so the route is declined and conserved beside the package rather than in it.

\subsection{The $k$-means$++$ start}
...
A converged alternation can hold one component across two generating ones and
two on a third: moving a component between them lowers the likelihood on the
way, so no iterate takes the move. Split-and-merge takes it in one step
\citep{ueda2000}: merge a pair ranked by the cosine of their responsibility
columns, split a component ranked by the local Kullback--Leibler divergence
between the data it owns, $f_k(y_i) = r_{ik} / \sum_{i'} r_{i'k}$, and its own
density, run partial expectation--maximization on the three with their total
responsibility held fixed, then the full alternation, and keep the result only
if the likelihood rose. The likelihood therefore never falls. Here a move is
a re-weighting of the responsibility columns, the merged one their sum and the
split one divided by a two-component seeding at two of its observations, so
the family's own M step realizes it. Two identical components receive
identical responsibilities and stay identical under the alternation; that
fixed point is the one the test plants, and the first move repairs it.
...
\subsection{Relabelling the draws of a mixture posterior}\label{sec:relabel}
Under a prior exchangeable in the components the posterior is invariant under
their permutation, so a chain that mixes visits all $K!$ namings of every mode,
and a summary over draws averages different components together. A
relabelling returns one permutation $\nu_t$ per draw $t$, in the order
convention: label $k$ of the relabelled draw is component $\nu_t(k)$ of the
drawn one, so parameters are relabelled as $\theta_{t,\nu_t(k)}$ and an
allocation as $\nu_t^{-1}(z_{ti})$. With allocation probabilities
$p_{tik} = P(z_i = k \mid x, \theta_t)$, allocations $z_{ti}$ and parameters
$\theta_{tk} \in \mathbb{R}^J$:
\begin{itemize}
\item \textbf{STEPHENS} \citep{stephens2000}, the default: minimize
  $\sum_t \sum_i \mathrm{KL}(p_{ti,\nu_t(\cdot)} \,\|\, q_i)$ over the
  permutations and $q$ by alternation. The mean of the relabelled
  probabilities minimizes it over $q$, and each $\nu_t$ is a linear assignment
  on $c_{tkl} = \sum_i p_{til}(\log p_{til} - \log q_{ik})$, so the objective
  never rises. Probabilities are clamped to $[10^{-6}, 1 - 10^{-6}]$ first.
\item \textbf{ECR} \citep{papastamoulis2010}: maximize the agreement
  $\sum_i [\nu_t^{-1}(z_{ti}) = z^\ast_i]$ with a pivot allocation $z^\ast$, a
  linear assignment on the $K \times K$ table of counts.
  \textbf{ECR-ITERATIVE-1} re-estimates $z^\ast$ as the mode of the relabelled
  allocations, a coordinate ascent on the summed agreement;
  \textbf{ECR-ITERATIVE-2} as the argmax of the mean relabelled probabilities.
\item \textbf{PRA} \citep{marin2005}: maximize
  $\sum_k \langle \theta^\ast_k, \theta_{t,\nu_t(k)} \rangle$ against a pivot
  draw $\theta^\ast$; separable in $k$, so a linear assignment where the
  published form enumerates $K!$.
\item \textbf{SJW} \citep{sperrin2010}: EM over the latent
  permutation. The E step weighs every $\nu$ by
  $\exp \sum_i S_{i,\nu^{-1}(z_{ti})}$, with $S_{ik}$ the complete-data
  log-likelihood of observation $i$ in component $k$ at the estimate
  $\hat\theta$; the M step sets $\hat\theta$ to the weighted mean of the
  relabelled draws. It enumerates $K!$ permutations per draw.
\item \textbf{AIC}: order the components by one parameter; the identifiability
  constraint the others are read against.
\end{itemize}
The published implementation \citep{papastamoulis2016} applies the order itself, not
its inverse, to the allocations inside ECR-ITERATIVE-1 and SJW; the two agree
only where a permutation is its own inverse, and the package keeps one
convention. The referees are brute force over $K!$ for every assignment step,
the monotonicity of the two alternations, and draws renamed by the random
permutation sampler \citep{fruhwirth2001}, whose permutations are known;
the worked example reads each method against the generating components and
parameters. The draws come from the conjugate Gibbs sampler
\citep{diebolt1994} and from the Hamiltonian and Langevin chains of
Section~\ref{sec:hmc}.

\section{Algorithms: The Count-Emission Mixture}
\label{sec:emissionmixture:algorithms}

\subsection{Expectation--maximization and a seeded start}
Expectation--maximization, Algorithm~\ref{alg:baum-welch} with the chain
...
with the family's own Bregman divergence in place of the squared distance of
...
The seeding is Algorithm~\ref{alg:kmeanspp} with the family's Bregman
divergence in place of the squared distance, which is not a refinement but
a necessity --- a squared distance between a depth and an allele count
compares two quantities that share no units. The divergence and the negative
log-density are different rules, on the terms Section~\ref{sec:mixture}
states, and the beta-binomial is not an exponential family in its success
count, so what stands in for the divergence there is the unit deviance it
generalizes to: the log-density gap to the best rate the family admits at the
observation, which is non-negative by construction.
...
\section{Algorithms: Hidden Markov Models}
\label{sec:hmm:algorithms}
...
\section{Algorithms: Phylogenetic Inference}
\label{sec:phylo:algorithms}

\subsection{The simulation algorithm}
For a given rate matrix $\Q$ and branch lengths $\branch$, the generative
direction of Equation~\ref{eq:site-independence} is a walk in \emph{pre-order}
--- a traversal visiting each node before any of its children, so that a
node's parent always carries a state by the time the node is reached. The root
is drawn from $\bpi$ and every other node from the row of $\Pmat(\tau_v)$ its
parent's state indexes. One site is Algorithm~\ref{alg:simulate}; $L$ sites are
$L$ independent runs, and the empirical substitution frequencies along any
branch converge to Equation~\ref{eq:jc}
(Section~\ref{sec:phylo:validation}).
...
\begin{equation}
  ...
  \label{eq:pruning}
\end{equation}
with $L_u(i) = \indicator[\,\text{$u$ observed in state $i$}\,]$ at a leaf, and
...
\subsection{The discrete search}
\label{sec:phylo:search}
...
Figure~\ref{fig:nnispr} runs both over the same start.
...
\subsection{Spectral learning, and what it inverts}
\label{sec:spectral-start}
Three starts share one idea and are stated separately below, because what each
assumes and what each guarantees are different. Spectral learning of a hidden
Markov model \citep{hsu2012} is a method of moments: the observable moments of
short windows are inverted to the parameters, with a consistency guarantee and
no likelihood surface to climb. \citet{mossel2006} carry the argument to a
phylogeny --- the same inversion of pairwise moments learns a nonsingular
tree --- where it becomes a distance per pair of taxa
(Section~\ref{sec:phylo:distances}), a tree from those distances
(Section~\ref{sec:phylo:nj}), and, on two states, an exact conjugation
inverting the whole pattern spectrum at once
(Section~\ref{sec:phylo:hadamard}). All three are a \emph{start} and not an
estimate: the fit and the search begin from them, and every reported number is
the optimum they reach.

\subsection{Distances, and the moment each inverts}
\label{sec:phylo:distances}
...
\subsection{Neighbor joining, and the four-point condition}
\label{sec:phylo:nj}
...
\emph{additive} matrix --- one whose entries are the path lengths of some
tree --- it returns that tree with every branch length exact.

Additivity is a property of four taxa at a time. A quartet has three
resolutions and a tree realizes exactly one: its two interior nodes are joined
by a path of length $m \ge 0$ that the path between differently paired taxa
crosses and the path between paired taxa misses. Summing the path lengths in
each of the three pairings therefore gives
\begin{equation}
  d_{ij} + d_{kl} \;\le\; d_{ik} + d_{jl} \;=\; d_{il} + d_{jk}
  \;=\; d_{ij} + d_{kl} + 2m,
  \label{eq:four-point}
\end{equation}
for the resolution $ij \mid kl$: each of the two larger sums counts the
interior path twice and they are therefore equal, and each exceeds the
smallest by exactly $2m$ (Figure~\ref{fig:fourpoint}).
Equation~\ref{eq:four-point} holding on every quartet is the \emph{four-point
condition}, and it is necessary and sufficient for a dissimilarity to be the
path metric of a tree \citep{felsenstein2004}. Two consequences are used
below. The \emph{smallest} of the three sums names the quartet's resolution,
which is what makes a topology an $\arg\min$ of a linear function of the
distances and is where Section~\ref{sec:tropical} starts. And the gap between
the smallest and the other two is twice the interior branch length, so a
quartet whose interior edge is short is resolved by an estimated matrix only
if the matrix's error is smaller than that edge.

That last is Atteson's radius in one quartet. On an estimated matrix
\citet{atteson1999} gives the guarantee in general: the topology is the true
one whenever every entry's error is below half the shortest branch, and
Equation~\ref{eq:jc-distance}'s variance is what lets a run state whether it
sits inside that radius.

\input{fourpoint_figure}
...
\subsection{The Hadamard spectrum}
\label{sec:phylo:hadamard}
...
the $n-3$ largest mutually compatible weights of an estimated $q$.

What the two-state transform applies to is a \emph{lumping} of the alphabet,
and which lumpings exist is a property of the substitution model rather than
of the transform. A four-state process is two-state under a partition of
$\states$ into two blocks whenever the total rate from a state into the
opposite block is the same for both states of its own block; the lumped
process is then Markov at that rate \citep{felsenstein2004}. Under
Jukes--Cantor every rate is equal, so all three pairings lump and each gives
the same two-state chain, at $\tfrac{2}{3}$ of every branch length --- the
reduction the implementation uses. Under a model separating transitions from
transversions the condition selects one pairing: the purine--pyrimidine
partition $\{\mathrm{A},\mathrm{G}\} \mid \{\mathrm{C},\mathrm{T}\}$, where a
state leaves its block at the transversion rate whichever state it is, so the
lumped chain is the transversion process and the transition rate drops out of
it. The other two pairings mix the two rates and are not Markov after lumping.
The two-state solution is therefore a known method and not an approximation:
it estimates the transversion process exactly and says nothing about the rate
it lumps away, which is one reason it is a start. Refining that answer back to
four states is a second fit rather than part of the transform; the four-state
conjugation over the Klein four-group would do it in one step and is not
implemented.
...
\subsection{The tropical relaxation}
\label{sec:tropical}
...
\section{Algorithms: Large Parsimony}
\label{sec:parsimony:algorithms}

\subsection{Hill climbing on the score}
...
\section{Algorithms: Potts Models in an External Field}
\label{sec:potts:algorithms}
...
\paragraph{What a region sees that an edge cannot.}
Equation~\ref{eq:bethe} is exact when the graph factorizes over its edges, and
a lattice's shortest cycle has four of them: no edge term can hold the
correlation that runs around a unit cell. Taking the cell itself as the unit of
the approximation is the Kikuchi free energy, Appendix~\ref{app:kikuchi}, whose
stationary points parent-to-child message passing finds
(Appendix~\ref{app:gbp}). It reduces to Equation~\ref{eq:bethe} when the
regions are the edges and the sites, so it is one expression and not a second
approximation, and on the $3 \times 3$ lattice it is three to four orders of
magnitude nearer the enumerated $\log Z$ for $3.4\times$ the wall clock. Two
statements bound what that buys. The advantage \emph{decays with coupling} ---
$23{,}378\times$ at $J = 0.3$ against $130\times$ at $J = 1.2$ --- since deep
in the ordered phase both approximations concentrate on the same configuration;
and at the $6 \times 4$ strip the plaquette messages settle at $J = 0.25$ and
\emph{do not settle at all} at $J = 0.5$, at any damping measured, so the
binding constraint at size is convergence rather than accuracy. Neither is a
bound: like Bethe, the region-based free energy is a stationary point of a
non-convex functional and may fall either side of $\log Z$.

\paragraph{Regime.}
Exact on a tree, in two passes. On a loopy graph an iterative schedule is run
with damping to a tolerance and returns the Bethe approximation; the tree
schedule is refused rather than run, a number that looks exact and is not being
the worst output an evaluator can produce. Three iterative orders share the
update and the fixed points and differ in the path: flooding sends every
message from the previous sweep (Jacobi), the sequential order sends each
factor's messages from the newest values (Gauss--Seidel), and the residual
order sends next the factor whose inputs last changed most
\citep{elidan2006}, a heap over the factors that spends its sends where the
graph is still moving and converges where flooding oscillates. The sweep count
of each is a measurement, not a guarantee.
...
The referee the ladder of Section~\ref{sec:ladder} puts above enumeration is
the transfer matrix, and it is one identity. Cut an $W \times L$ lattice into
$L$ columns of $W$ sites and write a column's configuration as a single
variable $c_\ell \in \states^{W}$, of which there are $q^{W}$. Every
interaction is then either inside a column or between adjacent ones, so
\begin{equation}
  Z = \sum_{c_1, \dots, c_L} \prod_{\ell} \nu(c_\ell)
      \prod_{\ell < L} \transfer(c_\ell, c_{\ell+1}),
  \qquad
  \transfer(c, c') = \exp\Big[ \sum_{w} J\,\delta(c_w, c'_w) \Big],
  \label{eq:transfer}
\end{equation}
with $\nu$ carrying a column's own field and vertical couplings. The sum is
a matrix product, $Z = \nu_1^{\top} \transfer^{L-1} \nu_L$ under open
boundaries and $\operatorname{tr}(\transfer^{L})$ under periodic ones, where
the translation invariance folds $\nu$ into $\transfer$. Either way it costs
$O(L q^{2W})$: exponential in the width and
\emph{linear} in the length. That asymmetry is the whole reason the method
earns a rung.

The length is linear only if the product is taken in the order it is written.
Where $\transfer$ is the same at every $\ell$ the product is a \emph{power},
and a power reassociates: repeated squaring reaches $\transfer^{L-1}$ in
$\lfloor \log_2 (L-1) \rfloor$ squarings of $O(q^{3W})$ each and one
vector-matrix product per set bit of $L-1$, for $O(q^{3W} \log L)$ in place of
$O(L q^{2W})$. The cube in $q^{W}$ is what buys the logarithm in $L$, so the
reassociation pays where the width is small and the length is not, and loses
the other way; which of the two costs less is a statement about $q^{W}$ and
$L$ together and not about the length alone. Enumeration over $q^{WL}$ configurations dies in both
directions at once, while the transfer matrix dies only in $W$, so it supplies
an exact normalizer on a strip of any length at a width that fits --- which is
what refereeing belief propagation on a loopy graph requires, since the
quantity to compare against has to be exact and cannot be enumerated.

The chain is the case $W = 1$, and there Equation~\ref{eq:transfer} is the
forward recursion of Equation~\ref{eq:forward} with the emission folded into
$\nu$: a hidden Markov model's likelihood and a one-dimensional Potts
normalizer are one matrix product under two names
(Section~\ref{sec:hmm:algorithms}). Replacing the sum by a maximum gives the
same product in the max-plus semiring, which is Viterbi on the chain and the
ground state of the strip. Two properties follow and both are asserted: the
largest eigenvalue of $\transfer$ gives the free energy per column in the
limit of many columns, $-\lim_{L} (\log Z)/L = -\log \lambda_{\max}$, and
$\transfer$ is symmetric with strictly positive entries, so
Perron--Frobenius makes $\lambda_{\max}$ simple and its eigenvector
positive.

\paragraph{What pins it.}
Against enumeration wherever both run --- $q^{WL}$ against $O(L q^{2W})$, so
that is a small lattice --- and, past it, against itself contracted the other
way: a lattice's normalizer is the same number whether the columns or the rows
are the transfer direction, and the two contractions share no loop order. The
eigenvalue statement is checked against $(\log Z)/L$ at growing $L$.

\subsection{Ground states as cuts}
...
\paragraph{A planted field, and what a local minimum must recover.}
\label{par:alpha-expansion:tiling}
A field linear in the state, $h_i(a) = \alpha_a c_i$, has its optimum on the
two extreme states, so a two-state cut finds it exactly and the instance does
not exercise the $q > 2$ case. A \emph{tiling} field does: partition the sites
into connected tiles $T_1, \dots, T_k$, each site in the tile of the nearest of
$k$ drawn centres in graph distance, and let tile $t$ reward one state $a_t$,
\begin{equation}
  h_i(a) = s_t\,\delta(a, a_t), \qquad i \in T_t .
  \label{eq:tiling-field}
\end{equation}
Let $b_t = \max_{i \in T_t} \sum_{j \sim i,\, j \notin T_t} J_{ij}$, the most
coupling one site of $T_t$ has to sites outside it. If $s_t > b_t$, every
labelling no expansion lowers carries $a_t$ on all of $T_t$: for $U$ the sites
of $T_t$ not labelled $a_t$, expanding $a_t$ onto $U$ gains $s_t$ per site,
keeps or gains every bond inside $T_t$, and loses at most $b_t$ per site on the
bonds leaving it, a change of at most $|U|\,(b_t - s_t) < 0$ unless $U$ is
empty. The same move taken one label of $U$ at a time is a swap move, and the
ground state is such a labelling, so the bound holds for both and for the
optimum. At $s_t \le b_t$ it says nothing, and what a solver recovers there is
measured.
...
is measured, since the bound is a theorem and the ratio is a number. On a
tiling field enumeration referees the recovery bound at twelve sites, and the
planted states referee it at 5,041, where no optimum is known.

\paragraph{Fusion.}
An expansion move chooses, per site, between the current label and
$\alpha$. The fusion move \citep{lempitsky2010} chooses between two whole
labellings $a$ and $b$: a site where they differ takes $y_i \in \{0, 1\}$,
keeping $a_i$ or taking $b_i$, and the energy over $y$ is binary with pairwise
costs $\theta_{ij}(y_i, y_j) = -J_{ij}\,[\ell_i(y_i) = \ell_j(y_j)]$. A pair is
submodular when $\theta_{ij}(0,0) + \theta_{ij}(1,1) \le \theta_{ij}(0,1) +
\theta_{ij}(1,0)$, which every pair of an expansion is, since $b$ is the
constant $\alpha$; two arbitrary labellings can break it. The roof dual
\citep{kolmogorov2007} minimizes such an energy by one cut over two nodes per
variable, and labels a subset of the sites; its labelled part is an autarky,
so written over the better of $a$ and $b$ it gives
\begin{equation}
  E(\mathrm{fuse}(a, b)) \le \min\{E(a), E(b)\}.
  \label{eq:fusion}
\end{equation}
Where every site is labelled, the result is the binary problem's optimum.
Enumeration of the $2^n$ choices referees both claims on small graphs, and
the expansion move referees the constant case.

\subsection{The local-polytope bound}
\label{sec:potts-lp-bound}
A labelling's energy bounds the optimum from above; a bound from below needs a
relaxation. Replace the labelling by site marginals $\mu_i(a)$ and edge
marginals $\mu_{ij}(a, b)$, each non-negative, each site's summing to one and
each edge's summing to its sites' along either index. That set is the
\emph{local polytope}, and it contains every labelling as a vertex, so
\begin{equation}
  \min_{\mu} \; \sum_i \sum_a \theta_i(a)\,\mu_i(a)
    + \sum_{(ij)} \sum_{a,b} \theta_{ij}(a,b)\,\mu_{ij}(a,b)
  \;\le\; \min_{\sigma} \energy(\sigma),
  \qquad
  \theta_i(a) = -h_i(a),\;\; \theta_{ij}(a,b) = -J_{ij}\,\delta(a,b),
  \label{eq:local-polytope}
\end{equation}
with equality on a tree, where the local polytope is the marginal polytope
\citep{wainwright2005map}. The dual of Equation~\ref{eq:local-polytope}
maximizes over \emph{reparametrizations}: messages $M_{ji}(a)$ added to site
$i$'s term and subtracted from edge $(ij)$'s leave $\energy$ unchanged at
every labelling. Cut the edges into chains monotone in a fixed site order,
each edge in one chain and site $i$ in $n_i$ of them, and give each chain the
reparametrized site terms divided by $n_i$ and the reparametrized edge terms
whole. The chains sum to $\energy$, so the sum of their minima --- each a
Viterbi recursion --- is at most $\min_\sigma \energy(\sigma)$ \emph{for any
messages}. Sequential tree-reweighted message passing \citep{kolmogorov2006}
visits the sites in the order and back, and at site $i$ sends each later
neighbour $j$
\begin{equation}
  M_{ij}(b) = \min_a \Big[ \tfrac{1}{n_i}\big(\theta_i(a)
    + \textstyle\sum_{k \sim i} M_{ki}(a)\big) - M_{ji}(a) + \theta_{ij}(a,b) \Big],
  \label{eq:trws-message}
\end{equation}
under which the chain bound does not decrease. For a Potts edge the minimum
over $a$ is the smaller of the diagonal term and the least off-diagonal one,
$O(q)$ per message rather than $O(q^2)$. The iteration stops at a fixed point
of the updates, which need not be the maximum of the dual: coordinate ascent
on a non-smooth objective can stall, and on a frustrated lattice it does.

\paragraph{What pins it.}
Enumeration bounds every iterate from above, and on a tree and a chain the
bound meets the enumerated optimum. At convergence it is compared with a
second coordinate ascent on the same dual that moves an edge's share against
its two sites; where both reach the maximum they agree to $10^{-9}$, and on
two frustrated $3 \times 3$ lattices the chains stop below the second, which
is recorded rather than tolerated. The explicit linear program referees the
maximum itself. What a bound meeting a labelling's energy establishes is that
labelling's optimality, with no oracle consulted; what a gap establishes is an
interval, not which end of it is loose.

\subsection{Simulated bifurcation}
\label{sec:potts-bifurcation}
Every route above is sequential over sites or over labels. Simulated
bifurcation \citep{goto2019, goto2021} relaxes each site and label to an
oscillator $x_{i,a} \in [-1, 1]$ with momentum $y_{i,a}$ and integrates all of
them at once. With one-hot rows the energy is $-S(x)$ for
\begin{equation}
  S(x) = \sum_i \sum_a h_{i,a}\, x_{i,a} + \sum_{(ij)} J_{ij} \sum_a x_{i,a}\, x_{j,a},
  \label{eq:bifurcation-score}
\end{equation}
which is multilinear on the box, so its maximum over the box sits at a corner
and the relaxation adds no optimum the discrete problem lacks. The dynamics
ascend $S$ under a restoring term $-(a_0 - a(t))\,x$ whose strength falls to
zero along a ramp $a(t)$, so each oscillator bifurcates from the origin toward
$\pm 1$ as the ramp runs out; a coordinate that leaves the box is set to its
sign with its momentum zeroed, Algorithm~\ref{alg:simulated-bifurcation}. The
labelling is the arg max over $a$ at each site and its energy is the
discrete one, never $S$. The force scale $c_0$ is read from the instance as
one half over the largest drive a site can see, the sum of its $|J_{ij}|$ and
its largest $|h_{i,a}|$: the dense-matrix scale of the original leaves a
per-site field as the whole drive and returns the field-only labelling.

\paragraph{Regime.}
A heuristic with no bound. It is the one route whose inner loop is an
elementwise update over every site, which is what a device runs; it earns its
place where couplings decide the ground state --- the frustrated lattice and
the planted glass, where the cut does not apply --- and loses to a single
descent where a per-site field does.

\paragraph{What pins it.}
The cut at two labels, exact at any size; enumeration at nine sites and on the
declared glass; the dual bound of Section~\ref{sec:frustrated:algorithms} where
neither reaches; and the same labelling from the NumPy and the tensor paths.

\subsection{The heat bath, Gibbs sampling and cluster moves}
...
detailed balance (Appendix~\ref{app:detailed-balance}). \textbf{Two sign
conventions.} Here, as in Equation~\ref{eq:potts}, $h$ is a log-weight: the
energy is $-\sum_i h_i - J\sum[\text{agree}]$. A field defined as an energy
the labels minimize, as the coupled model's $H$ of
Equation~\ref{eq:external-field} is, enters every sampler and solver here as
$h = -H$, and the convention a field is in is declared where the field is
built, so the negation is made once.
...
\section{Algorithms: Frustrated Lattices}
\label{sec:frustrated:algorithms}
...
\section{Algorithms: The Coupled Spatio-Sequential Model}
\label{sec:coupled:algorithms}
...
\begin{equation}
  \nabla_{\theta} \log p(x \mid \ell, \theta)
    = \sum_{n, m} \indicator[\ell_n = m] \sum_{s, k_{s,m}}
...
\end{equation}
an energy, which the samplers take negated
(Section~\ref{sec:potts-sampling}); and the label update is the ground state of a Potts model in that field,
\begin{equation}
  \ell^\star = \arg\min_{\ell}\ \beta \Big[
      \sum_{\langle n,n' \rangle} J_{nn'}\,\indicator[\ell_n \ne \ell_{n'}]
    + \sum_{n, m} \indicator[\ell_n = m]\, h_{nm} \Big],
...
Equation~\ref{eq:external-field} in its accept step. That $H_{nm}$ is an
energy, so it enters the move as $h = -H$ under the conventions of
Section~\ref{sec:potts-sampling}, and the step's $e^{-\beta \Delta H}$ is
Equation~\ref{eq:cluster-accept}. At $\beta \to 0$ no bond
forms, so the cluster is the root alone and the move is a single-site
Metropolis update with a uniform proposal, not a heat-bath draw; the two
coincide only at $\beta = 0$, where both relabel uniformly. At finite $\beta$
a single-site cluster has positive probability, so the move with its accept
step is irreducible on its own. At $\beta \gg 1$ it is the greedy entrapment
of iterated conditional modes; annealing in $\beta$ moves between them.
...
\State draw a root $\rho$ uniformly from the nodes; $\mu \gets \ell_\rho$;
       $\mathcal{C} \gets \{\rho\}$; frontier $Q \gets \{\rho\}$
\While{$Q \ne \varnothing$}
  \State take any $n$ from $Q$ \Comment{the order does not change the cluster's law}
...
\subsection{The data-sampled burn-in}
...
Bregman divergence in place of squared Euclidean distance
\citep{arthur2007,banerjee2005}: candidates are drawn proportionally to how
badly the current parameters explain them \emph{relative to the best the
family does at each}, constrained to the observations the current labels and
states assign to the class being seeded. The $8(\ln K + 2)$
...
\section{Algorithms: LDPC Decoding}
\label{sec:ldpc:algorithms}

\subsection{Sum-product in the log domain}
...
to the product of theirs. The hard decision $\hat c_i = \indicator[\tilde L_i
...
\section{Algorithms: Turbo Decoding}
\label{sec:turbo:algorithms}

\subsection{BCJR and the extrinsic exchange}
...
\begin{equation}
  ...
  \label{eq:bcjr-recursions}
\end{equation}
and the bitwise ratio the decoder reports is their product over the edges
each input bit labels,
\begin{equation}
  \tilde L_t = \log
...
\subsection{Successive cancellation, and the list}
\label{sec:polar:algorithms}
Equation~\ref{eq:polar-transform}'s butterfly is decoded one input at a time.
The recursion halves the block at each step: the left half sees the soft
combination $f$ of the two halves' ratios, and the right half sees the second
half corrected by what the left half decided, $g$
(Equation~\ref{eq:polar-fg}). A frozen input is not decided --- it is known ---
but its evidence still enters the path metric, which is what keeps an unlikely
prefix from ranking beside a likely one. Algorithm~\ref{alg:successive-cancellation}
is one candidate and Algorithm~\ref{alg:polar-list} is $L$ of them
\citep{arikan2009, talvardy2015}.
...
\section{Continuous Sampling: Hamiltonian Monte Carlo}
\label{app:hmc}
\label{sec:hmc}

The sampler every interval that is not a delta-method interval comes from, and
the one Appendix~\ref{app:tempering} runs replicas of. Stated here in full ---
the integrator, the acceptance, the warm-up that sets both, and what each is
held to --- because a section of the main text cites it rather than deriving
it.
...
\subsection{The integrator: leapfrog against Yoshida}
\label{app:hmc:integrator}
Every integrator here is a composition of the second-order kick--drift--kick
step, differing only in the sub-step lengths: a momentum half-kick, a position
drift, a second half-kick, with the adjacent half-kicks of consecutive
sub-steps merging into one. The leapfrog method is the trivial composition, one
sub-step of full length, and costs one gradient per step. Yoshida's fourth-order
method is the palindromic triple jump $(w_1, w_0, w_1)$ with
\begin{equation}
  w_1 = \frac{1}{2 - 2^{1/3}}, \qquad w_0 = -\frac{2^{1/3}}{2 - 2^{1/3}},
  \qquad 2 w_1 + w_0 = 1,
  \label{eq:yoshida}
\end{equation}
and costs three. Both are reversible --- the weights are a palindrome --- and
both are volume-preserving, which is what makes $\Delta \energy$ the whole of
the acceptance ratio for either; the order of accuracy changes the cost of a
draw and never the distribution drawn from. That $w_0 < 0$ is not a sign error
and is not avoidable: no symmetric composition of a second-order method reaches
fourth order with positive coefficients, so the middle sub-step runs backwards
in time and a plotted trajectory is non-monotone.

Whether the higher order pays is a measurement at equal \emph{gradient
evaluations}, and on these targets it does not. A fourth-order method pays
where the step is limited by accuracy; here it is limited by \emph{stability}.
Yoshida's largest sub-step is $|w_0| = 1.70$ times the nominal step, so its
stability limit in $\varepsilon$ is a factor of about $0.6$ of leapfrog's,
and three gradients per step multiply that. On the Potts posterior the
regression suite pins the consequence: leapfrog reaches an acceptance of
$0.7$ within 21 gradients per trajectory, where Yoshida needs at least 60 and
at 61 accepts nothing. Leapfrog is therefore the default, and the composition
is carried as an object with declared weights, order and cost so that the
comparison is at equal evaluations rather than at equal steps.

\subsection{Adaptation, as a warm-up}
...
\begin{align}
  \bar h_m &= \Big(1 - \frac{1}{m + t_0}\Big)\bar h_{m-1}
             + \frac{\delta - \alpha_m}{m + t_0}, \notag \\
  \ln \varepsilon_m &= \mu - \frac{\sqrt{m}}{\gamma}\,\bar h_m, \notag \\
  \ln \bar\varepsilon_m &= m^{-\kappa}\ln \varepsilon_m
             + (1 - m^{-\kappa})\ln \bar\varepsilon_{m-1},
...
\subsection{The regime the adaptation sits in}
...
\subsection{What a warm-up buys}
\label{app:hmc:warmup}
The warm-up is not free --- its draws are discarded --- so what it buys is
measured on a target whose answer is known. Figure~\ref{fig:hmc-warmup} runs
one adapted chain and one at fixed step and unit mass on the same correlated
Gaussian, from the same start and at the same starting step. That Gaussian's
two directions differ in width by a factor of 534, so at unit mass one step
size must serve both and no value serves either well: above the narrow
direction's stability limit the chain diverges, and below it the wide
direction is crossed by a random walk. The adapted chain's running mean
reaches the last tenth of a standard deviation at draw 211 and the fixed
chain's at 323, and the separation that matters is the cost of a draw ---
$2.08 \times 10^{-2}$ effective draws per gradient against
$5.85 \times 10^{-3}$, a factor of 3.6, with the warm-up's own gradients
charged to the chain that spent them. Both chains are correct: each is a
kernel with the right stationary distribution, and each recovers the exact
mean inside four standard errors at its own effective sample size. So what
separates them is cost per effective draw and nothing else, which is the only
claim a warm-up can make.

\qacaptionread{figures/hmc_warmup_caption.txt}{\hmcWarmupCaption}
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{figures/hmc_warmup}
  \caption{\hmcWarmupCaption}
  \label{fig:hmc-warmup}
\end{figure}
...
A temperature is a knob on the same model, and three methods turn it: a
schedule that lowers it, an ensemble that holds several at once, and a limit
that removes it. They are collected here because every problem with a discrete
search cites one of them, and because the comparison that decides between them
is one comparison, stated at the end.

\subsection{Schedules, annealing and replica exchange}
...
\subsection{Setting the ladder from what it measures}
\label{app:tempering:ladder}
...
The Hamiltonian ladder a tempering start runs is set the same way. A
measurement is a short run of Algorithm~\ref{alg:parallel-tempering} whose
step is one Hamiltonian transition per replica (Appendix~\ref{app:hmc}), $n$
rounds on a candidate of $R$ rungs costing $nR$ transitions, read either for
the exchange acceptance of every pair against the band or for the fraction of
visits made by walkers on their way up, which the feedback placement
of~\citet{katzgraber2006} redistributes at fixed $R$. Every measurement
draws from the start's one stream and begins at the lowest-valued point the
warm-up has visited, and so does the run on the settled ladder: the warm-up's
positions are discarded and its transitions are not. A budget
in seconds bounds the run as a ceiling, a round admitted only if the longest
round so far would end inside it. What the settled ladder buys is a
measurement at equal transitions, warm-up included, and not a consequence of
the band: on a three-component count mixture it was no better a start than
the hand ladder given the same transitions.

\subsection{Tempering against a sampler, and against restarts}
\label{app:tempering:comparison}
Three ways to spend a budget on a multimodal continuous objective, and they are
not variants of one method. \emph{Random restart}
(Algorithm~\ref{alg:random-restart}) fits from many starts and reports every
fit with its spread; it assumes nothing, parallelizes without communication,
and finds a basin only if a start lands in it, so on a surface whose basins are
narrow its success is a rate in the number of restarts. \emph{Hamiltonian Monte
Carlo} (Appendix~\ref{app:hmc}) follows the gradient of one density and crosses
a barrier only by paying its height in the acceptance ratio; it is the method
when the question is a \emph{distribution} --- an interval, a posterior mean ---
and it is the wrong instrument for a global minimum, since a chain that has not
crossed a barrier reports the basin it started in with no sign that it did not.
\emph{Tempering} is the one that attacks the barrier: hot replicas cross what
the cold one cannot and an exchange carries what they find down the ladder, at
the price of $R$ chains for one answer and a ladder that must itself be set
(Section~\ref{app:tempering:ladder}).

What follows is an accounting rule rather than a ranking. The budget is the
problem's own unit --- objective evaluations for a fit, sweeps for a chain ---
and every replica's sweeps and every restart's evaluations are charged to it,
the warm-up's and the ladder's included. Under that rule none of the three
dominates, and which wins is a property of the instance: it is measured on the
frustrated lattices of Section~\ref{sec:frustrated}, where the answer is known
from outside at every size, and on the multimodal test functions of
Section~\ref{sec:testfunctions}, where the minimizers are published. What is
asserted in general is only the refusal: a comparison at unequal budgets, or
one quoting the best of many restarts against the single answer of one chain,
has measured the accounting and not the method.
...
A self-contained account of the algorithms a learned proposal
(Section~\ref{sec:policy-gradient}) is drawn from. It is organised on the two
axes that separate them: how an algorithm handles the \emph{state space} ---
a table over discrete states, or a parameterized function over a continuous
one --- and how it \emph{updates an estimate} --- from a full observed return,
or by bootstrapping from its own estimate at the next state. The progression
runs from an exact framework that requires a model of the environment to
scalable approximations that require only samples from it.
Sutton and Barto's chapters are cited at each step \citep{suttonbarto2018}.

Two conventions hold throughout, and both are the searches' rather than the
literature's. The discount is $\gamma = 1$, because the return of
Equation~\ref{eq:return} telescopes to an improvement in the objective and a
discount would prefer improvement found early over the same improvement found
late. And every cost is counted in objective evaluations, so an algorithm that
reaches a better optimum by evaluating more candidates has not won.

\subsection{The decision process, and what is exact in it}
\label{app:rl:mdp}

A finite Markov decision process is a set of states $\mathcal{S}$, a set of
actions $\mathcal{A}(s)$ available at each, a transition law
$p(s' , r \mid s, a)$ and a policy $\pi(a \mid s)$. The state value is the
expected return from $s$ under $\pi$, and it satisfies the Bellman equation
\begin{equation}
  V^{\pi}(s) = \sum_{a} \pi(a \mid s) \sum_{s', r}
    p(s', r \mid s, a)\,\big[\, r + V^{\pi}(s') \,\big],
  \label{eq:bellman}
\end{equation}
with the action value $Q^{\pi}(s, a)$ the same expectation with the first
action fixed \citep[Ch.~3]{suttonbarto2018}. Equation~\ref{eq:bellman} is a
linear system in $|\mathcal{S}|$ unknowns, so on a state space small enough to
enumerate every quantity below is available exactly --- and that is what makes
the machinery refereeable here: an environment small enough to enumerate is the
oracle for a learning algorithm, as a tree small enough to enumerate is for a
search.

The optimal values satisfy the same equation with a maximum in place of the
policy average,
\begin{equation}
  V^{\star}(s) = \max_{a} \sum_{s', r} p(s', r \mid s, a)\,
    \big[\, r + V^{\star}(s') \,\big],
  \label{eq:bellman-optimality}
\end{equation}
and a policy greedy in $V^{\star}$ is optimal. Everything that follows is a
way of solving Equation~\ref{eq:bellman-optimality} with less than a model, or
with less than a table.

In the searches of Section~\ref{sec:policy-gradient} the reward is the
improvement in the objective, so the undiscounted return telescopes,
\begin{equation}
  G = \sum_{t=0}^{T-1} R_t
...
Dynamic programming assumes the transition law is known and the states are
enumerable, and solves Equation~\ref{eq:bellman} by iteration
\citep[Ch.~4]{suttonbarto2018}. \emph{Policy evaluation} sweeps
\begin{equation}
  V_{k+1}(s) \gets \sum_{a} \pi(a \mid s) \sum_{s', r}
    p(s', r \mid s, a)\,\big[\, r + V_{k}(s') \,\big],
  \label{eq:policy-evaluation}
\end{equation}
a contraction whose fixed point is $V^{\pi}$; \emph{policy improvement} sets
$\pi'(s) = \arg\max_a Q^{\pi}(s, a)$, which the policy improvement theorem
guarantees is no worse at every state. \emph{Policy iteration} alternates the
two to an optimal policy in finitely many steps on a finite process.
\emph{Value iteration} truncates the evaluation to one sweep, applying
Equation~\ref{eq:bellman-optimality} as an assignment, and converges to
$V^{\star}$ from any start.

Both are \emph{bootstrapped} --- an estimate is updated from other estimates
--- and both are \emph{exact}, in that the expectation is computed rather than
sampled. The cost is one pass over every state and action per sweep, which is
the reason they do not reach the searches of this document: the state of a
topology search is a topology with its fitted parameters, and the transition
law is a move set whose reward is a fit. Where an instance \emph{is} small
enough --- the enumerable lattice environment --- value iteration supplies the
$V^{\star}$ every approximate method below is scored against, and that is what
it is used for here.

\subsection{Monte Carlo methods}
\label{app:rl:mc}

A Monte Carlo method removes the model. It runs an episode under the policy,
observes the return $G_t$ from each visited state, and averages
\begin{equation}
  V(s_t) \gets V(s_t) + \alpha\,\big[\, G_t - V(s_t) \,\big],
  \label{eq:mc-update}
\end{equation}
which for a running mean with $\alpha = 1/n$ is the sample average of the
returns observed from $s$ \citep[Ch.~5]{suttonbarto2018}. It bootstraps from
nothing, so its estimate is unbiased for $V^{\pi}$ and its variance is the
return's; it needs episodes that terminate, which the searches here have by
construction, since an episode ends at a step budget or at a state no move
improves.

Two properties matter for what follows. A Monte Carlo estimate at one state is
independent of the estimates at the others, so it does not propagate an error
--- and does not propagate information either, which is why it is slow where
states are visited rarely. And control by Monte Carlo needs every action tried
from every state, supplied either by exploring starts or by an
$\epsilon$-soft policy, the same requirement the exploring wrapper of
Section~\ref{sec:policy-gradient} meets and for the same reason.

\subsection{Temporal-difference learning}
\label{app:rl:td}

Temporal-difference learning takes the bootstrapping of dynamic programming
and the sampling of Monte Carlo. One step of experience $(s_t, a_t, r_t,
s_{t+1})$ gives the error and the update
\begin{equation}
  ...
  \label{eq:td-zero}
\end{equation}
which is TD(0) \citep[Ch.~6]{suttonbarto2018}. The target $r_t + V(s_{t+1})$
is biased by the current estimate and has the variance of one transition
rather than of a whole episode: the trade that runs through every method here.

Control splits on \emph{whose} action the target reads. \emph{Sarsa} is
on-policy, reading the action the behaviour policy actually took next,
\begin{equation}
  Q(s_t, a_t) \gets Q(s_t, a_t)
    + \alpha\,\big[\, r_t + Q(s_{t+1}, a_{t+1}) - Q(s_t, a_t) \,\big],
  \label{eq:sarsa}
\end{equation}
so it evaluates the policy it is following, exploration included.
\emph{Q-learning} is off-policy, reading the best action available instead,
\begin{equation}
  Q(s_t, a_t) \gets Q(s_t, a_t)
    + \alpha\,\big[\, r_t + \max_{a} Q(s_{t+1}, a) - Q(s_t, a_t) \,\big],
  \label{eq:q-learning}
\end{equation}
so it learns the greedy policy's values while behaving otherwise. The
difference is not cosmetic for a search that must take a worsening move to
leave a local optimum: Sarsa's values account for the cost of exploring and
Q-learning's do not, and which is wanted depends on whether the exploration is
part of the method or a means of collecting data.

The $n$-step returns interpolate between Equation~\ref{eq:td-zero} and
Equation~\ref{eq:mc-update}, and the eligibility trace of
Section~\ref{app:rl:vfa} is the same interpolation run online
\citep[Ch.~7]{suttonbarto2018}.

\subsection{Value function approximation}
\label{app:rl:vfa}

A table is impossible where the states are a continuum, and pointless where
they are astronomically many and each is visited once --- the regime of every
search in this document, whose states are structures. The value is then
parameterized, $\hat{V}(s; w)$, and the update becomes a gradient step
\citep[Ch.~9]{suttonbarto2018}. \emph{Gradient Monte Carlo} descends the
squared error to the observed return,
\begin{equation}
  w \gets w + \alpha\,\big[\, G_t - \hat{V}(s_t; w) \,\big]\,
    \nabla_w \hat{V}(s_t; w),
  \label{eq:gradient-mc}
\end{equation}
which is stochastic gradient descent on an unbiased target and converges to a
local optimum of the mean squared value error. \emph{Semi-gradient TD}
replaces the return by the bootstrapped target,
\begin{equation}
  w \gets w + \alpha\,\big[\, r_t + \hat{V}(s_{t+1}; w) - \hat{V}(s_t; w) \,\big]\,
    \nabla_w \hat{V}(s_t; w),
  \label{eq:semi-gradient-td}
\end{equation}
and is \emph{semi}-gradient because the target's own dependence on $w$ is not
differentiated: it is not the gradient of any objective, which is why its
convergence holds for linear approximation on-policy and can diverge off-policy
with a function approximator and bootstrapping together --- the deadly triad
\citep[Ch.~11]{suttonbarto2018}. \emph{Eligibility traces} carry a decaying
record of the features recently visited,
\begin{equation}
  z_t = \lambda\, z_{t-1} + \nabla_w \hat{V}(s_t; w),
  \qquad
  w \gets w + \alpha\,\delta_t\, z_t,
  \label{eq:eligibility}
\end{equation}
so one error updates every state that led to it; $\lambda = 0$ is
Equation~\ref{eq:semi-gradient-td} and $\lambda = 1$ is
Equation~\ref{eq:gradient-mc}, and the parameter moves between the two
continuously \citep[Ch.~12]{suttonbarto2018}. The generalized advantage of
Equation~\ref{eq:gae} is the same interpolation written for an advantage
rather than a value.

Two consequences bind what a state's features may be. Generalization is the
point --- experience at one structure must inform a structure like it --- so
the features are what makes two states alike, and a feature that is constant
over a neighbourhood contributes nothing to a softmax over that neighbourhood
and is excluded (Section~\ref{sec:policy-gradient}). And a parameterized value
is scored against $V^{\pi}$ where enumeration supplies it, never against its own
training loss, which it can minimize while estimating the wrong function.

\subsection{Policy gradient}
\label{app:rl:pg}

The methods above derive a policy from values. A policy gradient parameterizes
the policy and optimizes it directly, which is what a search over a
neighbourhood of varying size needs: a softmax over the scores of the available
actions admits a neighbourhood of any size, where an $\arg\max$ over a fixed
action set does not. With $J(\pi)$ the expected return, the policy gradient
theorem gives an expectation over trajectories the policy itself generates
\citep[Ch.~13]{suttonbarto2018}, and REINFORCE is its Monte Carlo estimator,
\begin{equation}
...
greedy baseline is at a matched budget of objective evaluations per decision,
never in wall-clock time.
so the update is Equation~\ref{eq:gradient-mc}'s trade in a second
place: unbiased, and of the return's variance. A baseline $b(s)$ depending on
the state alone leaves the estimator unbiased --- the proof is
Appendix~\ref{app:rl:derivations} --- and the natural choice is the state
value, which turns REINFORCE into the actor--critic below.
...
\subsection{What applies to the problems of this document, and what does not}
\label{app:rl:applies}

The axes decide it. Every search here has a state space no table reaches --- a
topology, a labelling, a configuration --- so the tabular methods of
Sections~\ref{app:rl:dp} to~\ref{app:rl:td} appear only as oracles on an
enumerable instance, where they supply the $V^{\star}$ and the exact policy
gradient that referee an approximate method. The methods that \emph{run} are
the approximating ones, and three properties of these problems choose among
them.

The reward is a difference in the objective, so the return telescopes and is
available exactly at the end of an episode: a Monte Carlo return costs nothing
that the search has not already paid, which is why REINFORCE with a baseline is
the reference estimator here and bootstrapping is an optimization rather than a
necessity. The action set is a neighbourhood whose size varies with the
instance, which excludes every method whose output is indexed by a fixed action
set --- Q-learning over a table of $(s,a)$ pairs among them --- and admits the
softmax policy and the critic over aggregate state features. And an evaluation
is expensive, which makes sample efficiency the whole of the comparison: a
method is measured against greedy search with restarts at a matched budget of
objective evaluations, never at matched wall-clock time and never against a
single greedy run, which stops early and leaves the budget unspent.

What that leaves is a ladder of its own. The linear scorer is the reference,
because a recovery test can state a truth about its weights; the
multilayer-perceptron scorer is measured against it and never assumed better;
the critic is added where the return's variance is what limits the estimator,
and scored against the enumerated $V^{\pi}$; and the planner of
Equation~\ref{eq:puct} is admissible only where a successor can be evaluated
cheaply enough to expand many of them, which on these problems means a learned
model (Section~\ref{sec:planning}) and is not built.

\subsection{Derivations}
\label{app:rl:derivations}
...
An interval is the part of a fit a reader acts on, and two kinds are reported
here. Which applies is decided by the estimand and not by preference: a
continuous parameter gets an interval from the curvature of the likelihood at
the optimum or from a sampled posterior, and a rate --- a coverage, a
recovery fraction, a block error rate --- gets an interval for a binomial
proportion. Reporting the first where the second applies is the standing
error, because a proportion at zero or one has no curvature to read.

\subsection{A continuous parameter: the observed information}
\label{app:delta}
...
Where the asymptotics are what is in doubt, the posterior is sampled
instead (Appendix~\ref{app:hmc}) and the interval read as a quantile of the
draws; the two are compared on the analytic Gaussian, where both are known,
and their disagreement is the measurement that says which regime a fit is in.

\subsection{A rate: an interval for a binomial proportion}

A coverage over $R$ replicates, the fraction of starts reaching the global
optimum, and a block error rate over $R$ transmitted words are all one
estimand: a proportion $\hat{p} = r/R$ from independent trials. The normal
interval $\hat p \pm z\sqrt{\hat p (1 - \hat p)/R}$ is the delta method
applied to a binomial and fails exactly where these quantities live --- at
$\hat p = 0$ or $1$ it has zero width, which is a claim of certainty from a
finite sample. Two intervals are used instead. The Wilson interval inverts the
score test,
\begin{equation}
  \frac{\hat p + \frac{z^2}{2R}}{1 + \frac{z^2}{R}}
  \;\pm\;
  \frac{z}{1 + \frac{z^2}{R}}
  \sqrt{\frac{\hat p (1 - \hat p)}{R} + \frac{z^2}{4R^2}},
  \label{eq:wilson}
\end{equation}
which is non-degenerate at both ends and is what a reported rate carries. The
Clopper--Pearson interval inverts the exact binomial test instead, is
conservative by construction, and is what a \emph{refusal} is stated against:
where no failure is observed in $R$ words, the one-sided bound
$1 - \alpha^{1/R}$ is the largest rate the observation is consistent with, and
it is that number rather than zero that a claim may use.

A rate and its interval are reported with the seed set and the budget behind
them \citep{henderson2018, agarwal2021}; a rate without its trial count is not
a measurement.
...
is Equation~\ref{eq:pruning}; at a leaf $L_u(i) = \indicator[z_u = i]$
...
\subsection{The region-based free energy}
\label{app:kikuchi}
Equation~\ref{eq:bethe-factor} approximates the entropy by one term per factor
and a correction per variable. A \emph{region graph} replaces the factor by any
set of regions $R$ --- a region being a set of variables together with the
factors whose arguments it contains --- ordered by inclusion and closed under
intersection. Each carries a counting number fixed by requiring that every
variable and every factor be counted exactly once,
\begin{equation}
  c_R = 1 - \sum_{A \supsetneq R} c_A,
  \qquad \sum_{R \ni v} c_R = 1 \ \text{for every variable } v,
  \label{eq:counting-number}
\end{equation}
the first being M\"obius inversion over the partial order and the second the
condition it enforces. The region-based free energy is then
\begin{equation}
  F_{K} = \sum_R c_R \sum_{x_R} b_R
          \big(\log b_R - \log \psi_R\big),
  \label{eq:kikuchi}
\end{equation}
with $\psi_R$ the product of the factors the region owns
\citep{yedidia2005, mezard2009}. Two cases place it. With one region per
factor and one per variable, Equation~\ref{eq:counting-number} gives
$c_i = 1 - d_i$ at a variable of degree $d_i$ and
Equation~\ref{eq:kikuchi} \emph{is} Equation~\ref{eq:bethe-factor} --- the
same expression, not an approximation of it, which is what makes the
reduction a test rather than a remark. With the unit cells of a square
lattice as the largest regions, their pairwise intersections are the edges
and the sites sit below those, and the counting numbers come out
$+1$ on a cell, $-1$ on a shared edge and $+1$ at the centre site of a
$3 \times 3$ --- read off Equation~\ref{eq:counting-number} rather than
written down.

A region graph that counts a variable twice returns a number that looks like a
free energy and is not one, so the condition is checked on construction and a
violation is refused rather than run.

\subsection{The evidence gradient is a posterior expectation}
\label{app:logz}
Equation~\ref{eq:logz-gradient} is what makes one message pass serve both the
marginalization and the fit, so it is derived rather than asserted. Write the
unnormalized measure of Equation~\ref{eq:factor-graph} as
$\tilde p(x; \params) = \prod_a \psi_a(x_{\partial a}; \params)$ and
$Z(\params) = \sum_x \tilde p(x; \params)$. Differentiating under the sum,
which is finite here and so needs no further condition,
\begin{equation}
  \frac{\partial \ln Z}{\partial \params}
    = \frac{1}{Z} \sum_x \frac{\partial \tilde p}{\partial \params}
    = \sum_x \frac{\tilde p(x; \params)}{Z}\,
      \frac{\partial \ln \tilde p(x; \params)}{\partial \params}
    = \mathbb{E}_{p(x)}\Big[
        \frac{\partial \ln \tilde p(x; \params)}{\partial \params}
      \Big],
  \label{eq:logz-expectation}
\end{equation}
the middle step being $\partial \tilde p = \tilde p \, \partial \ln
\tilde p$. The logarithm turns the product over factors into a sum, and a
factor's log-potential depends on $x$ only through $x_{\partial a}$, so each
term's expectation is taken under the marginal of that factor's own variables
--- the belief $b_a$, which sum-product returns:
\begin{equation}
  \mathbb{E}_{p(x)}\Big[
      \sum_a \frac{\partial \ln \psi_a(x_{\partial a}; \params)}
                   {\partial \params} \Big]
    = \sum_a \mathbb{E}_{p(x_{\partial a})}
      \Big[ \frac{\partial \ln \psi_a(x_{\partial a}; \params)}
                  {\partial \params} \Big],
\end{equation}
which is Equation~\ref{eq:logz-gradient}. Two consequences are used in the
body. The expectation step of an expectation--maximization is the beliefs and
nothing further, since the quantity its maximization step differentiates is
this same sum; and on a loopy graph the $b_a$ are the Bethe beliefs rather
than the marginals, so the gradient is of an approximate evidence and the
optimum it reaches is reported as the stationary point of one.
...
\subsection{The region-based free energy is stationary at a parent-to-child
fixed point}
\label{app:gbp}
Constrain the region beliefs of Equation~\ref{eq:kikuchi} to be normalized and
consistent wherever regions overlap: a parent's belief marginalized onto a
child must be the child's. Introducing a multiplier for each parent-child
constraint and setting the derivative of Equation~\ref{eq:kikuchi} to zero
gives $b_R \propto \psi_R \prod$ (the multipliers of the constraints $R$ takes
part in), and identifying each multiplier with a message from the parent to the
child turns the consistency constraint into the update the messages are
iterated under --- the same argument as Appendix~\ref{app:bethe}, with a
region in place of a factor. So a fixed point of parent-to-child message
passing is a stationary point of the region-based free energy and conversely
\citep{yedidia2005}.

Three consequences are used in the body. At the Bethe region graph the fixed
point is belief propagation's, which is how the implementation is refereed ---
two routes to one fixed point, agreeing to $1.7 \times 10^{-10}$ on a
$3 \times 3$. Stationarity is not optimality: the functional is not convex, so
$-F_K$ may sit either side of $\log Z$ and no claim here is a bound. And
nothing guarantees a fixed point is \emph{reached}: the map has none of belief
propagation's contraction properties, damping slows the approach rather than
creating one, and a run that does not settle is refused --- a free energy read
off messages that never settled estimates nothing.
...
\paragraph{The decoupled bounds, \eqref{eq:decoupled-energy-bound} and
\eqref{eq:saturated-bound}.}
$\energy(\sigma) = -\sum_i h_i(\sigma_i) - \sum_{\langle i,j \rangle} J_{ij}
[\sigma_i = \sigma_j]$ is a sum of terms, each bounded below by its own
minimum over the configuration, and those minima are attained termwise by
$\max_\sigma h_i(\sigma)$ and by $\max(J_{ij}, 0)$; no configuration need
attain all of them at once, which is the slack. Exponentiating,
$Z \le k^N e^{-E_{\mathrm{dec}}}$ as in \eqref{eq:ground-state-bounds}, which
is the right inequality. For the left, $J \ge 0$ makes every coupling term of
each exponent non-negative, so dropping all of them lowers every summand of
$Z$, and what remains factorizes over sites. Both are one pass over the sites
and one over the edges, against one exact tree pass per edge for
\eqref{eq:spanning-tree-bound}, which is why they are the bracket at the
sizes that bound does not reach.

\section{The Analytic Results the Implementation Rests On}
\label{app:analytic-results}

Every claim this book makes about an implementation rests on a result that is
true independently of it. This appendix is the inventory: what each result
says, what it assumes, and where the text uses it. Where a derivation is
given, the row points at it rather than repeating it; where none is given, the
result is standard and the assumption is the part worth stating, because the
assumption is what an instance can violate.

Three generated tables carry the other half of the picture, and are the place
to look for a method rather than a result: Table~\ref{tab:declared-instances}
states every instance the registry declares and the shape each one has,
Table~\ref{tab:ladder} gives the oracle ladder per problem --- each rung, the
rung it is pinned against, the unit it spends, and what pins the pair --- and
Table~\ref{tab:algorithms-problems} says which algorithms apply to which
problem and what referees each. All three are written from the repository
rather than typed, so a method added without a referee is visible in them as a
gap rather than absent from them altogether.

\subsection{Results whose derivation is given here}

Five of the results the implementation uses most are derived in
Section~\ref{app:proofs}, and are listed here only so the inventory is
complete: the factorization of a tree likelihood over internal states
(Section~\ref{app:pruning}); the same marginalization on a chain, which is the
forward--backward recursion; sum-product on a tree and its loopy stationary
point, with the Bethe and region-based free energies it is stationary at;
detailed balance for a cluster move in a field, which is why a field forces an
accept step the zero-field construction does not need; and the exchange ratio
in parallel tempering (Section~\ref{app:tempering}).

\subsection{Results the text uses and does not derive}

\begin{longtable}{@{}p{0.27\textwidth}p{0.34\textwidth}p{0.31\textwidth}@{}}
  \caption{The analytic results the implementation rests on, what each
  assumes, and the statement that uses it. An assumption in the middle column
  is the thing an instance can violate --- which is what makes the row worth
  stating rather than the result, all of which are
  standard.}\label{tab:analytic-results}\\
  \toprule
  Result & What it assumes & Where it is used \\
  \midrule
  \endfirsthead
  \toprule
  Result & What it assumes & Where it is used \\
  ...
  The Jukes--Cantor transition matrix in closed form, and its inverse as an
  expected distance
  & Equal equilibrium frequencies and one substitution rate. The inverse is
  defined only below the saturation distance, above which an observed
  difference carries no information about elapsed time
  & Section~\ref{sec:phylo}: the simulator draws from it, and a distance
  estimate inverts it \\

  The Hadamard conjugation of a two-state process on a tree
  & Two states and a reversible process; the edge spectrum and the sequence
  spectrum are related by an orthogonal transform
  & Section~\ref{sec:phylo}: an evaluator that shares no traversal with the
  recursion, which is what makes it an independent answer rather than a second
  spelling \\

  The transfer matrix for a chain, and its powers by repeated squaring
  & A chain, or a strip of fixed width read as a chain of columns. Exactness
  is in the model's structure, not in the arithmetic: a wider strip is exact
  at a cost exponential in the width
  & Section~\ref{sec:potts}: the exact normalizer a sampler's estimate is read
  against, and the $\log n$ route to the $n$th power \\

  Maximum-flow/minimum-cut duality
  & Pairwise terms that are submodular at two labels. Beyond two labels the
  cut is no longer the optimum, and an expansion move gives a bound rather
  than an answer
  & Section~\ref{sec:potts}: the exact ground state at two labels, and the
  move that reaches more labels through it \\

  The BCJR recursion and the extrinsic split
  & A trellis, and information exchanged between constituent decoders that
  excludes what the receiving decoder already knows --- the split is what
  keeps the iteration from feeding a decoder its own belief
  & Sections~\ref{sec:ldpc} and~\ref{sec:turbo}: the per-bit posterior on a
  trellis, and the quantity the turbo iteration passes \\

  Channel polarization and the reliability order
  & A symmetric binary-input channel, and a transform whose synthetic channels
  separate toward useless and noiseless as the block grows. The order is a
  property of the channel parameter, so a frozen set earned at one parameter
  is not earned at another
  & Section~\ref{sec:polar}: the frozen set, and the order successive
  cancellation decodes in \\

  The exponential family's Bregman divergence
  & A density of the form $\exp(-D_\phi(y,\mu))\,b_\phi(y)$. Squared Euclidean
  distance is the Gaussian's case, which is why the general rule must reduce
  to $k$-means$++$ there and nowhere else by accident
  & Section~\ref{sec:mixture}: $D^2$ sampling under the family's own
  divergence rather than under a negative log density, which normalizes rather
  than shifts \\

  The Gumbel--softmax relaxation
  & A temperature above zero throughout, and exactness only at the corners of
  the simplex: the relaxed objective equals the discrete one there and
  nowhere between
  & Section~\ref{sec:relaxation}: a gradient route over discrete states, whose
  estimator's bias and variance are measured rather than assumed small \\

  McNemar's exact test
  & Paired outcomes over shared seeds. The exact form is the two-sided
  binomial tail on the discordant pairs, which is what lets a comparison over
  a few dozen starts state a result at all
  & Section~\ref{sec:comparisons}: the test a budget-matched comparison is
  settled by, in place of two means read side by side \\

  The chi-square goodness-of-fit test against an exact law
  & Independent draws, which a Markov chain does not give: the thinning
  interval is part of the test, not an implementation detail, and an
  unthinned chain fails the assumption before it fails the test
  & Section~\ref{sec:potts}: how a sampler is shown to draw from the
  distribution it claims \\

  The enumeration counts: $(2n-5)!!$ unrooted topologies, $k^{m}$ labellings
  of $m$ internal nodes, $q^{n}$ configurations
  & Nothing --- they are counts. They are here because they decide what
  ``enumerable'' means, and every exact oracle in this book is bounded by one
  of them
  & Throughout: the size at which an exact answer stops being available, and
  therefore the size at which a claim changes its referee \\
\end{longtable}
...
\begin{algorithm}[htbp]
\caption{Simulated bifurcation, Equation~\ref{eq:bifurcation-score}}
\label{alg:simulated-bifurcation}
\begin{algorithmic}[1]
\State $x_{i,a} \gets$ small uniform draws, $y_{i,a} \gets 0$, $c_0 \gets 1 / (2 \max_i (\sum_{j \sim i} |J_{ij}| + \max_a |h_{i,a}|))$
\For{$t = 0, \dots, T-1$}
  \State $a \gets a_0\, t / T$
  \State $f_{i,a} \gets h_{i,a} + \sum_{j \sim i} J_{ij}\, x_{j,a}$ \Comment{one gather over the edges}
  \State $y \gets y + \Delta t\,\big(-(a_0 - a)\, x + c_0\, f\big)$
  \State $x \gets x + \Delta t\, a_0\, y$
  \State where $|x_{i,a}| > 1$: $x_{i,a} \gets \operatorname{sign}(x_{i,a})$, $y_{i,a} \gets 0$
\EndFor
\State \Return $\sigma_i = \arg\max_a x_{i,a}$ \Comment{scored by $\energy(\sigma)$, never by $S(x)$}
\end{algorithmic}
\end{algorithm}
...
\For{iteration $1$ to the cap}
  ...
  \State $\tilde L_i \gets L_i + \sum_{b \in \partial i} m_{b \to i}$;
         $\hat c_i \gets \indicator[\tilde L_i < 0]$
...
\EndFor
\State \Return $\hat u_i = \indicator[(\Pi^{-1}\tilde L^{2})_i < 0]$, and
...
\begin{algorithm}[htbp]
\caption{Successive cancellation on the butterfly,
Equation~\ref{eq:polar-fg}}
\label{alg:successive-cancellation}
\begin{algorithmic}[1]
\Function{Decode}{$L$, $\mathcal{F}$}
  \Comment{$L$: the ratios this subtree sees; $\mathcal{F}$: its frozen mask}
  \If{$|L| = 1$}
    \State \Return $0$ if $\mathcal{F}_0$, else $\indicator[L_0 < 0]$
  \EndIf
  \State $a, b \gets$ the first and second halves of $L$
  \State $\hat u^{\ell} \gets$ \Call{Decode}{$f(a, b)$, first half of
         $\mathcal{F}$}
  \State $\hat u^{r} \gets$ \Call{Decode}{$g(a, b, \hat c^{\ell})$, second half
         of $\mathcal{F}$}
  \State \Return $(\hat u^{\ell}, \hat u^{r})$ and the codeword bits
         $(\hat c^{\ell} \oplus \hat c^{r}, \hat c^{r})$
\EndFunction
\State \Return the message bits of $\hat u$ on $\mathcal{I}$
\end{algorithmic}
\end{algorithm}

\begin{algorithm}[htbp]
\caption{Successive-cancellation list decoding, $L$ prefixes
\citep{talvardy2015}}
\label{alg:polar-list}
\begin{algorithmic}[1]
\State one path, metric $0$
\For{each input $i$ in order, through the recursion of
     Algorithm~\ref{alg:successive-cancellation}}
  \If{$i$ is frozen}
    \State every path takes $\hat u_i = 0$ and pays
           $\log(1 + e^{-(1 - 2\hat u_i) L_i})$
  \Else
    \State every path forks into $\hat u_i \in \{0, 1\}$, each paying the same
           penalty
    \State keep the $L$ paths of smallest metric, and reorder the arrays of
           every surviving path to match
  \EndIf
\EndFor
\State \Return the message of the surviving path of smallest metric
\end{algorithmic}
\end{algorithm}
