...
| [`ldpc.ipynb`](ldpc.ipynb) | Low-density parity-check codes: Gallager's (3,6) ensemble, and the bicycle construction at matched length and degrees | The general sum-product on the parity-check factor graph; enumeration of all 32,768 codewords of a cycle-free code, and of the 64 of the bicycle fixture; the density-evolution threshold 0.4294; the two exact decodings, which bound the decoder on the quantity each minimizes |
