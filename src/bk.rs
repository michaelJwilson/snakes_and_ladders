...
mod profile {
    ...
    #[ignore]
    fn profile_cut() {
        let dir = std::env::var("CUT_DIR").unwrap();
        for side in [142usize, 284] {
            let field = read_f64(&format!("{dir}/field{side}.bin"));
            let edges: Vec<i64> = std::fs::read(format!("{dir}/edges{side}.bin"))
                .unwrap()
                .chunks_exact(8)
                .map(|b| i64::from_le_bytes(b.try_into().unwrap()))
                .collect();
            let coupling = read_f64(&format!("{dir}/coupling{side}.bin"));
            let n = side * side;
            let reps = std::env::var("REPS")
                .ok()
                .and_then(|r| r.parse().ok())
                .unwrap_or(9);
            let (mut build, mut solve) = (vec![], vec![]);
            for _ in 0..reps {
                let t = Instant::now();
                let mut graph = Graph::from_edges(n, &edges, &coupling, &coupling).unwrap();
                for node in 0..n {
                    let (a, b) = (-field[2 * node], -field[2 * node + 1]);
                    let o = a.min(b);
                    graph.add_terminal(node, b - o, a - o).unwrap();
                }
                build.push(t.elapsed().as_secs_f64());
                let t = Instant::now();
                let _ = graph.solve();
                solve.push(t.elapsed().as_secs_f64());
            }
            build.sort_by(f64::total_cmp);
            solve.sort_by(f64::total_cmp);
            eprintln!(
                "{side}: build {:.2} ms solve {:.2} ms",
                build[reps / 2] * 1e3,
                solve[reps / 2] * 1e3
            );
        }
