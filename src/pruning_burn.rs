...
//! counterpart here. `python/snakes_and_ladders/likelihood/pruning_burn.py`
...
pub fn pruning_gradient_impl(
    ...
    let base = Tensor::<Backend, 3>::from_data(
        TensorData::new(vec![1.0 / kf; k * k], [1, k, k]),
        &device,
    );
    ...
    let pi_column =
        Tensor::<Backend, 2>::from_data(TensorData::new(pi.to_vec(), [k, 1]), &device);
...
pub fn pruning_gradient(
    ...
    pruning_gradient_impl(branch_length, &children, observations, k, pi, weight, rescale)
...
mod tests {
    ...
    fn fixture() -> (Vec<f64>, Vec<Vec<usize>>, Vec<i64>, Vec<i64>, usize) {
    ...
    fn test_value_matches_the_hand_written_kernel() {
        ...
        let oracle =
            pruning_log_likelihood_impl(&branch_length, &children, observations, 4, &[0.25; 4], true)
                .unwrap();
