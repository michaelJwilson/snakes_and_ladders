...
# The shape the handle exists for: many topologies against one alignment.
# One cell, not two -- the trend with `n x L` is already established above,
# and what this pair reports is the difference between paying for the
# alignment once and paying for it per candidate.
SCREENING_CANDIDATES = 20


def _candidates(alignment: dict[str, np.ndarray]) -> list[Node]:
    rng = np.random.default_rng(444)
    names = sorted(alignment)
    return [
        with_uniform_branch_lengths(random_topology(names, rng), 0.1)
        for _ in range(SCREENING_CANDIDATES)
    ]


def test_screening_sweep_through_the_function(benchmark: BenchmarkFixture) -> None:
    """Every candidate re-marshals the alignment: the current binding's cost."""
    tau, pi, alignment = _problem(CELLS[0])
    candidates = _candidates(alignment)

    def sweep() -> float:
        return sum(
            pruning_rust.log_likelihood(candidate, 4, pi, alignment)
...
def test_screening_sweep_through_the_handle(benchmark: BenchmarkFixture) -> None:
    """The alignment crosses once; report against the function above.

    The handle is built inside the timed callable, so the comparison charges
    it for the construction it saves per candidate. The two return the same
    sum bitwise -- pinned in
    ``tests/regression/likelihood/test_pruning_rust.py``.
    """
    tau, pi, alignment = _problem(CELLS[0])
    candidates = _candidates(alignment)

    def sweep() -> float:
        problem = pruning_rust.PruningProblem(4, pi, alignment)
        return sum(problem.log_likelihood(candidate) for candidate in candidates)
