...
def regenerated_and_committed() -> set[str]:
    ...
    for stem in cited_stems(*selection_documents()):
        ...
        for name in (f"{stem}_caption.txt",):
...
@pytest.mark.structural
def test_the_figures_no_document_cites_are_not_this_build_s() -> None:
    """The committed figures split into what this build writes and what does not.

    `infra/build_documents.sh` regenerates the cited figures alone; the rest are
    committed and regenerated by `infra/release.sh` (`--all`). The split is the
    reason the contract table says "cited stem" rather than "figure", so it is
    checked rather than assumed: every tracked file under the figures directory
    belongs to some manifest stem, and each stem contributes exactly two.
    """
    cited = set(cited_stems(*selection_documents()))
    every = {spec.stem for spec in FIGURES}
    assert cited < every, (
        "every manifest figure is cited; the release-gate split is gone"
    )
