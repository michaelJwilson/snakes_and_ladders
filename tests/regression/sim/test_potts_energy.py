...
The two sum the edge terms in different orders --- one gather and a pairwise
sum here, a term per edge there --- so they agree to a relative `1e-12` and
not bitwise (issue #341). The four instances are the fixtures the
consolidation was validated on: a chain, a lattice, a glass with couplings of
both signs, and a lattice whose field varies by site. A fifth is the size of
the release-q10 rung, where issue #1044 found the edge term's value moving
with the BLAS thread count.
"""
