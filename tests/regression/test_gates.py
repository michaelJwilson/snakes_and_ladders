...
the markers mean -- is written in twelve files, and they had already drifted:
...
policy down once; this reads each of the twelve back out of its own source text
...
#: `**Nine rows in 30 s**` and `the other seven rows`: the review table's size
#: and budget, and the count that has to follow from the first.
_ROW_COUNT = re.compile(r"\*\*(\w+) rows in (\d+) s\*\*")
...
def test_pyproject_registers_the_markers_and_the_text_gates_names() -> None:
...
@pytest.mark.critical
@pytest.mark.structural
def test_the_kind_module_names_the_same_markers_gates_names() -> None:
...
@pytest.mark.critical
@pytest.mark.structural
def test_the_workflow_runs_the_pull_request_and_main_gates_gates_names() -> None:
...
@pytest.mark.critical
@pytest.mark.structural
def test_the_release_gate_runs_every_tier_gates_names() -> None:
...
@pytest.mark.critical
@pytest.mark.structural
def test_the_shell_scripts_export_the_caps_gates_names() -> None:
...
@pytest.mark.critical
@pytest.mark.structural
def test_the_review_gate_runs_the_rows_gates_names() -> None:
...
@pytest.mark.critical
@pytest.mark.structural
def test_dev_md_counts_the_review_gate_rows_gates_names() -> None:
    ...
    assert stated, "DEV.md no longer states the review table's size and budget"
    ...
    assert int(stated.group(2)) == gates.REVIEW_GATE.budget_seconds
    assert _WORDS[remaining.group(1).lower()] == len(gates.REVIEW_GATE.rows) - 2


@pytest.mark.critical
@pytest.mark.structural
def test_dev_md_states_the_tier_budgets_gates_names() -> None:
