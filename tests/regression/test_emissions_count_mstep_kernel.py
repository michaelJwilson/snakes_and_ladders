...
def test_the_tails_sum_is_the_digamma_difference() -> None:
    ...
    tails = emissions._weight_tails(counts, weights)
...
def test_the_kernel_is_the_batched_solves_on_the_emission_mixture(tier: str) -> None:
    ...
    rust = emissions._solve_dispersion_rust(totals, posterior, means)
    oracle = emissions._solve_dispersion_batched(totals, posterior, means)
    ...
    rust_bb = emissions._solve_beta_binomial_rust(
        successes, posterior, totals, rates, concentrations
    )
    oracle_bb = emissions._solve_beta_binomial_batched(
        successes, posterior, totals, rates, concentrations
    )
...
def test_the_kernel_takes_a_trial_count_per_component() -> None:
    ...
    rust = emissions._solve_beta_binomial_rust(
        values[:, 1], posterior, trials, rates, concentrations
    )
    oracle = emissions._solve_beta_binomial_batched(
        values[:, 1], posterior, trials, rates, concentrations
    )
