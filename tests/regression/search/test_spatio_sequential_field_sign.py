...
annealed start therefore pass ``-H``, and this pins it: at a temperature low
...
ground-state equation never increases. Handed ``+H``, the same steps raise the
...
def _traces(sign: float) -> tuple[list[float], list[float]]:
    ...
    for _ in range(STEPS):
        _wolff_update(labels, params.graph, sign * -field, beta, rng)
...
def test_a_cold_wolff_step_never_raises_the_label_objective() -> None:
    ...
    objective, field_term = _traces(1.0)
